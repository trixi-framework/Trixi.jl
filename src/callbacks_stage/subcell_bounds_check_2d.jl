...
@muladd begin
    ...
    @inline function check_bounds(
            u, equations::AbstractEquations{2}, # only works for 2D
        ...
        if local_twosided
            for v in limiter.local_twosided_variables_cons
                ...
                @batch reduction = ((max, deviation_min), (max, deviation_max)) for element in eachelement(
                        solver,
                        cache
                    )
        ...
        if local_onesided
            for (variable, min_or_max) in limiter.local_onesided_variables_nonlinear
                ...
                @batch reduction = (max, deviation) for element in eachelement(solver, cache)
                    for j in eachnode(solver), i in eachnode(solver)
                        v = variable(
                            get_node_vars(u, equations, solver, i, j, element),
                            equations
                        )
                        # Note: We always save the absolute deviations >= 0 and therefore use the
        ...
        for (key, _) in idp_bounds_delta_local
            ...
            idp_bounds_delta_global[key] = max(
