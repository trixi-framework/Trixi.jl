...
@muladd begin
    ...
    function BoundsCheckCallback(;
            output_directory = "out", save_errors = false,
            interval = 1
        )
    ...
    function (callback::BoundsCheckCallback)(u_ode, integrator, stage)
        ...
        @trixi_timeit timer() "check_bounds" check_bounds(
            u, equations, solver, cache,
        ...
        save_errors = callback.save_errors && (callback.interval > 0) &&
            ...
            (iter + 1) % callback.interval == 0 ||   # Every `interval` time steps
                ...
                (iter + 1) >= integrator.opts.maxiters
        )  # Maximum iterations reached
        if save_errors
            @trixi_timeit timer() "save_errors" save_bounds_check_errors(
    ...
    @inline function save_bounds_check_errors(
            output_directory, t, iter, equations,
            volume_integral::VolumeIntegralSubcellLimiting
        )
        save_bounds_check_errors(
            output_directory, t, iter, equations,
    ...
    function init_callback(
            callback::BoundsCheckCallback, semi,
            volume_integral::VolumeIntegralSubcellLimiting
        )
    ...
    @inline function finalize_callback(
            callback::BoundsCheckCallback, semi,
    ...
    @inline function save_bounds_check_errors(
            output_directory, time, iter, equations,
            limiter::SubcellLimiterIDP
        )
        (; local_twosided, positivity, local_onesided) = limiter
        ...
        open(joinpath(output_directory, "deviations.txt"), "a") do f
            ...
            if local_twosided
                for v in limiter.local_twosided_variables_cons
                    v_string = string(v)
                    print(
                        f, ", ", idp_bounds_delta_local[Symbol(v_string, "_min")],
