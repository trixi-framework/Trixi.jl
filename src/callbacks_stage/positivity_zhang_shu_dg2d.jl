...
function limiter_zhang_shu_positivity!(u, variable,
    ...
    # TODO
    # if variable == density
    #     bound_min = variable_bounds[Symbol("1_min")]
    # else
    if variable == pressure
        if pressure in dg.volume_integral.limiter.positivity_variables_nonlinear
            bound_min = variable_bounds[:pressure_min]
        else
            error("Positivity limiting for variable $variable by LimiterZhangShuLocalBounds
                   requires matching limiter of the IDPLimiter.")
        end
    else
        error("Unknown variable $variable for subcell limiter coefficients")
    end
    ...
    @threaded for element in eachelement(dg, cache)
        ...
        # detect if limiting is necessary
        threshold = minimum(view(bound_min, :, :, element))
        value_min < threshold || continue
