...
function perform_idp_correction!(u, dt,
    ...
    @unpack alpha_interfaces = dg.volume_integral.limiter.cache.subcell_limiter_coefficients
    ...
    @threaded for element in eachelement(dg, cache)
        for j in eachnode(dg), i in eachnode(dg)
            ...
            alpha_flux1 = (1 - alpha_interfaces[1, i, j, element]) *
                          get_node_vars(antidiffusive_flux1_R, equations, dg,
                                        i, j, element)
            alpha_flux1_ip1 = (1 - alpha_interfaces[1, i + 1, j, element]) *
                              get_node_vars(antidiffusive_flux1_L, equations, dg,
                                            i + 1, j, element)
            alpha_flux2 = (1 - alpha_interfaces[2, i, j, element]) *
                          get_node_vars(antidiffusive_flux2_R, equations, dg,
                                        i, j, element)
            alpha_flux2_jp1 = (1 - alpha_interfaces[2, i, j + 1, element]) *
