...
function multiply_dimensionwise!(data_out::AbstractArray{<:Any, 2}, matrix::AbstractMatrix,
  ...
  @avx inline=true for i in axes(data_out, 2), v in axes(data_out, 1)
...
function multiply_scalar_dimensionwise!(data_out::AbstractArray{<:Any, 1},
  ...
  @avx inline=true for i in axes(data_out, 1)
    res = zero(eltype(data_out))
    for ii in axes(matrix, 2)
...
function multiply_scalar_dimensionwise!(data_out::AbstractArray{<:Any, 2},
  ...
  @avx inline=true for j in axes(tmp1, 2), i in axes(tmp1, 1)
    res = zero(eltype(tmp1))
    for ii in axes(matrix, 2)
  ...
  @avx inline=true for j in axes(data_out, 2), i in axes(data_out, 1)
    res = zero(eltype(data_out))
    for jj in axes(matrix, 2)
...
function multiply_dimensionwise!(data_out::AbstractArray{<:Any, 4}, matrix::AbstractMatrix,
  ...
  @avx inline=true for k in axes(tmp1, 4), j in axes(tmp1, 3), i in axes(tmp1, 2), v in axes(tmp1, 1)
    res = zero(eltype(tmp1))
    for ii in axes(matrix, 2)
  ...
  @avx inline=true for k in axes(tmp2, 4), j in axes(tmp2, 3), i in axes(tmp2, 2), v in axes(tmp2, 1)
  ...
  @avx inline=true for k in axes(data_out, 4), j in axes(data_out, 3), i in axes(data_out, 2), v in axes(data_out, 1)
    res = zero(eltype(data_out))
...
function multiply_scalar_dimensionwise!(data_out::AbstractArray{<:Any, 3}, matrix::AbstractMatrix,
  ...
  @avx inline=true for k in axes(tmp1, 3), j in axes(tmp1, 2), i in axes(tmp1, 1)
    res = zero(eltype(tmp1))
    for ii in axes(matrix, 2)
  ...
  @avx inline=true for k in axes(tmp2, 3), j in axes(tmp2, 2), i in axes(tmp2, 1)
    res = zero(eltype(tmp2))
    for jj in axes(matrix, 2)
  ...
  @avx inline=true for k in axes(data_out, 3), j in axes(data_out, 2), i in axes(data_out, 1)
    res = zero(eltype(data_out))
    for kk in axes(matrix, 2)
...
function multiply_dimensionwise!(data_out::AbstractArray{<:Any, 4},
  ...
  @avx inline=true for k in axes(tmp2, 4), j in axes(tmp2, 3), i in axes(tmp2, 2), v in axes(tmp2, 1)
    res = zero(eltype(tmp1))
    for ii in axes(matrix2, 2)
      res += matrix2[i, ii] * data_in[v, ii, j, k]
    end
    tmp2[v, i, j, k] = res
  end
