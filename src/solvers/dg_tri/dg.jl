...
const DGWeakForm = DG{<:RefElemData,Mortar,<:SurfaceIntegralWeakForm{<:FluxPlusDissipation},
...
function Trixi.ndofs(mesh, dg::DG{<:RefElemData}, cache)
    rd = dg.basis
    @unpack md = cache
    return rd.Np * md.num_elements
end
Trixi.wrap_array(u_ode::StructArray, semi::Trixi.AbstractSemidiscretization) = u_ode
Trixi.wrap_array(u_ode::StructArray, mesh, equations, dg::DG{<:RefElemData}, cache) = u_ode

function allocate_coefficients(mesh, equations, dg::DG{<:RefElemData}, cache)
    md = cache.md
...
function compute_coefficients!(u::StructArray, initial_condition, t,
                               mesh, equations, dg::DG{<:RefElemData}, cache)
    md = cache.md
...
function prolong2interfaces!(cache, u, mesh, equations, 
...
function create_cache(mesh, equations, dg::DG, RealT, uEltype) where {DG <: DGWeakForm}
...
function calc_volume_integral!(du,u::StructArray,mesh,
                               nonconservative_terms::Val{false}, equations,
                               volume_integral::VolumeIntegralWeakForm, dg::ModalDG{2}, cache)

    rd = dg.basis  
    @unpack md,invMQrTrW,invMQsTrW,u_values,f_values = cache
    @unpack rxJ,sxJ,ryJ,syJ = cache.md
...
negate_and_invert_jacobian!(du, mesh, equations, dg::DG{<:RefElemData}, cache) = du .*= cache.invJ

calc_sources!(du, u, t, source_terms::Nothing, equations::AbstractEquations{2,NVARS}, 
              dg::DG{<:RefElemData}, cache) where {NVARS} = nothing

# uses quadrature + projection to compute source terms. 
# todo: use interpolation here instead of quadrature?
function calc_sources!(du, u, t, source_terms, equations::AbstractEquations{2,NVARS}, 
                       dg::DG{<:RefElemData}, cache) where {NVARS}
    rd = dg.basis
    @unpack Pq = rd
    @unpack md, u_values, f_values = cache
    for e in Base.OneTo(md.num_elements)
