...
@muladd begin
    ...
    function init_element!(
            elements, element, basis::LobattoLegendreBasis,
            corners_or_surface_curves
        )
        calc_node_coordinates!(
            elements.node_coordinates, element, get_nodes(basis),
            corners_or_surface_curves
        )

        calc_metric_terms!(
            elements.jacobian_matrix, element, get_nodes(basis),
            corners_or_surface_curves
        )
        ...
        calc_contravariant_vectors!(
            elements.contravariant_vectors, element,
            elements.jacobian_matrix
        )

        calc_normal_directions!(
            elements.normal_directions, element, get_nodes(basis),
            corners_or_surface_curves
        )
    ...
    function init_interfaces(
        ...
        if isperiodic(mesh)
            init_interfaces!(
                interfaces, mesh.neighbour_information, mesh.boundary_names,
    ...
    function init_interfaces!(
            interfaces, edge_information, boundary_names, n_elements,
            periodic::False
        )
    ...
    function init_boundaries(
        ...
        if mesh.n_boundaries > 0
            init_boundaries!(
                boundaries, mesh.neighbour_information, mesh.boundary_names,
                elements
            )
        end
        return boundaries
    ...
    function init_boundaries!(
            boundaries::UnstructuredBoundaryContainer2D, edge_information,
        ...
        for j in 1:n_surfaces
            if edge_information[4, j] == 0
                ...
                if primary_side == 1
                    boundaries.node_coordinates[:, :, bndy_count] .= enc[
                        :, :, 1,
                        primary_element,
                    ]
