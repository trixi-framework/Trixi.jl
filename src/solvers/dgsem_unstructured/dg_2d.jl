...
@muladd begin
    ...
    function prolong2interfaces!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            elseif secondary_side == 2
                for i in eachnode(dg), v in eachvariable(equations)
                    interfaces_u[2, v, i, interface] = u[
                        v, nnodes(dg), i,
                        secondary_element,
                    ]
                end
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for primary_index in eachnode(dg)
                ...
                u_ll = get_one_sided_surface_node_vars(
                    u, equations, dg, 1, primary_index,
                    interface
                )
                u_rr = get_one_sided_surface_node_vars(
                    u, equations, dg, 2, secondary_index,
                    interface
                )
                ...
                outward_direction = get_surface_normal(
                    normal_directions, primary_index,
                    primary_side,
                    primary_element
                )
    ...
    @inline function calc_boundary_flux!(
        ...
        # pull the outward pointing (normal) directional vector
        outward_direction = get_surface_normal(
            normal_directions, node_index, side_index,
            element_index
        )
    ...
    @inline function calc_boundary_flux!(
        ...
        noncons_flux = nonconservative_flux(
            u_inner, u_inner, outward_direction,
            outward_direction, equations
        )
    ...
    function max_discrete_metric_identities(dg::DGSEM, cache)
        ...
        for i in 1:ndims_, element in eachelement(dg, cache)
            ...
            @views mul!(
                metric_id_dx, derivative_matrix,
                contravariant_vectors[i, 1, :, :, element]
            )
            @views mul!(
                metric_id_dy, contravariant_vectors[i, 2, :, :, element],
                derivative_matrix'
            )
