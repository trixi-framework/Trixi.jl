...
function prolong2interfaces!(cache, u, mesh::UnstructuredMesh2D, equations, dg::DG)
    ...
    @threaded for interface in eachinterface(dg, cache)
        ...
        if primary_side == 1
            @trixi_bounds for i in eachnode(dg), v in eachvariable(equations)
...
function calc_interface_flux!(surface_flux_values,
    ...
    @threaded for interface in eachinterface(dg, cache)
        ...
        @trixi_bounds for primary_index in eachnode(dg)
...
function prolong2boundaries!(backend::Nothing, cache, u,
    ...
    @threaded for boundary in eachboundary(dg, cache)
        ...
        if side == 1
            @trixi_bounds for l in eachnode(dg), v in eachvariable(equations)
...
function calc_boundary_flux!(backend, cache, t, boundary_condition::BC,
    ...
    @threaded for local_index in eachindex(boundary_indexing)
        ...
        @trixi_bounds for node in eachnode(dg)
