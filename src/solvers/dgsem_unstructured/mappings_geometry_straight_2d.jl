...
@muladd begin
    ...
    function straight_side_quad_map_metrics(xi, eta, corner_points)
        X_xi = 0.25f0 * (
            (1 - eta) * (corner_points[2, 1] - corner_points[1, 1]) +
                (1 + eta) * (corner_points[3, 1] - corner_points[4, 1])
        )

        X_eta = 0.25f0 * (
            (1 - xi) * (corner_points[4, 1] - corner_points[1, 1]) +
                (1 + xi) * (corner_points[3, 1] - corner_points[2, 1])
        )

        Y_xi = 0.25f0 * (
            (1 - eta) * (corner_points[2, 2] - corner_points[1, 2]) +
                (1 + eta) * (corner_points[3, 2] - corner_points[4, 2])
        )

        Y_eta = 0.25f0 * (
            (1 - xi) * (corner_points[4, 2] - corner_points[1, 2]) +
    ...
    function calc_node_coordinates!(
        ...
        for j in eachindex(nodes), i in eachindex(nodes)
            node_coordinates[:, i, j, element] .= straight_side_quad_map(
                nodes[i], nodes[j],
                corners
            )
        end
    ...
    function calc_normal_directions!(normal_directions, element, nodes, corners)
        ...
        for j in eachindex(nodes)
            # side 2
            X_xi, X_eta, Y_xi, Y_eta = straight_side_quad_map_metrics(
                1, nodes[j],
                corners
            )
