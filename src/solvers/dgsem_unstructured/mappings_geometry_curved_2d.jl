...
@muladd begin
    ...
    function transfinite_quad_map(xi, eta, surface_curves::AbstractVector{<:CurvedSurface})
        ...
        x = (
            0.5f0 * ((1 - xi) * x4 + (1 + xi) * x2 + (1 - eta) * x1 + (1 + eta) * x3)
                -
                0.25f0 * (
                (1 - xi) * ((1 - eta) * x_corner1 + (1 + eta) * x_corner4) +
                    (1 + xi) * ((1 - eta) * x_corner2 + (1 + eta) * x_corner3)
            )
        )

        y = (
            0.5f0 * ((1 - xi) * y4 + (1 + xi) * y2 + (1 - eta) * y1 + (1 + eta) * y3)
                -
                0.25f0 * (
                (1 - xi) * ((1 - eta) * y_corner1 + (1 + eta) * y_corner4) +
    ...
    function transfinite_quad_map_metrics(
        ...
        X_xi = (
            0.5f0 * (x2 - x4 + (1 - eta) * x1_prime + (1 + eta) * x3_prime)
                -
                0.25f0 * (
                (1 - eta) * (x_corner2 - x_corner1) +
                    (1 + eta) * (x_corner3 - x_corner4)
            )
        )

        X_eta = (
            0.5f0 * ((1 - xi) * x4_prime + (1 + xi) * x2_prime + x3 - x1)
                -
                0.25f0 * (
                (1 - xi) * (x_corner4 - x_corner1) +
                    (1 + xi) * (x_corner3 - x_corner2)
            )
        )

        Y_xi = (
            0.5f0 * (y2 - y4 + (1 - eta) * y1_prime + (1 + eta) * y3_prime)
                -
                0.25f0 * (
                (1 - eta) * (y_corner2 - y_corner1) +
                    (1 + eta) * (y_corner3 - y_corner4)
            )
        )

        Y_eta = (
            0.5f0 * ((1 - xi) * y4_prime + (1 + xi) * y2_prime + y3 - y1)
                -
                0.25f0 * (
                (1 - xi) * (y_corner4 - y_corner1) +
    ...
    function calc_node_coordinates!(
            node_coordinates::AbstractArray{<:Any, 4}, element,
            nodes,
            surface_curves::AbstractVector{<:CurvedSurface}
        )
        for j in eachindex(nodes), i in eachindex(nodes)
            node_coordinates[:, i, j, element] .= transfinite_quad_map(
                nodes[i], nodes[j],
                surface_curves
            )
        end
    ...
    function calc_normal_directions!(
            normal_directions, element, nodes,
            surface_curves::AbstractVector{<:CurvedSurface}
        )
        ...
        for j in eachindex(nodes)
            # side 2
            X_xi, X_eta, Y_xi, Y_eta = transfinite_quad_map_metrics(
                1, nodes[j],
                surface_curves
            )
