...
@muladd begin
    ...
    mutable struct P4estMPIMortarContainer{
            NDIMS, uEltype <: Real, RealT <: Real, NDIMSP1,
            NDIMSP2, NDIMSP3,
        } <: AbstractContainer
    ...
    function Base.resize!(mpi_mortars::P4estMPIMortarContainer, capacity)
        ...
        mpi_mortars.normal_directions = unsafe_wrap(
            Array, pointer(_normal_directions),
            (
                n_dims,
    ...
    function init_mpi_mortars(
            mesh::Union{ParallelP4estMesh, ParallelT8codeMesh}, equations,
            basis, elements
        )
        NDIMS = ndims(mesh)
        RealT = real(mesh)
        ...
        mpi_mortars = P4estMPIMortarContainer{
            NDIMS, uEltype, RealT, NDIMS + 1, NDIMS + 2,
            NDIMS + 3,
        }(
            u, local_neighbor_ids,
    ...
    function reinitialize_containers!(mesh::ParallelP4estMesh, equations, dg::DGSEM, cache)
        ...
        init_mpi_cache!(
            mpi_cache, mesh, mpi_interfaces, mpi_mortars,
    ...
    function ParallelInitSurfacesIterFaceUserData(
        ...
        return ParallelInitSurfacesIterFaceUserData{
            ...
            typeof(mpi_mortars), typeof(mesh),
        }(
            interfaces,
            1,
            mortars,
    ...
    function cfunction(::typeof(init_surfaces_iter_face_parallel), ::Val{2})
        @cfunction(
            init_surfaces_iter_face_parallel, Cvoid,
    ...
    function init_mpi_interfaces_iter_face_inner(info_pw, sides_pw, user_data)
        ...
        init_mpi_interface_node_indices!(
            mpi_interfaces, faces, local_side,
            info_pw.orientation[],
            mpi_interface_id
        )
    ...
    function init_mpi_mortars_iter_face_inner(info_pw, sides_pw, user_data)
        ...
        local_small_quad_positions = findall(
        ...
        tree_small_quad_ids = map(
            p -> sides_pw[hanging_side].is.hanging.quadid[][p],
    ...
    function cfunction(::typeof(count_surfaces_iter_face_parallel), ::Val{2})
        @cfunction(
            count_surfaces_iter_face_parallel, Cvoid,
            (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid})
        )
    end
    # 3D
