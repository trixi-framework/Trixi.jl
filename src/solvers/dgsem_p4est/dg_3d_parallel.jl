...
@muladd begin
    ...
    function prolong2mpiinterfaces!(
        ...
        @threaded for interface in eachmpiinterface(dg, cache)
            ...
            i_element_start, i_element_step_i, i_element_step_j = index_to_start_step_3d(
                local_indices[1],
                index_range
            )
            j_element_start, j_element_step_i, j_element_step_j = index_to_start_step_3d(
                local_indices[2],
                index_range
            )
            k_element_start, k_element_step_i, k_element_step_j = index_to_start_step_3d(
                local_indices[3],
                index_range
            )
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    for v in eachvariable(equations)
                        mpi_interfaces.u[local_side, v, i, j, interface] = u[
                            v, i_element,
                            j_element,
                            k_element,
                            local_element,
    ...
    function calc_mpi_interface_flux!(
        ...
        @threaded for interface in eachmpiinterface(dg, cache)
            ...
            i_surface_start, i_surface_step_i, i_surface_step_j = index_to_start_step_3d(
                local_surface_indices[1],
                index_range
            )
            j_surface_start, j_surface_step_i, j_surface_step_j = index_to_start_step_3d(
                local_surface_indices[2],
                index_range
            )
    ...
    @inline function calc_mpi_interface_flux!(
        ...
        for v in eachvariable(equations)
            surface_flux_values[
                v, surface_i_node_index, surface_j_node_index,
                local_direction_index, local_element_index,
            ] = flux_[v]
        end
    end
            ...
            for (element, position) in zip(local_neighbor_ids, local_neighbor_positions)
                if position == 5 # -> large element
                    ...
                    for j in eachnode(dg)
                        for i in eachnode(dg)
                            for v in eachvariable(equations)
                                cache.mpi_mortars.u[1, v, position, i, j, mortar] = u[
                                    v,
                                    i_small,
                                    j_small,
                                    k_small,
        ...
        for (element, position) in zip(
            ...
            if position == 5 # -> large element
                ...
                for j in eachnode(dg)
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            surface_flux_values[v, i_large, j_large, large_direction, element] = u_buffer[
                                v,
                                i,
                                j,
                            ]
