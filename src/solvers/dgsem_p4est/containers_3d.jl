...
@muladd begin
    ...
    function init_elements!(
            elements, mesh::Union{P4estMesh{3}, T8codeMesh{3}},
        ...
        for element in 1:ncells(mesh)
            ...
            calc_contravariant_vectors!(
                contravariant_vectors, element, jacobian_matrix,
    ...
    function calc_node_coordinates!(
        ...
        for tree in eachindex(trees)
            ...
            for i in eachindex(quadrants)
                ...
                nodes_out_x = 2 * (
                    quad_length * 1 / 2 * (nodes .+ 1) .+
                        quad.x / p4est_root_len
                ) .- 1
                nodes_out_y = 2 * (
                    quad_length * 1 / 2 * (nodes .+ 1) .+
                        quad.y / p4est_root_len
                ) .- 1
                nodes_out_z = 2 * (
                    quad_length * 1 / 2 * (nodes .+ 1) .+
                        quad.z / p4est_root_len
                ) .- 1
    ...
    @inline function init_interface_node_indices!(
        ...
        for side in 1:2
            ...
            if side == 1
                ...
                surface_index1, surface_index2 = orientation_to_indices_p4est(
                    faces[2],
                    faces[1],
                    orientation
                )
    ...
    function orientation_to_indices_p4est(my_face, other_face, orientation_code)
        ...
        if !flipped
            if orientation_code == 0
                ...
                surface_index2 = :j_forward
            elseif (
                    (lower && orientation_code == 2) # Corner 0 of my side matches corner 2 of other side
                        ||
                        (!lower && orientation_code == 1)
            ...
            elseif (
                    (lower && orientation_code == 1) # Corner 0 of my side matches corner 1 of other side
                        ||
                        (!lower && orientation_code == 2)
