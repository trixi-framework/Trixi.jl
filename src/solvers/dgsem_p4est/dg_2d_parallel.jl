...
@muladd begin
    ...
    function prolong2mpiinterfaces!(
        ...
        @threaded for interface in eachmpiinterface(dg, cache)
            ...
            i_element_start, i_element_step = index_to_start_step_2d(
                local_indices[1],
                index_range
            )
            j_element_start, j_element_step = index_to_start_step_2d(
                local_indices[2],
                index_range
            )
            ...
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    mpi_interfaces.u[local_side, v, i, interface] = u[
                        v, i_element,
                        j_element,
                        local_element,
                    ]
    ...
    @inline function calc_mpi_interface_flux!(
        ...
        u_ll, u_rr = get_surface_node_vars(
            u, equations, dg, interface_node_index,
            interface_index
        )
        ...
        @threaded for mortar in eachmpimortar(dg, cache)
            ...
            for (element, position) in zip(local_neighbor_ids, local_neighbor_positions)
                if position == 3 # -> large element
                    ...
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mpi_mortars.u[1, v, position, i, mortar] = u[
                                v, i_small,
                                j_small,
                                element,
                            ]
        ...
        for (element, position) in zip(
            ...
            if position == 3 # -> large element
                ...
                if :i_backward in large_indices
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            surface_flux_values[v, end + 1 - i, large_direction, element] = u_buffer[
                                v,
                                i,
                            ]
                        end
                    ...
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            surface_flux_values[v, i, large_direction, element] = u_buffer[
                                v,
                                i,
                            ]
                        end
