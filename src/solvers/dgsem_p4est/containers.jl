...
@muladd begin
    ...
    mutable struct P4estElementContainer{
            NDIMS, RealT <: Real, uEltype <: Real, NDIMSP1,
            NDIMSP2, NDIMSP3,
        } <: AbstractContainer
    ...
    function Base.resize!(elements::P4estElementContainer, capacity)
        ...
        elements.jacobian_matrix = unsafe_wrap(
            Array, pointer(_jacobian_matrix),
            (
                n_dims, n_dims,
    ...
    function init_elements(
            mesh::Union{P4estMesh{NDIMS, RealT}, T8codeMesh{NDIMS, RealT}},
            equations,
            basis,
            ::Type{uEltype}
        ...
        contravariant_vectors = unsafe_wrap(
            Array, pointer(_contravariant_vectors),
        ...
        elements = P4estElementContainer{
            NDIMS, RealT, uEltype, NDIMS + 1, NDIMS + 2,
            NDIMS + 3,
        }(
    ...
    function cfunction(::typeof(count_surfaces_iter_face), ::Val{2})
        @cfunction(
            count_surfaces_iter_face, Cvoid,
            (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid})
        )
    end
    # 3D
