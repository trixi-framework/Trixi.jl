...
mutable struct P4estElementContainer{NDIMS,RealT<:Real,uEltype<:Real,NDIMSP1,NDIMSP2,
...
@inline function Base.eltype(::P4estElementContainer{NDIMS,RealT,uEltype}) where {NDIMS,
                                                                                  RealT,
                                                                                  uEltype}
  uEltype
end
...
function Base.resize!(elements::P4estElementContainer, capacity)
  ...
  elements.jacobian_matrix = unsafe_wrap(Array, pointer(_jacobian_matrix),
                                         (n_dims, n_dims, ntuple(_ -> n_nodes, n_dims)...,
                                          capacity))
...
function init_elements(mesh::P4estMesh{NDIMS,RealT}, equations,
  ...
  surface_flux_values = unsafe_wrap(Array, pointer(_surface_flux_values),
                                    (nvariables(equations),
                                     ntuple(_ -> nnodes(basis), NDIMS - 1)..., NDIMS * 2,
                                     nelements))

  elements = P4estElementContainer{NDIMS,RealT,uEltype,NDIMS + 1,NDIMS + 2,NDIMS + 3}(node_coordinates,
                                                                                      jacobian_matrix,
...
function init_mortars(mesh::P4estMesh, equations, basis, elements)
  ...
  mortars = P4estMortarContainer{NDIMS,uEltype,NDIMS + 1,NDIMS + 3}(u, neighbor_ids,
                                                                    node_indices,
                                                                    _u, _neighbor_ids,
                                                                    _node_indices)
...
function cfunction(::typeof(init_surfaces_iter_face), ::Val{2})
  @cfunction(init_surfaces_iter_face, Cvoid, (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid}))
end
# 3D
function cfunction(::typeof(init_surfaces_iter_face), ::Val{3})
  @cfunction(init_surfaces_iter_face, Cvoid, (Ptr{p8est_iter_face_info_t}, Ptr{Cvoid}))
end
...
function cfunction(::typeof(count_surfaces_iter_face), ::Val{2})
  @cfunction(count_surfaces_iter_face, Cvoid, (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid}))
end
# 3D
function cfunction(::typeof(count_surfaces_iter_face), ::Val{3})
  @cfunction(count_surfaces_iter_face, Cvoid, (Ptr{p8est_iter_face_info_t}, Ptr{Cvoid}))
end
