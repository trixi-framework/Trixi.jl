...
@muladd begin
    ...
    function start_mpi_send!(mpi_cache::P4estMPICache, mesh, equations, dg, cache)
        ...
        for rank in 1:length(mpi_cache.mpi_neighbor_ranks)
            ...
            for (index, interface) in enumerate(mpi_cache.mpi_neighbor_interfaces[rank])
                ...
                @views send_buffer[first:last] .= vec(
                    cache.mpi_interfaces.u[
                        local_side, ..,
                        interface,
                    ]
                )
        ...
        for (index, rank) in enumerate(mpi_cache.mpi_neighbor_ranks)
            mpi_cache.mpi_send_requests[index] = MPI.Isend(
                mpi_cache.mpi_send_buffers[index],
    ...
    function start_mpi_receive!(mpi_cache::P4estMPICache)
        for (index, rank) in enumerate(mpi_cache.mpi_neighbor_ranks)
            mpi_cache.mpi_recv_requests[index] = MPI.Irecv!(
                mpi_cache.mpi_recv_buffers[index],
                rank, rank, mpi_comm()
            )
        end
    ...
    function create_cache(
            mesh::ParallelP4estMesh, equations::AbstractEquations, dg::DG,
            ::Any, ::Type{uEltype}
        ) where {uEltype <: Real}
        ...
        mpi_cache = init_mpi_cache(
            mesh, mpi_interfaces, mpi_mortars,
        ...
        cache = (;
            elements, interfaces, mpi_interfaces, boundaries, mortars, mpi_mortars,
            mpi_cache,
        )
    ...
    function init_mpi_cache(
            mesh::ParallelP4estMesh, mpi_interfaces, mpi_mortars, nvars,
            nnodes, uEltype
        )
        mpi_cache = P4estMPICache(uEltype)
        init_mpi_cache!(
            mpi_cache, mesh, mpi_interfaces, mpi_mortars, nvars, nnodes,
            uEltype
        )
    ...
    function init_mpi_cache!(
            mpi_cache::P4estMPICache, mesh::ParallelP4estMesh,
            mpi_interfaces, mpi_mortars, nvars, n_nodes, uEltype
        )
        mpi_neighbor_ranks, mpi_neighbor_interfaces, mpi_neighbor_mortars = init_mpi_neighbor_connectivity(
            mpi_interfaces,
            mpi_mortars,
            mesh
        )

        mpi_send_buffers, mpi_recv_buffers, mpi_send_requests, mpi_recv_requests = init_mpi_data_structures(
        ...
        n_elements_by_rank = vcat(
            Int.(
                unsafe_wrap(
                    Array, mesh.p4est.global_first_quadrant,
                    mpi_nranks()
                )
            ),
            n_elements_global
    ...
    function init_mpi_neighbor_connectivity(
        ...
        user_data = InitNeighborRankConnectivityIterFaceUserData(
            mpi_interfaces,
            mpi_mortars, mesh
        )

        iterate_p4est(
            mesh.p4est, user_data; ghost_layer = mesh.ghost,
        ...
        for (index, rank) in enumerate(mpi_neighbor_ranks)
            mpi_neighbor_interfaces[index] = interface_ids[
                findall(
                    ==(rank),
                    neighbor_ranks_interface
                ),
            ]
            mpi_neighbor_mortars[index] = mortar_ids[
                findall(
                    x -> (rank in x),
    ...
    function cfunction(::typeof(init_neighbor_rank_connectivity_iter_face), ::Val{2})
        @cfunction(
            init_neighbor_rank_connectivity_iter_face, Cvoid,
    ...
    function init_neighbor_rank_connectivity_iter_face_inner(info, user_data)
        ...
        if info_pw.sides.elem_count[] == 2 # MPI interfaces/mortars have two neighboring elements
            ...
            sides_pw = (
                load_pointerwrapper_side(info_pw, 1),
            ...
            if sides_pw[1].is_hanging[] == false && sides_pw[2].is_hanging[] == false # No hanging nodes for MPI interfaces
                ...
                local_tree_pw = load_pointerwrapper_tree(
                    mesh.p4est,
                    sides_pw[local_side].treeid[] + 1
                ) # one-based indexing
                ...
                neighbor_rank = findfirst(
                    r -> proc_offsets[r] <= ghost_id <
                        proc_offsets[r + 1],
                    1:mpi_nranks()
                ...
                trees_pw = (
                    load_pointerwrapper_tree(mesh.p4est, sides_pw[1].treeid[] + 1),
                ...
                remote_small_quad_positions = findall(
                    sides_pw[hanging_side].is.hanging.is_ghost[] .==
                        true
                )
                proc_offsets = unsafe_wrap(
                    Array,
                ...
                ghost_ids = map(
                    pos -> sides_pw[hanging_side].is.hanging.quadid[][pos],
                ...
                if sides_pw[full_side].is.full.is_ghost[] == true
                    ...
                    large_quad_owner_rank = findfirst(
                        r -> proc_offsets[r] <= ghost_id <
                            proc_offsets[r + 1],
                        1:mpi_nranks()
                    ) - 1 # MPI ranks are 0-based
    ...
    function exchange_normal_directions!(
        ...
        # Start data exchange
        for (index, rank) in enumerate(mpi_neighbor_ranks)
            send_requests[index] = MPI.Isend(
                send_buffers[index], rank, mpi_rank(),
                mpi_comm()
            )
