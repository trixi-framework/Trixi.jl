...
# size `2 * data_size`.
@inline function buffer_mortar_indices(mesh::Union{ParallelP4estMesh{2},ParallelT8codeMesh{2}}, index_base,
                                       data_size)
    return (
...
# size `4 * data_size`.
@inline function buffer_mortar_indices(mesh::Union{ParallelP4estMesh{3},ParallelT8codeMesh{3}}, index_base,
                                       data_size)
    return (
            # first, last for local element in position 1 (small element)
            (index_base + 1,
...
function exchange_normal_directions!(mpi_mortars, mpi_cache, mesh::Union{ParallelP4estMesh,ParallelT8codeMesh},
                                     n_nodes)
    RealT = real(mesh)
    n_dims = ndims(mesh)
