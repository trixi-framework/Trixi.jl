...
@muladd begin
    ...
    function calc_gradient!(
        ...
        @trixi_timeit timer() "volume integral" begin
            ...
            @threaded for element in eachelement(dg, cache)
                ...
                for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                    Ja11, Ja12, Ja13 = get_contravariant_vector(
                        1, contravariant_vectors,
                        i, j, k, element
                    )
                    Ja21, Ja22, Ja23 = get_contravariant_vector(
                        2, contravariant_vectors,
                        i, j, k, element
                    )
                    Ja31, Ja32, Ja33 = get_contravariant_vector(
                        3, contravariant_vectors,
                        i, j, k, element
                    )

                    gradients_reference_1 = get_node_vars(
                        gradients_x, equations_parabolic,
                        dg,
                        i, j, k, element
                    )
                    gradients_reference_2 = get_node_vars(
                        gradients_y, equations_parabolic,
                        dg,
                        i, j, k, element
                    )
                    gradients_reference_3 = get_node_vars(
                        gradients_z, equations_parabolic,
                        dg,
                        i, j, k, element
                    )
                    ...
                    set_node_vars!(
                        gradients_x, gradient_x_node, equations_parabolic, dg,
                        i, j, k, element
                    )
                    set_node_vars!(
                        gradients_y, gradient_y_node, equations_parabolic, dg,
                        i, j, k, element
                    )
                    set_node_vars!(
                        gradients_z, gradient_z_node, equations_parabolic, dg,
                        i, j, k, element
                    )
                end
            end
        ...
        @trixi_timeit timer() "prolong2interfaces" begin
            prolong2interfaces!(
                cache_parabolic, u_transformed, mesh,
    ...
    @inline function mortar_fluxes_to_elements!(
        ...
        for position in 1:4 # Loop over small elements
            ...
            for j in eachnode(dg), i in eachnode(dg)
                for v in eachvariable(equations)
                    surface_flux_values[v, i, j, small_direction, element] = fstar[
                        v, i, j,
                        position,
                    ]
                end
        ...
        i_large_start, i_large_step_i, i_large_step_j = index_to_start_step_3d(
            large_surface_indices[1],
            index_range
        )
        j_large_start, j_large_step_i, j_large_step_j = index_to_start_step_3d(
            large_surface_indices[2],
            index_range
        )
        ...
        for j in eachnode(dg)
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    surface_flux_values[v, i_large, j_large, large_direction, large_element] = u_buffer[
                        v,
                        i,
                        j,
                    ]
    ...
    @inline function calc_interface_flux!(
            surface_flux_values, mesh::P4estMesh{3},
        ...
        u_ll, u_rr = get_surface_node_vars(
            u, equations, dg, primary_i_node_index,
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                flux1 = get_node_vars(
                    flux_viscous_x, equations_parabolic, dg, i, j, k,
                    element
                )
                flux2 = get_node_vars(
                    flux_viscous_y, equations_parabolic, dg, i, j, k,
                    element
                )
                flux3 = get_node_vars(
                    flux_viscous_z, equations_parabolic, dg, i, j, k,
                    element
                )
    ...
    function prolong2interfaces!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            i_primary_start, i_primary_step_i, i_primary_step_j = index_to_start_step_3d(
                primary_indices[1],
                index_range
            )
            j_primary_start, j_primary_step_i, j_primary_step_j = index_to_start_step_3d(
                primary_indices[2],
                index_range
            )
            k_primary_start, k_primary_step_i, k_primary_step_j = index_to_start_step_3d(
                primary_indices[3],
                index_range
            )
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    ...
                    for v in eachvariable(equations_parabolic)
                        ...
                        interfaces.u[1, v, i, j, interface] = dot(
                            flux_viscous,
                            normal_direction
                        )
                    end
            ...
            j_secondary_start, j_secondary_step_i, j_secondary_step_j = index_to_start_step_3d(
                secondary_indices[2],
                index_range
            )
            k_secondary_start, k_secondary_step_i, k_secondary_step_j = index_to_start_step_3d(
                secondary_indices[3],
                index_range
            )
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    ...
                    for v in eachvariable(equations_parabolic)
                        ...
                        interfaces.u[2, v, i, j, interface] = -dot(
                            flux_viscous,
                            normal_direction
                        )
                    end
    ...
    function calc_interface_flux!(
        ...
        @threaded for interface in eachinterface(dg, cache_parabolic)
            ...
            i_secondary_start, i_secondary_step_i, i_secondary_step_j = index_to_start_step_3d(
                secondary_surface_indices[1],
                index_range
            )
            j_secondary_start, j_secondary_step_i, j_secondary_step_j = index_to_start_step_3d(
                secondary_surface_indices[2],
                index_range
            )
            i_secondary = i_secondary_start
            j_secondary = j_secondary_start
    ...
    function prolong2mortars_divergence!(
            cache, flux_viscous,
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            i_small_start, i_small_step_i, i_small_step_j = index_to_start_step_3d(
                small_indices[1],
                index_range
            )
            j_small_start, j_small_step_i, j_small_step_j = index_to_start_step_3d(
                small_indices[2],
                index_range
            )
                ...
                for j in eachnode(dg)
                    for i in eachnode(dg)
                        ...
                        for v in eachvariable(equations)
                            ...
                            cache.mortars.u[1, v, position, i, j, mortar] = dot(
                                flux_viscous,
                                normal_direction
                            )
                        end
            ...
            k_large_start, k_large_step_i, k_large_step_j = index_to_start_step_3d(
                large_indices[3],
                index_range
            )
    ...
    # the parabolic terms.
    function calc_mortar_flux_divergence!(
            surface_flux_values,
            mesh::Union{P4estMesh{3}, T8codeMesh{3}},
            equations::AbstractEquationsParabolic,
            mortar_l2::LobattoLegendreMortarL2,
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            for position in 1:4 # Loop over small elements
                ...
                for j in eachnode(dg)
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            viscous_flux_normal_ll = cache.mortars.u[
                                1, v, position, i, j,
                                mortar,
                            ]
                            viscous_flux_normal_rr = cache.mortars.u[
                                2, v, position, i, j,
                                mortar,
                            ]

                            # TODO: parabolic; only BR1 at the moment
                            fstar[v, i, j, position] = 0.5f0 * (
    ...
    @inline function calc_mortar_flux!(
        ...
        u_ll, u_rr = get_surface_node_vars(
            u, equations, dg, position_index, i_node_index,
        ...
        # Copy flux to buffer
        set_node_vars!(
            fstar, flux_, equations, dg, i_node_index, j_node_index,
            position_index
        )
    end
    ...
    function prolong2boundaries!(
        ...
        @threaded for boundary in eachboundary(dg, cache_parabolic)
            ...
            i_node_start, i_node_step_i, i_node_step_j = index_to_start_step_3d(
                node_indices[1],
                index_range
            )
            j_node_start, j_node_step_i, j_node_step_j = index_to_start_step_3d(
                node_indices[2],
                index_range
            )
            k_node_start, k_node_step_i, k_node_step_j = index_to_start_step_3d(
                node_indices[3],
                index_range
            )
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    ...
                    for v in eachvariable(equations_parabolic)
                        ...
                        boundaries.u[v, i, j, boundary] = dot(
                            flux_viscous,
                            normal_direction
                        )
                    end
        ...
        @threaded for local_index in eachindex(boundary_condition_indices)
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    ...
                    u_inner = get_node_vars(
                        boundaries.u, equations_parabolic, dg, i, j,
                        boundary_index
                    )
                    ...
                    x = get_node_coords(
                        node_coordinates, equations_parabolic, dg, i_node,
                        j_node, k_node,
                        element
                    )

                    flux_ = boundary_condition_parabolic(
                        flux_inner, u_inner,
                        normal_direction,
                        x, t, operator_type,
                        equations_parabolic
