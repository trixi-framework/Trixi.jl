...
@muladd begin
    ...
    function prolong2interfaces!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    for v in eachvariable(equations)
                        interfaces.u[1, v, i, j, interface] = u[
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    for v in eachvariable(equations)
                        interfaces.u[2, v, i, j, interface] = u[
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for j in eachnode(dg)
                for i in eachnode(dg)
                    ...
                    calc_interface_flux!(
                        surface_flux_values, mesh, nonconservative_terms,
                        equations,
    ...
    @inline function calc_interface_flux!(
        ...
        for v in eachvariable(equations)
            surface_flux_values[
                v, primary_i_node_index, primary_j_node_index,
                primary_direction_index, primary_element_index,
            ] = flux_[v]
            surface_flux_values[
                v, secondary_i_node_index, secondary_j_node_index,
    ...
    function calc_boundary_flux!(
            cache, t, boundary_condition, boundary_indexing,
            mesh::Union{P4estMesh{3}, T8codeMesh{3}},
            equations, surface_integral, dg::DG
        )
        @unpack boundaries = cache
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            for position in 1:4
                ...
                for j in eachnode(dg)
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u[1, v, position, i, j, mortar] = u[
                                v, i_small,
                                j_small,
                                k_small,
                                element,
            ...
            for position in 1:4
                ...
                for j in eachnode(dg)
                    for i in eachnode(dg)
                        ...
                        calc_mortar_flux!(
                            fstar, mesh, nonconservative_terms, equations,
                            surface_integral, dg, cache,
    ...
    @inline function calc_mortar_flux!(
        ...
        # Copy flux to buffer
        set_node_vars!(
            fstar, flux, equations, dg, i_node_index, j_node_index,
            position_index
        )
    end
    ...
    @inline function calc_mortar_flux!(
        ...
        # the interpretation of global SBP operators coupled discontinuously via
        # central fluxes/SATs
        noncons = nonconservative_flux(
            u_ll, u_rr, normal_direction, normal_direction,
            equations
        )
        ...
        # Copy to buffer
        set_node_vars!(
            fstar, flux_plus_noncons, equations, dg, i_node_index, j_node_index,
            position_index
        )
    end
