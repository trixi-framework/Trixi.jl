...
@muladd begin
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    interfaces.u[1, v, i, interface] = u[
            ...
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    interfaces.u[2, v, i, interface] = u[
    ...
    function calc_boundary_flux!(
            cache, t, boundary_condition::BC, boundary_indexing,
            mesh::Union{P4estMesh{2}, T8codeMesh{2}},
            equations, surface_integral, dg::DG
        ) where {BC}
        @unpack boundaries = cache
    ...
    @inline function calc_boundary_flux!(
        ...
        normal_direction = get_normal_direction(
            direction_index, contravariant_vectors,
            i_index, j_index, element_index
        )

        # Coordinates at boundary node
        x = get_node_coords(
            node_coordinates, equations, dg, i_index, j_index,
            element_index
        )
    ...
    @inline function calc_boundary_flux!(
        ...
        noncons_ = nonconservative_flux(
            u_inner, u_inner, normal_direction,
            normal_direction, equations
        )
    ...
    function prolong2mortars!(
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            for position in 1:2
                ...
                for i in eachnode(dg)
                    for v in eachvariable(equations)
                        cache.mortars.u[1, v, position, i, mortar] = u[
                            v, i_small, j_small,
                            element,
                        ]
                    end
