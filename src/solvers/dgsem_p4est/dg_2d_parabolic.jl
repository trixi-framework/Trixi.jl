...
@muladd begin
    ...
    function create_cache_parabolic(
        ...
        viscous_container = init_viscous_container_2d(
...
=#
    function rhs_parabolic!(
            du, u, t, mesh::Union{P4estMesh{2}, P4estMesh{3}},
            equations_parabolic::AbstractEquationsParabolic,
        ...
        @trixi_timeit timer() "calculate viscous fluxes" begin
            calc_viscous_fluxes!(
                flux_viscous, gradients, u_transformed, mesh,
        ...
        @trixi_timeit timer() "prolong2interfaces" begin
            prolong2interfaces!(
                cache_parabolic, flux_viscous, mesh, equations_parabolic,
                dg.surface_integral, dg, cache
            )
        end
        ...
        @trixi_timeit timer() "prolong2mortars" begin
            prolong2mortars_divergence!(
                cache, flux_viscous, mesh, equations_parabolic,
                dg.mortar, dg.surface_integral, dg
            )
        end
            ...
            @threaded for element in eachelement(dg, cache)
                ...
                for j in eachnode(dg), i in eachnode(dg)
                    u_node = get_node_vars(
                        u_transformed, equations_parabolic, dg, i, j,
                        element
                    )
                ...
                for j in eachnode(dg), i in eachnode(dg)
                    Ja11, Ja12 = get_contravariant_vector(
                        1, contravariant_vectors, i, j,
                        element
                    )
                    Ja21, Ja22 = get_contravariant_vector(
                    ...
                    set_node_vars!(
                        gradients_x, gradient_x_node, equations_parabolic, dg, i,
                        j,
                        element
                    )
                    set_node_vars!(
                        gradients_y, gradient_y_node, equations_parabolic, dg, i,
                        j,
                        element
                    )
                end
    ...
    @inline function mortar_fluxes_to_elements!(
        ...
        for position in 1:2
            ...
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    surface_flux_values[v, i, small_direction, element] = fstar[position][
                        v,
                        i,
                    ]
                end
        ...
        if :i_backward in large_indices
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    surface_flux_values[v, end + 1 - i, large_direction, large_element] = u_buffer[
                        v,
                        i,
                    ]
                end
            ...
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    surface_flux_values[v, i, large_direction, large_element] = u_buffer[
                        v,
                        i,
                    ]
                end
    ...
    @inline function calc_interface_flux!(
            surface_flux_values, mesh::P4estMesh{2},
        ...
        u_ll, u_rr = get_surface_node_vars(
            u, equations, dg, primary_node_index,
            interface_index
        )
    ...
    function prolong2interfaces!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            i_primary_start, i_primary_step = index_to_start_step_2d(
                primary_indices[1],
                index_range
            )
            j_primary_start, j_primary_step = index_to_start_step_2d(
                primary_indices[2],
                index_range
            )
            ...
            i_secondary_start, i_secondary_step = index_to_start_step_2d(
                secondary_indices[1],
                index_range
            )
            j_secondary_start, j_secondary_step = index_to_start_step_2d(
                secondary_indices[2],
                index_range
            )
    ...
    function prolong2mortars_divergence!(
            cache, flux_viscous::Vector{Array{uEltype, 4}},
            mesh::Union{P4estMesh{2}, T8codeMesh{2}},
            equations,
            mortar_l2::LobattoLegendreMortarL2,
            surface_integral,
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            i_small_start, i_small_step = index_to_start_step_2d(
                small_indices[1],
                index_range
            )
            j_small_start, j_small_step = index_to_start_step_2d(
                small_indices[2],
                index_range
            )
            ...
            for position in 1:2
                ...
                for i in eachnode(dg)
                    ...
                    for v in eachvariable(equations)
                        flux_viscous = SVector(
                            flux_viscous_x[v, i_small, j_small, element],
                            flux_viscous_y[v, i_small, j_small, element]
                        )

                        cache.mortars.u[1, v, position, i, mortar] = dot(
                            flux_viscous,
                            normal_direction
                        )
                    end
            ...
            i_large_start, i_large_step = index_to_start_step_2d(
                large_indices[1],
                index_range
            )
            j_large_start, j_large_step = index_to_start_step_2d(
                large_indices[2],
                index_range
            )
            ...
            for i in eachnode(dg)
                ...
                for v in eachvariable(equations)
                    flux_viscous = SVector(
                        flux_viscous_x[v, i_large, j_large, element],
    ...
    @inline function calc_mortar_flux!(
        ...
        u_ll, u_rr = get_surface_node_vars(
            u, equations, dg, position_index, node_index,
            mortar_index
        )
    ...
    function prolong2boundaries!(
        ...
        @threaded for boundary in eachboundary(dg, cache_parabolic)
            ...
            for i in eachnode(dg)
                # this is the outward normal direction on the primary element
                normal_direction = get_normal_direction(
                    direction, contravariant_vectors,
                    i_node, j_node, element
                )

                for v in eachvariable(equations_parabolic)
                    flux_viscous = SVector(
                        flux_viscous_x[v, i_node, j_node, element],
    ...
    function calc_boundary_flux_by_type!(
            cache, t, BCs::Tuple{}, BC_indices::Tuple{},
        ...
        @threaded for local_index in eachindex(boundary_condition_indices)
            ...
            for node_index in eachnode(dg)
                ...
                u_inner = get_node_vars(
                    boundaries.u, equations_parabolic, dg, node_index,
                    boundary_index
                )
                ...
                flux_ = boundary_condition_parabolic(
                    flux_inner, u_inner, normal_direction,
                    x, t, operator_type,
                    equations_parabolic
                )
