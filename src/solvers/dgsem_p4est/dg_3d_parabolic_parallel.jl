...
@inline function calc_interface_flux_gradient!(surface_flux_values,
                                               mesh::Union{P4estMeshParallel{3},
                                                           T8codeMeshParallel{3}},
                                               equations_parabolic,
                                               dg::DG, parabolic_scheme, cache,
                        ...
                        for v in eachvariable(equations_parabolic)
                            flux_node = SVector(flux_viscous_x[v, i_large, j_large,
                                                ...
                                                flux_viscous_y[v, i_large, j_large,
                                                               k_large, element],
                                                flux_viscous_z[v, i_large, j_large,
                                                               k_large, element])
...
@inline function calc_mpi_mortar_flux_divergence!(fstar,
    ...
    for v in eachvariable(equations_parabolic)
        viscous_flux_normal_ll = u[1, v, position_index, i_node_index, j_node_index,
                                   mortar_index]
        viscous_flux_normal_rr = u[2, v, position_index, i_node_index, j_node_index,
                                   mortar_index]
...
function calc_volume_integral_gradient!(gradients, u_transformed,
                                        mesh::Union{P4estMeshParallel{3},
                                                    T8codeMeshParallel{3}}, # for dispatch only
                                        equations_parabolic::AbstractEquationsParabolic,
                                        dg::DG, cache)
    ...
    @threaded for interface in eachmpiinterface(dg, cache)
        ...
        for j in eachnode(dg)
            for i in eachnode(dg)
                ...
                for v in eachvariable(equations_parabolic)
                    flux_viscous = SVector(flux_viscous_x[v, i_elem, j_elem, k_elem,
                                                          local_element],
                                           flux_viscous_y[v, i_elem, j_elem, k_elem,
                                                          local_element],
                                           flux_viscous_z[v, i_elem, j_elem, k_elem,
                                                          local_element])
