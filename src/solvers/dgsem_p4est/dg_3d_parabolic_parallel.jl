...
function rhs_parabolic!(du, u, t, mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}},
    ...
    cache_p = (; cache...,
                elements   = cache_parabolic.elements,
    ...
    @trixi_timeit timer() "calculate viscous fluxes" begin
        calc_viscous_fluxes!(flux_viscous, gradients, u_transformed, mesh,
                             equations_parabolic, dg, cache_p)
    end
    ...
    @trixi_timeit timer() "reset ∂u/∂t" begin
        set_zero!(du, dg, cache_p)
    end

    # Local volume integral
    @trixi_timeit timer() "volume integral" begin
        calc_volume_integral!(du, flux_viscous, mesh, equations_parabolic, dg, cache_p)
    end

    # Prolong viscous fluxes to MPI mortars
    @trixi_timeit timer() "prolong2mpimortars divergence" begin
        prolong2mpimortars_divergence!(cache_p, flux_viscous, mesh, equations_parabolic,
                                       dg.mortar, dg)
    end
    ...
    @trixi_timeit timer() "prolong2interfaces" begin
        prolong2interfaces!(cache_p, flux_viscous, mesh, equations_parabolic, dg)
    end

    @trixi_timeit timer() "interface flux" begin
        calc_interface_flux!(cache_p.elements.surface_flux_values, mesh,
                             equations_parabolic, dg, parabolic_scheme, cache_p)
    end

    # Local boundary fluxes
    @trixi_timeit timer() "prolong2boundaries" begin
        prolong2boundaries!(cache_p, flux_viscous, mesh, equations_parabolic, dg)
    end

    @trixi_timeit timer() "boundary flux" begin
        calc_boundary_flux_divergence!(cache_p, t,
    ...
    @trixi_timeit timer() "prolong2mortars" begin
        prolong2mortars_divergence!(cache_p, flux_viscous, mesh, equations_parabolic,
                                    dg.mortar, dg)
    end

    @trixi_timeit timer() "mortar flux" begin
        calc_mortar_flux_divergence!(cache_p.elements.surface_flux_values,
                                     mesh, equations_parabolic, dg.mortar,
                                     dg, parabolic_scheme, cache_p)
    end
    ...
    @trixi_timeit timer() "MPI interface flux divergence" begin
        calc_mpi_interface_flux_divergence!(cache_p.elements.surface_flux_values,
                                            mesh, equations_parabolic,
                                            dg, parabolic_scheme, cache_p)
    end
    ...
    @trixi_timeit timer() "MPI mortar flux divergence" begin
        calc_mpi_mortar_flux_divergence!(cache_p.elements.surface_flux_values,
                                         mesh, equations_parabolic, dg.mortar,
                                         dg, parabolic_scheme, cache_p)
    end
    ...
    @trixi_timeit timer() "surface integral" begin
        calc_surface_integral!(du, u, mesh, equations_parabolic,
                               dg.surface_integral, dg, cache_p)
    end

    @trixi_timeit timer() "Jacobian" begin
        apply_jacobian_parabolic!(du, mesh, equations_parabolic, dg, cache_p)
    end
...
@inline function calc_interface_flux_gradient!(surface_flux_values, mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}},
                                               equations_parabolic,
                                               dg::DG, parabolic_scheme, cache,
...
function calc_volume_integral!(du, flux_viscous, mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}},
                               equations_parabolic::AbstractEquationsParabolic,
                               dg::DGSEM, cache)
...
function calc_interface_flux!(surface_flux_values, mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}},
...
function prolong2mortars_divergence!(cache, flux_viscous,
                                     mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}}, equations_parabolic,
                                     mortar_l2::LobattoLegendreMortarL2,
                                     dg::DGSEM)
    @unpack neighbor_ids, node_indices = cache.mortars
...
@inline function calc_mpi_interface_flux_divergence!(surface_flux_values,
    ...
    viscous_flux_normal_ll, viscous_flux_normal_rr =
        get_surface_node_vars(u, equations_parabolic, dg,
...
@inline function calc_mpi_mortar_flux_divergence!(fstar,
    ...
    for v in eachvariable(equations_parabolic)
        viscous_flux_normal_ll = u[1, v, position_index, i_node_index, j_node_index, mortar_index]
        viscous_flux_normal_rr = u[2, v, position_index, i_node_index, j_node_index, mortar_index]
...
function calc_volume_integral_gradient!(gradients, u_transformed,
                                        mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}}, # for dispatch only
                                        equations_parabolic::AbstractEquationsParabolic,
                                        dg::DG, cache)
...
function calc_boundary_flux!(cache, t,
                             ...
                             operator_type, mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}},
                             equations_parabolic::AbstractEquationsParabolic,
...
function prolong2mpiinterfaces!(cache, flux_viscous::Tuple,
    ...
    @threaded for interface in eachmpiinterface(dg, cache)
        ...
        k_start, k_step_i, k_step_j = index_to_start_step_3d(local_indices[3], index_range)
...
function apply_jacobian_parabolic!(du::AbstractArray, mesh::Union{P4estMeshParallel{3}, T8codeMeshParallel{3}},
                                   equations_parabolic::AbstractEquationsParabolic,
                                   dg::DG, cache)
