    ...
    @inline function calcflux_fhat!(
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in (i + 1):nnodes(dg)
                ...
                multiply_add_to_node_vars!(
                    flux_temp, derivative_split[i, ii], fluxtilde1,
                    equations, dg, i, j
                )
                multiply_add_to_node_vars!(
                    flux_temp, derivative_split[ii, i], fluxtilde1,
                    equations, dg, ii, j
                )
            end
        end
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for jj in (j + 1):nnodes(dg)
                ...
                multiply_add_to_node_vars!(
                    flux_temp, derivative_split[j, jj], fluxtilde2,
                    equations, dg, i, j
                )
                multiply_add_to_node_vars!(
                    flux_temp, derivative_split[jj, j], fluxtilde2,
                    equations, dg, i, jj
                )
            end
        end
