...
@muladd begin
    ...
    function init_elements!(elements, mesh::StructuredMesh{3}, basis::LobattoLegendreBasis)
        ...
        for cell_z in 1:size(mesh, 3), cell_y in 1:size(mesh, 2), cell_x in 1:size(mesh, 1)
            ...
            calc_node_coordinates!(
                node_coordinates, element, cell_x, cell_y, cell_z,
    ...
    function calc_jacobian_matrix!(
            jacobian_matrix::AbstractArray{<:Any, 6}, element,
            node_coordinates, basis
        )
    ...
    function calc_inverse_jacobian!(
            inverse_jacobian::AbstractArray{<:Any, 4}, element,
    ...
    function initialize_left_neighbor_connectivity!(
            left_neighbors, mesh::StructuredMesh{3},
            linear_indices
        )
        ...
        for cell_z in 1:size(mesh, 3), cell_y in 1:size(mesh, 2)
            ...
            if isperiodic(mesh, 1)
                # Periodic boundary
                left_neighbors[1, linear_indices[1, cell_y, cell_z]] = linear_indices[
                    end,
                    cell_y,
                    cell_z,
                ]
        ...
        for cell_z in 1:size(mesh, 3), cell_x in 1:size(mesh, 1)
            ...
            if isperiodic(mesh, 2)
                # Periodic boundary
                left_neighbors[2, linear_indices[cell_x, 1, cell_z]] = linear_indices[
                    cell_x,
                    end,
                    cell_z,
                ]
        ...
        for cell_y in 1:size(mesh, 2), cell_x in 1:size(mesh, 1)
            ...
            if isperiodic(mesh, 3)
                # Periodic boundary
                left_neighbors[3, linear_indices[cell_x, cell_y, 1]] = linear_indices[
                    cell_x,
                    cell_y,
                    end,
                ]
