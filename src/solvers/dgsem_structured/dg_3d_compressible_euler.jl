...
@inline function flux_differencing_kernel!(
    ...
    for j in eachnode(dg), jj in (j + 1):nnodes(dg)
        @turbo for k in eachnode(dg), i in eachnode(dg)
            ...
            normal_direction_1 = 0.5 * (
                contravariant_vectors_y[i, j, k, 1] +
                    contravariant_vectors_y[i, jj, k, 1]
            )
            normal_direction_2 = 0.5 * (
                contravariant_vectors_y[i, j, k, 2] +
                    contravariant_vectors_y[i, jj, k, 2]
            )
            normal_direction_3 = 0.5 * (
                contravariant_vectors_y[i, j, k, 3] +
    ...
    GC.@preserve u_prim begin
        ...
        @turbo for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ij = i + nnodes(dg) * (j - 1)
            contravariant_vectors_z[ij, k, 1] = contravariant_vectors[
                1, 3, i, j, k,
                element,
            ]
            contravariant_vectors_z[ij, k, 2] = contravariant_vectors[
                2, 3, i, j, k,
                element,
            ]
            contravariant_vectors_z[ij, k, 3] = contravariant_vectors[
                3, 3, i, j, k,
                element,
            ]
        end
