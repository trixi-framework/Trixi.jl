    ...
    function calc_bounds_twosided_interface!(
        ...
        for element in eachelement(dg, cache)
            ...
            if left != 0
                for j in eachnode(dg)
                    ...
                    var_min[nnodes(dg), j, left] = min(
                        var_min[nnodes(dg), j, left],
                        var_element
                    )
                    var_max[nnodes(dg), j, left] = max(
                        var_max[nnodes(dg), j, left],
                        var_element
                    )
                end
            end
            if lower != 0
                for i in eachnode(dg)
                    ...
                    var_min[i, nnodes(dg), lower] = min(
                        var_min[i, nnodes(dg), lower],
                        var_element
                    )
                    var_max[i, nnodes(dg), lower] = max(
                        var_max[i, nnodes(dg), lower],
                        var_element
                    )
                end
            end
        ...
        if !isperiodic(mesh, 1)
            ...
            for cell_y in axes(mesh, 2)
                ...
                for j in eachnode(dg)
                    Ja1 = get_contravariant_vector(
                        1, contravariant_vectors, nnodes(dg), j,
                        element
                    )
                    ...
                    var_min[nnodes(dg), j, element] = min(
                        var_min[nnodes(dg), j, element],
                        var_outer
                    )
                    var_max[nnodes(dg), j, element] = max(
                        var_max[nnodes(dg), j, element],
                        var_outer
                    )
                end
            end
        ...
        if !isperiodic(mesh, 2)
            ...
            for cell_x in axes(mesh, 1)
                ...
                for i in eachnode(dg)
                    Ja2 = get_contravariant_vector(
                        2, contravariant_vectors, i, nnodes(dg),
                        element
                    )
                    ...
                    var_min[i, nnodes(dg), element] = min(
                        var_min[i, nnodes(dg), element],
                        var_outer
                    )
                    var_max[i, nnodes(dg), element] = max(
                        var_max[i, nnodes(dg), element],
                        var_outer
                    )
                end
            end
        ...
        for element in eachelement(dg, cache)
            ...
            if left != 0
                for j in eachnode(dg)
                    var_left = variable(
                        get_node_vars(
                            u, equations, dg, nnodes(dg), j,
                            left
                        ), equations
                    )
                    var_element = variable(
                        get_node_vars(u, equations, dg, 1, j, element),
                        equations
                    )

                    var_minmax[1, j, element] = minmax(var_minmax[1, j, element], var_left)
                    var_minmax[nnodes(dg), j, left] = minmax(
                        var_minmax[
                            nnodes(dg), j,
                            left,
                        ], var_element
            ...
            if lower != 0
                for i in eachnode(dg)
                    var_lower = variable(
                        get_node_vars(
                            u, equations, dg, i, nnodes(dg),
                            lower
                        ), equations
                    )
                    var_element = variable(
                        get_node_vars(u, equations, dg, i, 1, element),
                        equations
                    )

                    var_minmax[i, 1, element] = minmax(var_minmax[i, 1, element], var_lower)
                    var_minmax[i, nnodes(dg), lower] = minmax(
                        var_minmax[
                            i, nnodes(dg),
                            lower,
                        ],
        ...
        if !isperiodic(mesh, 1)
            ...
            for cell_y in axes(mesh, 2)
                ...
                for j in eachnode(dg)
                    ...
                    var_minmax[nnodes(dg), j, element] = minmax(
                        var_minmax[
                            nnodes(dg), j,
                            element,
                        ],
        ...
        if !isperiodic(mesh, 2)
            ...
            for cell_x in axes(mesh, 1)
                ...
                for i in eachnode(dg)
                    ...
                    var_minmax[i, nnodes(dg), element] = minmax(
                        var_minmax[
                            i, nnodes(dg),
                            element,
                        ],
