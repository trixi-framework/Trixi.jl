...
@muladd begin
    ...
    function init_elements!(
            elements, mesh::Union{StructuredMesh{2}, StructuredMeshView{2}},
        ...
        for cell_y in 1:size(mesh, 2), cell_x in 1:size(mesh, 1)
            ...
            calc_node_coordinates!(
                node_coordinates, element, cell_x, cell_y, mesh.mapping,
                mesh, basis
            )
    ...
    function calc_contravariant_vectors!(
        ...
        @turbo for j in indices((contravariant_vectors, jacobian_matrix), (4, 4)),
            ...
            # First contravariant vector Ja^1
            contravariant_vectors[1, 1, i, j, element] = jacobian_matrix[
                2, 2, i, j,
                element,
            ]
            contravariant_vectors[2, 1, i, j, element] = -jacobian_matrix[
                1, 2, i, j,
                element,
            ]

            # Second contravariant vector Ja^2
            contravariant_vectors[1, 2, i, j, element] = -jacobian_matrix[
                2, 1, i, j,
                element,
            ]
            contravariant_vectors[2, 2, i, j, element] = jacobian_matrix[
                1, 1, i, j,
                element,
            ]
        end
    ...
    function calc_inverse_jacobian!(
            inverse_jacobian::AbstractArray{<:Any, 3}, element,
            jacobian_matrix
        )
