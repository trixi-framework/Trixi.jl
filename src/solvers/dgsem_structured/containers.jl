...
@muladd begin
    ...
    struct ElementContainer{
            NDIMS, RealT <: Real, uEltype <: Real, NDIMSP1, NDIMSP2,
            NDIMSP3,
        }
    ...
    function init_elements(
        ...
        inverse_jacobian = Array{RealT, NDIMS + 1}(
            undef,
            ntuple(_ -> nnodes(basis), NDIMS)...,
            nelements
        )
    ...
    function Base.eltype(::ElementContainer{NDIMS, RealT, uEltype}) where {
            NDIMS, RealT,
            uEltype,
        }
        uEltype
