...
@muladd begin
    ...
    function rhs!(
        ...
        @trixi_timeit timer() "boundary flux" begin
            calc_boundary_flux!(
                cache, u, t, boundary_conditions, mesh, equations,
                dg.surface_integral, dg
            )
        end
...
=#
    @inline function weak_form_kernel!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            Ja21, Ja22, Ja23 = get_contravariant_vector(
                2, contravariant_vectors, i, j, k,
                element
            )
            contravariant_flux2 = Ja21 * flux1 + Ja22 * flux2 + Ja23 * flux3
            for jj in eachnode(dg)
                multiply_add_to_node_vars!(
                    du, alpha * derivative_dhat[jj, j],
            ...
            Ja31, Ja32, Ja33 = get_contravariant_vector(
                3, contravariant_vectors, i, j, k,
                element
            )
            contravariant_flux3 = Ja31 * flux1 + Ja32 * flux2 + Ja33 * flux3
            for kk in eachnode(dg)
                multiply_add_to_node_vars!(
                    du, alpha * derivative_dhat[kk, k],
    ...
    @inline function flux_differencing_kernel!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in (i + 1):nnodes(dg)
                ...
                Ja1_node_ii = get_contravariant_vector(
                ...
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[i, ii], fluxtilde1,
                    equations, dg, i, j, k, element
                )
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[ii, i], fluxtilde1,
            ...
            for jj in (j + 1):nnodes(dg)
                ...
                # pull the contravariant vectors and compute the average
                Ja2_node_jj = get_contravariant_vector(
                ...
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[j, jj], fluxtilde2,
                    equations, dg, i, j, k, element
                )
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[jj, j], fluxtilde2,
            ...
            for kk in (k + 1):nnodes(dg)
                ...
                # pull the contravariant vectors and compute the average
                Ja3_node_kk = get_contravariant_vector(
                ...
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[k, kk], fluxtilde3,
                    equations, dg, i, j, k, element
                )
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[kk, k], fluxtilde3,
    ...
    @inline function flux_differencing_kernel!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in eachnode(dg)
                ...
                # pull the contravariant vectors and compute the average
                Ja1_node_ii = get_contravariant_vector(
                    1, contravariant_vectors, ii, j, k,
                    element
                )
                Ja1_avg = 0.5f0 * (Ja1_node + Ja1_node_ii)
                # compute the contravariant nonconservative flux in the direction of the
                # averaged contravariant vector
                fluxtilde1 = nonconservative_flux(
                    u_node, u_node_ii, Ja1_node, Ja1_avg,
                    equations
                )
            ...
            for jj in eachnode(dg)
                ...
                # pull the contravariant vectors and compute the average
                Ja2_node_jj = get_contravariant_vector(
                    2, contravariant_vectors, i, jj, k,
                    element
                )
                Ja2_avg = 0.5f0 * (Ja2_node + Ja2_node_jj)
                # compute the contravariant nonconservative flux in the direction of the
                # averaged contravariant vector
                fluxtilde2 = nonconservative_flux(
                    u_node, u_node_jj, Ja2_node, Ja2_avg,
                    equations
                )
                integral_contribution = integral_contribution +
            ...
            for kk in eachnode(dg)
                ...
                # pull the contravariant vectors and compute the average
                Ja3_node_kk = get_contravariant_vector(
                    3, contravariant_vectors, i, j, kk,
                    element
                )
                Ja3_avg = 0.5f0 * (Ja3_node + Ja3_node_kk)
                # compute the contravariant nonconservative flux in the direction of the
                # averaged contravariant vector
                fluxtilde3 = nonconservative_flux(
                    u_node, u_node_kk, Ja3_node, Ja3_avg,
                    equations
                )
                integral_contribution = integral_contribution +
    ...
    @inline function calcflux_fv!(
        ...
        for k in eachnode(dg), j in eachnode(dg)
            normal_direction = get_contravariant_vector(
                1, contravariant_vectors, 1, j, k,
                element
            )
    ...
    @inline function calc_interface_flux!(
            surface_flux_values, left_element, right_element,
            orientation, u,
    ...
    function calc_boundary_flux!(
            cache, u, t, boundary_condition::BoundaryConditionPeriodic,
    ...
    function calc_boundary_flux!(
            cache, u, t, boundary_conditions::NamedTuple,
            mesh::StructuredMesh{3}, equations, surface_integral,
            dg::DG
        )
