...
@muladd begin
    ...
    function DGMulti(;
        ...
        DGMulti(
            element_type, approximation_type, volume_integral, surface_integral;
    ...
    """
    function DGMultiBasis(
            element_type, polydeg; approximation_type = Polynomial(),
            kwargs...
        )
    ...
    function DGMultiMesh(
            dg::DGMulti, geometric_term_type, md::MeshData{NDIMS},
            boundary_faces
        ) where {NDIMS}
        return DGMultiMesh{
            NDIMS, typeof(geometric_term_type), typeof(md),
    ...
    """
    function DGMultiMesh(
            dg::DGMulti{NDIMS}, vertex_coordinates, EToV::AbstractArray;
            is_on_boundary = nothing,
    ...
    """
    function DGMultiMesh(
            dg::DGMulti{2, Tri}, triangulateIO,
        ...
        md = MeshData(vertex_coordinates, EToV, dg.basis)
        md = StartUpDG.make_periodic(md, periodicity)
        boundary_faces = StartUpDG.tag_boundary_faces(
            triangulateIO, dg.basis, md,
            boundary_dict
        )
    ...
    """
    function DGMultiMesh(
            dg::DGMulti{NDIMS}, cells_per_dimension;
    ...
    """
    function DGMultiMesh(
            dg::DGMulti{NDIMS}, cells_per_dimension, mapping;
            is_on_boundary = nothing,
            periodicity = ntuple(_ -> false, NDIMS), kwargs...
        ) where {NDIMS}
        vertex_coordinates, EToV = StartUpDG.uniform_mesh(
    ...
    """
    function DGMultiMesh(
            dg::DGMulti{NDIMS}, filename::String;
            periodicity = ntuple(_ -> false, NDIMS)
        ) where {NDIMS}
        ...
        boundary_faces = Dict(
            Pair.(
                keys(md.mesh_type.boundary_faces),
    ...
    struct LazyMatrixLinearCombo{Tcoeffs, N, Tv, TA <: AbstractMatrix{Tv}} <:
        ...
        function LazyMatrixLinearCombo(matrices, coeffs)
            ...
            new{
                typeof(first(coeffs)), length(matrices), eltype(first(matrices)),
    ...
    function SimpleKronecker(NDIMS, A, eltype_A = eltype(A))
        ...
        tmp_storage = [
            zeros(eltype_A, ntuple(_ -> size(A, 2), NDIMS)...)
