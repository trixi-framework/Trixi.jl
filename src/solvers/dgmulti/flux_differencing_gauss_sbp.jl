...
struct TensorProductGaussFaceOperator{
        NDIMS, OperatorType <: AbstractGaussOperator,
...
function TensorProductGaussFaceOperator(
    ...
    return TensorProductGaussFaceOperator{1, T_op, Tm, Tw, Tf, Ti}(
...
function TensorProductGaussFaceOperator(
    ...
    face_indices_tensor_product = zeros(
        Int, 2, nnodes_1d, nnodes_1d,
...
@muladd begin
    ...
    function create_cache(
        ...
        interp_matrix_lobatto_to_gauss = SimpleKronecker(
            NDIMS,
            interp_matrix_lobatto_to_gauss_1D,
            uEltype
        )
        interp_matrix_gauss_to_lobatto = SimpleKronecker(
            NDIMS,
        ...
        projection_matrix_gauss_to_face = TensorProductGaussFaceOperator(
            Projection{Static.False()}(),
            dg
        )
        ...
        return (;
            cache..., projection_matrix_gauss_to_face, gauss_LIFT, inv_gauss_weights,
    ...
    function entropy_projection!(
        ...
        apply_to_each_field(
            mul_by!(interp_matrix_gauss_to_face), entropy_var_face_values,
            entropy_var_values
        )

        # directly copy over volume values (no entropy projection required)
        entropy_projected_volume_values = view(
            entropy_projected_u_values, volume_indices,
            :
        )
    ...
    function calc_surface_integral!(
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            ...
            apply_to_each_field(
                mul_by!(gauss_LIFT), gauss_volume_local,
    ...
    @inline function flux_differencing_kernel!(
        ...
        local_flux_differencing!(
            fluxdiff_local, u_local, element,
    ...
    function project_rhs_to_gauss_nodes!(
            du, rhs_local, element, mesh::DGMultiMesh,
