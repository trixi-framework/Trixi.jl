...
function low_order_flux_differencing_kernel!(du, u, element, mesh::DGMultiMesh,
  ...
  flux_conservative, flux_nonconservative = volume_flux_fv
  ...
  for i in row_ids
    ...
    for id in nzrange(A_base, i)
      ...
      # note that we do not need to normalize `normal_direction_ij` since
      # it is typically normalized within the flux computation.
      f_cons_ij = flux_conservative(u_i, u_j, normal_direction_ij, equations)

      # The `normal_direction_ij::AbstractVector` has to be passed in twice.
      # This is because nonconservative fluxes are evaluated using both the
      # normal and its average at interfaces (on curved meshes).
      f_noncons_ij = flux_nonconservative(u_i, u_j, normal_direction_ij,
                                          normal_direction_ij, equations)
      du_i = du_i + 2 * f_cons_ij + f_noncons_ij
    end
    rhs_local[i] = du_i
  end
...
function low_order_flux_differencing_kernel!(du, u, element, mesh::DGMultiMesh{NDIMS, <:NonAffine},
  ...
  for i in row_ids
    ...
    for id in nzrange(A_base, i)
      ...
      f_noncons_ij = flux_nonconservative(u_i, u_j, normal_direction_ij,
                                          normal_direction_ij, equations)
      du_i = du_i + 2 * (f_cons_ij + 0.5 * f_noncons_ij)
    end
    rhs_local[i] = du_i
  end
