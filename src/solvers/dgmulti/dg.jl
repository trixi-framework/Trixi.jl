...
@muladd begin
    ...
    function create_cache(
            mesh::DGMultiMesh{NDIMS}, equations, dg::DGMultiWeakForm, RealT,
            uEltype
        ) where {NDIMS}
        rd = dg.basis
        md = mesh.md
        ...
        return (;
            md, weak_differentiation_matrices, lift_scalings, invJ, dxidxhatj,
    ...
    function compute_coefficients!(
        ...
        @threaded for i in each_quad_node_global(mesh, dg, cache)
            u_values[i] = initial_condition(
                SVector(getindex.(md.xyzq, i)),
                t, equations
            )
        end
    ...
    function prolong2interfaces!(
            cache, u, mesh::DGMultiMesh, equations,
    ...
    function calc_volume_integral!(
            du, u, mesh::DGMultiMesh{NDIMS, <:NonAffine},
            have_nonconservative_terms::False, equations,
            volume_integral::VolumeIntegralWeakForm, dg::DGMulti,
            cache
        ) where {NDIMS}
    ...
    function calc_interface_flux!(
            cache, surface_integral::SurfaceIntegralWeakForm,
            mesh::DGMultiMesh,
            have_nonconservative_terms::False, equations,
            dg::DGMulti{NDIMS}
        ) where {NDIMS}
    ...
    function calc_interface_flux!(
        ...
        @threaded for face_node_index in each_face_node_global(mesh, dg, cache)
            ...
            if idM != idP
                ...
                nonconservative_part = flux_nonconservative(
                    uM, uP, normal, normal,
                    equations
                )
    ...
    function calc_surface_integral!(
            du, u, mesh::DGMultiMesh, equations,
    ...
    # We pass the `surface_integral` argument solely for dispatch
    function prolong2interfaces!(
            cache, u, mesh::DGMultiMesh, equations, surface_integral,
            dg::DGMultiSBP
        )
        rd = dg.basis
        @unpack Fmask = rd
    ...
    function calc_boundary_flux!(
            cache, t, boundary_conditions::BoundaryConditionPeriodic,
    ...
    function calc_single_boundary_flux!(
            cache, t, boundary_condition, boundary_key, mesh,
            have_nonconservative_terms::False, equations,
            dg::DGMulti{NDIMS}
        ) where {NDIMS}
        rd = dg.basis
        ...
        for f in mesh.boundary_faces[boundary_key]
            for i in Base.OneTo(num_pts_per_face)
                ...
                flux_face_values[i, f] = boundary_condition(
                    u_face_values[i, f],
        ...
        for f in mesh.boundary_faces[boundary_key]
            for i in Base.OneTo(num_pts_per_face)
                ...
                flux_face_values[i, f] = (
                    cons_flux_at_face_node +
                        0.5 * noncons_flux_at_face_node
                ) * Jf[i, f]
            end
        end
    ...
    function invert_jacobian!(
            du, mesh::DGMultiMesh, equations, dg::DGMulti, cache;
            scaling = -1
        )
        @threaded for e in eachelement(mesh, dg, cache)
            invJ = cache.invJ[1, e]
    ...
    function invert_jacobian!(
            du, mesh::DGMultiMesh{NDIMS, <:NonAffine}, equations,
            dg::DGMulti, cache; scaling = -1
        ) where {NDIMS}
    ...
    function calc_sources!(
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            ...
            for i in each_quad_node(mesh, dg, cache)
                source_values[i] = source_terms(
                    u_e[i], SVector(getindex.(md.xyzq, i, e)),
                    t, equations
                )
            end
