...
@muladd begin
        ...
        for i in row_ids
            ...
            for j in col_ids
                ...
                if j > i
                    ...
                    AF_ij = 2 * A[i, j] *
                        volume_flux(
                        u_i, u_j, orientation_or_normal_direction,
                        equations
                    )
                    du_i = du_i + AF_ij
                    du[j] = du[j] - AF_ij
    ...
    function create_cache(
            mesh::DGMultiMesh, equations, dg::DGMultiFluxDiffSBP, RealT,
            uEltype
        )
        rd = dg.basis
        md = mesh.md
        ...
        local_values_threaded = [
            allocate_nested_array(uEltype, nvars, (rd.Nq,), dg)
        ...
        fluxdiff_local_threaded = [
            zeros(SVector{nvars, uEltype}, rd.Nq)
                for _ in 1:Threads.nthreads()
        ]

        return (;
            md, Qrst_skew, dxidxhatj = md.rstxyzJ,
    ...
    function create_cache(mesh::DGMultiMesh, equations, dg::DGMultiFluxDiff, RealT, uEltype)
        ...
        nvars = nvariables(equations)
        entropy_var_values = allocate_nested_array(
            uEltype, nvars, (rd.Nq, md.num_elements),
            dg
        )
        ...
        u_values = allocate_nested_array(uEltype, nvars, size(md.xq), dg)
        u_face_values = view(
            entropy_projected_u_values, (rd.Nq + 1):num_quad_points_total,
            :
        )
        ...
        fluxdiff_local_threaded = [
            zeros(SVector{nvars, uEltype}, num_quad_points_total)
                for _ in 1:Threads.nthreads()
        ]
        rhs_local_threaded = [
    ...
    @inline function entropy2cons!(
        ...
        @threaded for i in eachindex(projected_entropy_var_values)
            entropy_projected_u_values[i] = entropy2cons(
                projected_entropy_var_values[i],
                equations
            )
        end
    end
    ...
    @inline function has_sparse_operators(
            ::Union{Tri, Tet},
            approx_type::AT
        ) where {AT <: SBP}
        False()
    ...
    @inline function local_flux_differencing!(
            fluxdiff_local, u_local, element_index,
    ...
    @inline function local_flux_differencing!(
        ...
        for dim in eachdim(mesh)
            ...
            half_Qi_skew = build_lazy_physical_derivative(
                element_index, dim, mesh, dg,
                cache, 0.5
            )
            # False() indicates the flux is non-symmetric.
            hadamard_sum!(
                fluxdiff_local, half_Qi_skew,
    ...
    function calc_sources!(
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            for i in each_quad_node(mesh, dg, cache)
                du[i, e] += source_terms(
                    u[i, e], SVector(getindex.(md.xyzq, i, e)), t,
                    equations
                )
            end
        end
    ...
    function rhs!(
            du, u, t, mesh, equations, initial_condition, boundary_conditions::BC,
            source_terms::Source, dg::DGMultiFluxDiff, cache
        ) where {Source, BC}
        ...
        @trixi_timeit timer() "volume integral" begin
            calc_volume_integral!(
                du, u, mesh, have_nonconservative_terms(equations),
                equations,
