...
function create_cache_parabolic(
    ...
    local_u_values_threaded = [
        similar(u_transformed, dg.basis.Nq)
    ...
    local_flux_face_values_threaded = [
        similar(scalar_flux_face_values[:, 1])
            for _ in 1:Threads.nthreads()
    ]

    return (;
        u_transformed, gradients, flux_viscous,
...
function calc_gradient_surface_integral!(
        gradients, u, scalar_flux_face_values,
...
function calc_gradient_volume_integral!(
    ...
    @threaded for e in eachelement(mesh, dg)
        for i in eachdim(mesh), j in eachdim(mesh)
            ...
            apply_to_each_field(
                mul_by_accum!(
                    strong_differentiation_matrices[j],
                    dxidxhatj
                ),
...
function calc_gradient_volume_integral!(
        gradients, u, mesh::DGMultiMesh{NDIMS, <:NonAffine},
        equations::AbstractEquationsParabolic,
        dg::DGMulti, cache, cache_parabolic
    ) where {NDIMS}
...
function calc_gradient!(
        gradients, u::StructArray, t, mesh::DGMultiMesh,
        equations::AbstractEquationsParabolic,
    ...
    calc_gradient_volume_integral!(
        gradients, u, mesh, equations, dg, cache,
        cache_parabolic
    )
    ...
    calc_boundary_flux!(
        scalar_flux_face_values, u_face_values, t, Gradient(),
        boundary_conditions,
...
function invert_jacobian_gradient!(
        gradients, mesh::DGMultiMesh, equations, dg::DGMulti,
...
function invert_jacobian_gradient!(
        gradients, mesh::DGMultiMesh{NDIMS, <:NonAffine},
        equations,
        dg::DGMulti, cache, cache_parabolic
    ) where {NDIMS}
...
function calc_boundary_flux!(
        flux, u, t, operator_type, ::BoundaryConditionPeriodic,
...
function calc_boundary_flux!(
    ...
    calc_boundary_flux!(
        flux, u, t, operator_type, Base.tail(boundary_conditions),
        mesh, equations, dg, cache, cache_parabolic
    )
end
...
function calc_single_boundary_flux!(
        flux_face_values, u_face_values, t,
...
function calc_viscous_fluxes!(
        flux_viscous, u, gradients, mesh::DGMultiMesh,
        equations::AbstractEquationsParabolic,
        dg::DGMulti, cache, cache_parabolic
    )
    for dim in eachdim(mesh)
...
function calc_viscous_penalty!(
        scalar_flux_face_values, u_face_values, t,
        boundary_conditions,
        mesh, equations::AbstractEquationsParabolic, dg::DGMulti,
...
function calc_divergence_volume_integral!(
        du, u, flux_viscous, mesh::DGMultiMesh,
        equations::AbstractEquationsParabolic,
        dg::DGMulti, cache, cache_parabolic
    )
...
function calc_divergence!(
        du, u::StructArray, t, flux_viscous, mesh::DGMultiMesh,
        equations::AbstractEquationsParabolic,
    ...
    calc_divergence_volume_integral!(
        du, u, flux_viscous, mesh, equations, dg, cache,
        cache_parabolic
    )
    ...
    calc_boundary_flux!(
        scalar_flux_face_values, cache_parabolic.u_face_values, t,
        Divergence(),
    ...
    apply_to_each_field(
        mul_by_accum!(cache_parabolic.divergence_lift_matrix), du,
...
function rhs_parabolic!(
    ...
    @trixi_timeit timer() "calc viscous fluxes" begin
        calc_viscous_fluxes!(
            flux_viscous, u_transformed, gradients,
    ...
    @trixi_timeit timer() "calc divergence" begin
        calc_divergence!(
            du, u_transformed, t, flux_viscous, mesh, equations_parabolic,
