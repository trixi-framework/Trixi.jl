...
@muladd begin
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j in eachnode(dg), k in eachnode(dg)
                mul!(
                    view(du_vectors, :, j, k, element), D, view(f_element, :, j, k),
            ...
            for i in eachnode(dg), k in eachnode(dg)
                mul!(
                    view(du_vectors, i, :, k, element), D, view(f_element, i, :, k),
                    one(eltype(du)), one(eltype(du))
                )
            end
            ...
            for i in eachnode(dg), j in eachnode(dg)
                mul!(
                    view(du_vectors, i, j, :, element), D, view(f_element, i, j, :),
                    one(eltype(du)), one(eltype(du))
                )
            end
        end
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j in eachnode(dg), k in eachnode(dg)
                mul!(
                    view(du_vectors, :, j, k, element), D_minus,
                    view(f_plus_element, :, j, k),
                    one(eltype(du)), one(eltype(du))
                )
                mul!(
                    view(du_vectors, :, j, k, element), D_plus,
            ...
            for i in eachnode(dg), k in eachnode(dg)
                mul!(
                    view(du_vectors, i, :, k, element), D_minus,
                    view(f_plus_element, i, :, k),
                    one(eltype(du)), one(eltype(du))
                )
                mul!(
                    view(du_vectors, i, :, k, element), D_plus,
            ...
            for i in eachnode(dg), j in eachnode(dg)
                mul!(
                    view(du_vectors, i, j, :, element), D_minus,
                    view(f_plus_element, i, j, :),
                    one(eltype(du)), one(eltype(du))
                )
                mul!(
                    view(du_vectors, i, j, :, element), D_plus,
