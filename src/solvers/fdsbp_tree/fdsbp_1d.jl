...
@muladd begin
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            mul!(
                view(du_vectors, :, element), D, view(f_element, :),
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            mul!(
                view(du_vectors, :, element), D_plus, view(f_minus_element, :),
                one(eltype(du)), one(eltype(du))
            )
            mul!(
                view(du_vectors, :, element), D_minus, view(f_plus_element, :),
                one(eltype(du)), one(eltype(du))
            )
        end
