...
@muladd begin
    ...
    function create_cache(
            mesh::Union{TreeMesh{2}, UnstructuredMesh2D}, equations,
    ...
    function create_cache(
        ...
        f_minus_plus_threaded = [
            similar(f, ntuple(_ -> nnodes(dg), ndims(mesh))...)
    ...
    function calc_volume_integral!(
        ...
        if nvariables(equations) == 1
            ...
            du_vectors = reshape(
                reinterpret(
                    SVector{nvariables(equations), eltype(du)},
                    du
                ),
                nnodes(dg), nnodes(dg), nelements(dg, cache)
            )
            ...
            du_vectors = reinterpret(
                reshape, SVector{nvariables(equations), eltype(du)},
                du
            )
        end
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j in eachnode(dg)
                mul!(
                    view(du_vectors, :, j, element), D, view(f_element, :, j),
            ...
            for i in eachnode(dg)
                mul!(
                    view(du_vectors, i, :, element), D, view(f_element, i, :),
                    one(eltype(du)), one(eltype(du))
                )
            end
        end
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j in eachnode(dg)
                mul!(
                    view(du_vectors, :, j, element), D_minus, view(f_plus_element, :, j),
                    one(eltype(du)), one(eltype(du))
                )
                mul!(
                    view(du_vectors, :, j, element), D_plus, view(f_minus_element, :, j),
                    one(eltype(du)), one(eltype(du))
                )
            end
            ...
            for i in eachnode(dg)
                mul!(
                    view(du_vectors, i, :, element), D_minus, view(f_plus_element, i, :),
                    one(eltype(du)), one(eltype(du))
                )
                mul!(
                    view(du_vectors, i, :, element), D_plus, view(f_minus_element, i, :),
                    one(eltype(du)), one(eltype(du))
                )
            end
        end
        ...
        @threaded for element in eachelement(dg, cache)
            for l in eachnode(dg)
                ...
                multiply_add_to_node_vars!(
                    du, inv_weight_left, -(f_num - f_node),
                ...
                multiply_add_to_node_vars!(
                    du, inv_weight_right, +(f_num - f_node),
    ...
    function calc_interface_flux!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for i in eachnode(dg)
                ...
                flux_minus_rr = splitting(
                    u_rr, Val{:minus}(), orientations[interface],
                    equations
                )
                flux_plus_ll = splitting(
                    u_ll, Val{:plus}(), orientations[interface],
                    equations
                )
    ...
    function integrate_via_indices(
            ...
            dg::FDSBP, cache, args...; normalize = true
        ) where {Func}
