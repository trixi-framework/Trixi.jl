...
Base.@propagate_inbounds function calc_fstar!(destination::AbstractArray{<:Any, 3},
                                              equations,
...
function apply_jacobian!(backend::Nothing, du, mesh::TreeMesh{3},
    ...
    @boundscheck begin
        check_axes(du, equations, dg, cache)
        checkbounds(inverse_jacobian, eachelement(dg, cache))
    end
...
function calc_sources!(backend::Nothing, du, u, t, source_terms,
    ...
    @boundscheck begin
        ...
        checkbounds(node_coordinates, 1:ndims(equations), eachnode(dg), eachnode(dg),
                    eachnode(dg), eachelement(dg, cache))
    end
