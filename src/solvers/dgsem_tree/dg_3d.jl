...
@muladd begin
    ...
    function create_cache(
        ...
        fstar1_L_threaded = A4dp1_x[
            A4dp1_x(
                    undef, nvariables(equations), nnodes(dg) + 1,
                    nnodes(dg), nnodes(dg)
                )
        ...
        fstar2_L_threaded = A4dp1_y[
            A4dp1_y(
                    undef, nvariables(equations), nnodes(dg),
        ...
        return (;
            cache..., element_ids_dg, element_ids_dgfv, fstar1_L_threaded,
            fstar1_R_threaded,
    ...
    function create_cache(
        ...
        return (;
            fstar1_L_threaded, fstar1_R_threaded, fstar2_L_threaded,
            fstar2_R_threaded,
    ...
    function create_cache(
        ...
        fstar_upper_left_threaded = A3d[
            A3d(
                    undef, nvariables(equations), nnodes(mortar_l2),
                    nnodes(mortar_l2)
                )
                for _ in 1:Threads.nthreads()
        ]
...
=#
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in eachnode(dg)
                multiply_add_to_node_vars!(
                    du, alpha * derivative_dhat[ii, i], flux1,
            ...
            for jj in eachnode(dg)
                multiply_add_to_node_vars!(
                    du, alpha * derivative_dhat[jj, j], flux2,
            ...
            for kk in eachnode(dg)
                multiply_add_to_node_vars!(
                    du, alpha * derivative_dhat[kk, k], flux3,
    ...
    @inline function flux_differencing_kernel!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in (i + 1):nnodes(dg)
                ...
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[i, ii], flux1,
                    equations, dg, i, j, k, element
                )
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[ii, i], flux1,
                    equations, dg, ii, j, k, element
                )
            end
            ...
            for jj in (j + 1):nnodes(dg)
                ...
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[j, jj], flux2,
                    equations, dg, i, j, k, element
                )
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[jj, j], flux2,
                    equations, dg, i, jj, k, element
                )
            end
            ...
            for kk in (k + 1):nnodes(dg)
                ...
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[k, kk], flux3,
                    equations, dg, i, j, k, element
                )
                multiply_add_to_node_vars!(
                    du, alpha * derivative_split[kk, k], flux3,
                    equations, dg, i, j, kk, element
                )
            end
        end
    ...
    @inline function flux_differencing_kernel!(
        ...
        flux_differencing_kernel!(
            du, u, element, mesh, False(), equations, symmetric_flux,
            dg, cache, alpha
        )
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            multiply_add_to_node_vars!(
                du, alpha * 0.5f0, integral_contribution, equations,
                dg, i, j, k, element
            )
        end
    end
    ...
    function calc_volume_integral!(
        ...
        @trixi_timeit timer() "blended DG-FV" @threaded for idx_element in eachindex(element_ids_dgfv)
            ...
            fv_kernel!(
                du, u, mesh, nonconservative_terms, equations, volume_flux_fv,
    ...
    @inline function fv_kernel!(
        ...
        calcflux_fv!(
            fstar1_L, fstar1_R, fstar2_L, fstar2_R, fstar3_L, fstar3_R, u,
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            if orientations[interface] == 1
                ...
                for k in eachnode(dg), j in eachnode(dg), v in eachvariable(equations)
                    interfaces_u[1, v, j, k, interface] = u[
                        v, nnodes(dg), j, k,
                        left_element,
                    ]
            ...
            elseif orientations[interface] == 2
                ...
                for k in eachnode(dg), i in eachnode(dg), v in eachvariable(equations)
                    interfaces_u[1, v, i, k, interface] = u[
                        v, i, nnodes(dg), k,
                        left_element,
                    ]
                ...
                for j in eachnode(dg), i in eachnode(dg), v in eachvariable(equations)
                    interfaces_u[1, v, i, j, interface] = u[
                        v, i, j, nnodes(dg),
                        left_element,
                    ]
    ...
    function calc_boundary_flux!(
            cache, t, boundary_condition::BoundaryConditionPeriodic,
            mesh::TreeMesh{3}, equations, surface_integral, dg::DG
        )
    ...
    function calc_boundary_flux!(
            cache, t, boundary_conditions::NamedTuple,
            mesh::TreeMesh{3}, equations, surface_integral, dg::DG
        )
        ...
        calc_boundary_flux_by_direction!(
            surface_flux_values, t, boundary_conditions[1],
            equations, surface_integral, dg, cache,
            1, firsts[1], lasts[1]
        )
        calc_boundary_flux_by_direction!(
            surface_flux_values, t, boundary_conditions[2],
            equations, surface_integral, dg, cache,
            2, firsts[2], lasts[2]
        )
        calc_boundary_flux_by_direction!(
            surface_flux_values, t, boundary_conditions[3],
            equations, surface_integral, dg, cache,
            3, firsts[3], lasts[3]
        )
        calc_boundary_flux_by_direction!(
            surface_flux_values, t, boundary_conditions[4],
            equations, surface_integral, dg, cache,
            4, firsts[4], lasts[4]
        )
        calc_boundary_flux_by_direction!(
            surface_flux_values, t, boundary_conditions[5],
            equations, surface_integral, dg, cache,
            5, firsts[5], lasts[5]
        )
        calc_boundary_flux_by_direction!(
            surface_flux_values, t, boundary_conditions[6],
            equations, surface_integral, dg, cache,
            6, firsts[6], lasts[6]
        )
    end
        ...
        @threaded for boundary in first_boundary:last_boundary
            ...
            for j in eachnode(dg), i in eachnode(dg)
                ...
                flux = boundary_condition(
                    u_inner, orientations[boundary], direction, x, t,
                    surface_flux,
                    equations
                )
            ...
            if cache.mortars.large_sides[mortar] == 1 # -> small elements on right side
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for k in eachnode(dg), j in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[2, v, j, k, mortar] = u[
                                v, 1, j, k,
                                upper_left_element,
                            ]
                            cache.mortars.u_upper_right[2, v, j, k, mortar] = u[
                                v, 1, j, k,
                                upper_right_element,
                            ]
                            cache.mortars.u_lower_left[2, v, j, k, mortar] = u[
                                v, 1, j, k,
                                lower_left_element,
                            ]
                            cache.mortars.u_lower_right[2, v, j, k, mortar] = u[
                                v, 1, j, k,
                                lower_right_element,
                            ]
                        end
                ...
                elseif cache.mortars.orientations[mortar] == 2
                    ...
                    for k in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[2, v, i, k, mortar] = u[
                                v, i, 1, k,
                                upper_left_element,
                            ]
                            cache.mortars.u_upper_right[2, v, i, k, mortar] = u[
                                v, i, 1, k,
                                upper_right_element,
                            ]
                            cache.mortars.u_lower_left[2, v, i, k, mortar] = u[
                                v, i, 1, k,
                                lower_left_element,
                            ]
                            cache.mortars.u_lower_right[2, v, i, k, mortar] = u[
                                v, i, 1, k,
                                lower_right_element,
                            ]
                        end
                    ...
                    for j in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[2, v, i, j, mortar] = u[
                                v, i, j, 1,
                                upper_left_element,
                            ]
                            cache.mortars.u_upper_right[2, v, i, j, mortar] = u[
                                v, i, j, 1,
                                upper_right_element,
                            ]
                            cache.mortars.u_lower_left[2, v, i, j, mortar] = u[
                                v, i, j, 1,
                                lower_left_element,
                            ]
                            cache.mortars.u_lower_right[2, v, i, j, mortar] = u[
                                v, i, j, 1,
                                lower_right_element,
                            ]
                        end
                ...
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for k in eachnode(dg), j in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[1, v, j, k, mortar] = u[
                                v,
                                nnodes(dg),
                                j, k,
                                upper_left_element,
                            ]
                            cache.mortars.u_upper_right[1, v, j, k, mortar] = u[
                                v,
                                nnodes(dg),
                                j, k,
                                upper_right_element,
                            ]
                            cache.mortars.u_lower_left[1, v, j, k, mortar] = u[
                                v,
                                nnodes(dg),
                                j, k,
                                lower_left_element,
                            ]
                            cache.mortars.u_lower_right[1, v, j, k, mortar] = u[
                                v,
                                nnodes(dg),
                                j, k,
                                lower_right_element,
                ...
                elseif cache.mortars.orientations[mortar] == 2
                    ...
                    for k in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[1, v, i, k, mortar] = u[
                                v, i,
                                nnodes(dg),
                                k,
                                upper_left_element,
                            ]
                            cache.mortars.u_upper_right[1, v, i, k, mortar] = u[
                                v, i,
                                nnodes(dg),
                                k,
                                upper_right_element,
                            ]
                            cache.mortars.u_lower_left[1, v, i, k, mortar] = u[
                                v, i,
                                nnodes(dg),
                                k,
                                lower_left_element,
                            ]
                            cache.mortars.u_lower_right[1, v, i, k, mortar] = u[
                                v, i,
                                nnodes(dg),
                                k,
                                lower_right_element,
                    ...
                    for j in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[1, v, i, j, mortar] = u[
                                v, i, j,
                                nnodes(dg),
                                upper_left_element,
                            ]
                            cache.mortars.u_upper_right[1, v, i, j, mortar] = u[
                                v, i, j,
                                nnodes(dg),
                                upper_right_element,
                            ]
                            cache.mortars.u_lower_left[1, v, i, j, mortar] = u[
                                v, i, j,
                                nnodes(dg),
                                lower_left_element,
                            ]
                            cache.mortars.u_lower_right[1, v, i, j, mortar] = u[
                                v, i, j,
                                nnodes(dg),
                                lower_right_element,
                            ]
    ...
    function calc_mortar_flux!(
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            calc_fstar!(
                fstar_upper_left, equations, surface_flux, dg, u_upper_left, mortar,
                orientation
            )
            calc_fstar!(
                fstar_upper_right, equations, surface_flux, dg, u_upper_right,
                mortar, orientation
            )
            calc_fstar!(
                fstar_lower_left, equations, surface_flux, dg, u_lower_left, mortar,
                orientation
            )
            calc_fstar!(
                fstar_lower_right, equations, surface_flux, dg, u_lower_right,
                mortar, orientation
            )
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            if large_sides[mortar] == 1 # -> small elements on right side
                for j in eachnode(dg), i in eachnode(dg)
                    ...
                    u_upper_left_ll, u_upper_left_rr = get_surface_node_vars(
                        u_upper_left,
                        equations, dg,
                        i, j, mortar
                    )
                    u_upper_right_ll, u_upper_right_rr = get_surface_node_vars(
                        u_upper_right,
                        equations,
                        dg, i, j,
                        mortar
                    )
                    u_lower_left_ll, u_lower_left_rr = get_surface_node_vars(
                        u_lower_left,
                        equations, dg,
                        i, j, mortar
                    )
                    u_lower_right_ll, u_lower_right_rr = get_surface_node_vars(
                        u_lower_right,
                        equations,
                        dg, i, j,
                        mortar
                    ...
                    multiply_add_to_node_vars!(
                        fstar_upper_left, 0.5f0, noncons_upper_left,
                        equations, dg, i, j
                    )
                    multiply_add_to_node_vars!(
                        fstar_upper_right, 0.5f0,
                        noncons_upper_right,
                        equations, dg, i, j
                    )
                    multiply_add_to_node_vars!(
                        fstar_lower_left, 0.5f0, noncons_lower_left,
                        equations, dg, i, j
                    )
                    multiply_add_to_node_vars!(
                        fstar_lower_right, 0.5f0,
                        noncons_lower_right,
                        equations, dg, i, j
                    )
                end
    ...
    @inline function calc_fstar!(
            destination::AbstractArray{<:Any, 3}, equations,
    ...
    function calc_surface_integral!(
            du, u, mesh::Union{TreeMesh{3}, StructuredMesh{3}},
