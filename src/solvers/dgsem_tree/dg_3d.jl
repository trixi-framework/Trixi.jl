...
function create_cache(mesh::Union{TreeMesh{3}, StructuredMesh{3}, P4estMesh{3}},
                      equations, volume_integral::VolumeIntegralFluxDifferencing, dg::DG,
                      uEltype)
  NamedTuple()
end
...
function create_cache(mesh::Union{TreeMesh{3}, StructuredMesh{3}, P4estMesh{3}},
  ...
  fstar_upper_left_threaded = A3d[A3d(undef, nvariables(equations), nnodes(mortar_l2),
                                      nnodes(mortar_l2))
                                  for _ in 1:Threads.nthreads()]
  fstar_upper_right_threaded = A3d[A3d(undef, nvariables(equations), nnodes(mortar_l2),
                                       nnodes(mortar_l2))
                                   for _ in 1:Threads.nthreads()]
  fstar_lower_left_threaded = A3d[A3d(undef, nvariables(equations), nnodes(mortar_l2),
                                      nnodes(mortar_l2))
                                  for _ in 1:Threads.nthreads()]
  fstar_lower_right_threaded = A3d[A3d(undef, nvariables(equations), nnodes(mortar_l2),
                                       nnodes(mortar_l2))
                                   for _ in 1:Threads.nthreads()]
...
@inline function weak_form_kernel!(du, u,
  ...
  for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
    ...
    for ii in eachnode(dg)
      multiply_add_to_node_vars!(du, alpha * derivative_dhat[ii, i], flux1, equations, dg,
                                 ii, j, k, element)
    end

    flux2 = flux(u_node, 2, equations)
    for jj in eachnode(dg)
      multiply_add_to_node_vars!(du, alpha * derivative_dhat[jj, j], flux2, equations, dg,
                                 i, jj, k, element)
    end

    flux3 = flux(u_node, 3, equations)
    for kk in eachnode(dg)
      multiply_add_to_node_vars!(du, alpha * derivative_dhat[kk, k], flux3, equations, dg,
                                 i, j, kk, element)
    end
  end
...
@inline function flux_differencing_kernel!(du, u,
  ...
  for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
    ...
    for ii in (i + 1):nnodes(dg)
      ...
      multiply_add_to_node_vars!(du, alpha * derivative_split[i, ii], flux1, equations,
                                 dg, i, j, k, element)
      multiply_add_to_node_vars!(du, alpha * derivative_split[ii, i], flux1, equations,
                                 dg, ii, j, k, element)
    end
    ...
    for jj in (j + 1):nnodes(dg)
      ...
      multiply_add_to_node_vars!(du, alpha * derivative_split[j, jj], flux2, equations,
                                 dg, i, j, k, element)
      multiply_add_to_node_vars!(du, alpha * derivative_split[jj, j], flux2, equations,
                                 dg, i, jj, k, element)
    end
    ...
    for kk in (k + 1):nnodes(dg)
      ...
      multiply_add_to_node_vars!(du, alpha * derivative_split[k, kk], flux3, equations,
                                 dg, i, j, k, element)
      multiply_add_to_node_vars!(du, alpha * derivative_split[kk, k], flux3, equations,
                                 dg, i, j, kk, element)
    end
  end
end
...
@inline function flux_differencing_kernel!(du, u,
  ...
  flux_differencing_kernel!(du, u, element, mesh, False(), equations, symmetric_flux, dg,
                            cache, alpha)
  ...
  for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
    ...
    multiply_add_to_node_vars!(du, alpha * 0.5, integral_contribution, equations, dg, i,
                               j, k, element)
  end
end
...
@inline function calcflux_fv!(fstar1_L, fstar1_R, fstar2_L, fstar2_R, fstar3_L, fstar3_R,
                              u,
  ...
  @threaded for mortar in eachmortar(dg, cache)
    ...
    if cache.mortars.large_sides[mortar] == 1 # -> small elements on right side
      ...
      if cache.mortars.orientations[mortar] == 1
        ...
        for k in eachnode(dg), j in eachnode(dg)
          for v in eachvariable(equations)
            cache.mortars.u_upper_left[1, v, j, k, mortar] = u[v, nnodes(dg), j, k,
                                                               upper_left_element]
            cache.mortars.u_upper_right[1, v, j, k, mortar] = u[v, nnodes(dg), j, k,
                                                                upper_right_element]
            cache.mortars.u_lower_left[1, v, j, k, mortar] = u[v, nnodes(dg), j, k,
                                                               lower_left_element]
            cache.mortars.u_lower_right[1, v, j, k, mortar] = u[v, nnodes(dg), j, k,
                                                                lower_right_element]
          end
        end
      elseif cache.mortars.orientations[mortar] == 2
        ...
        for k in eachnode(dg), i in eachnode(dg)
          for v in eachvariable(equations)
            cache.mortars.u_upper_left[1, v, i, k, mortar] = u[v, i, nnodes(dg), k,
                                                               upper_left_element]
            cache.mortars.u_upper_right[1, v, i, k, mortar] = u[v, i, nnodes(dg), k,
                                                                upper_right_element]
            cache.mortars.u_lower_left[1, v, i, k, mortar] = u[v, i, nnodes(dg), k,
                                                               lower_left_element]
            cache.mortars.u_lower_right[1, v, i, k, mortar] = u[v, i, nnodes(dg), k,
                                                                lower_right_element]
          end
        end
        ...
        for j in eachnode(dg), i in eachnode(dg)
          for v in eachvariable(equations)
            cache.mortars.u_upper_left[1, v, i, j, mortar] = u[v, i, j, nnodes(dg),
                                                               upper_left_element]
            cache.mortars.u_upper_right[1, v, i, j, mortar] = u[v, i, j, nnodes(dg),
                                                                upper_right_element]
            cache.mortars.u_lower_left[1, v, i, j, mortar] = u[v, i, j, nnodes(dg),
                                                               lower_left_element]
            cache.mortars.u_lower_right[1, v, i, j, mortar] = u[v, i, j, nnodes(dg),
                                                                lower_right_element]
          end
        end
      end
...
function calc_mortar_flux!(surface_flux_values,
  ...
  @threaded for mortar in eachmortar(dg, cache)
    ...
    calc_fstar!(fstar_upper_left, equations, surface_flux, dg, u_upper_left, mortar,
                orientation)
    calc_fstar!(fstar_upper_right, equations, surface_flux, dg, u_upper_right, mortar,
                orientation)
    calc_fstar!(fstar_lower_left, equations, surface_flux, dg, u_lower_left, mortar,
                orientation)
    calc_fstar!(fstar_lower_right, equations, surface_flux, dg, u_lower_right, mortar,
                orientation)
  ...
  @threaded for mortar in eachmortar(dg, cache)
    ...
    if large_sides[mortar] == 1 # -> small elements on right side
      for j in eachnode(dg), i in eachnode(dg)
        ...
        u_upper_left_ll, u_upper_left_rr = get_surface_node_vars(u_upper_left, equations,
                                                                 dg, i, j, mortar)
        u_upper_right_ll, u_upper_right_rr = get_surface_node_vars(u_upper_right,
                                                                   equations, dg, i, j,
                                                                   mortar)
        u_lower_left_ll, u_lower_left_rr = get_surface_node_vars(u_lower_left, equations,
                                                                 dg, i, j, mortar)
        u_lower_right_ll, u_lower_right_rr = get_surface_node_vars(u_lower_right,
                                                                   equations, dg, i, j,
                                                                   mortar)
        # Call pointwise nonconservative term
        noncons_upper_left = nonconservative_flux(u_upper_left_ll, u_upper_left_rr,
                                                  orientation, equations)
        noncons_upper_right = nonconservative_flux(u_upper_right_ll, u_upper_right_rr,
                                                   orientation, equations)
        noncons_lower_left = nonconservative_flux(u_lower_left_ll, u_lower_left_rr,
                                                  orientation, equations)
        noncons_lower_right = nonconservative_flux(u_lower_right_ll, u_lower_right_rr,
                                                   orientation, equations)
        # Add to primary and secondary temporay storage
        multiply_add_to_node_vars!(fstar_upper_left, 0.5, noncons_upper_left, equations,
                                   dg, i, j)
        multiply_add_to_node_vars!(fstar_upper_right, 0.5, noncons_upper_right, equations,
                                   dg, i, j)
        multiply_add_to_node_vars!(fstar_lower_left, 0.5, noncons_lower_left, equations,
                                   dg, i, j)
        multiply_add_to_node_vars!(fstar_lower_right, 0.5, noncons_lower_right, equations,
                                   dg, i, j)
      end
      ...
      for j in eachnode(dg), i in eachnode(dg)
        ...
        u_lower_right_ll, u_lower_right_rr = get_surface_node_vars(u_lower_right,
                                                                   ...
                                                                   mortar)
        # Call pointwise nonconservative term
        noncons_upper_left = nonconservative_flux(u_upper_left_rr, u_upper_left_ll,
                                                  orientation, equations)
        noncons_upper_right = nonconservative_flux(u_upper_right_rr, u_upper_right_ll,
                                                   orientation, equations)
        noncons_lower_left = nonconservative_flux(u_lower_left_rr, u_lower_left_ll,
                                                  orientation, equations)
        noncons_lower_right = nonconservative_flux(u_lower_right_rr, u_lower_right_ll,
                                                   orientation, equations)
        # Add to primary and secondary temporay storage
        multiply_add_to_node_vars!(fstar_upper_left, 0.5, noncons_upper_left, equations,
                                   dg, i, j)
...
function calc_sources!(du, u, t, source_terms,
  ...
  @threaded for element in eachelement(dg, cache)
    for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
      ...
      x_local = get_node_coords(cache.elements.node_coordinates, equations, dg, i, j, k,
                                element)
