...
@muladd begin
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for i in eachnode(dg)
                flux_1_node = get_node_vars(
                    flux_viscous, equations_parabolic, dg, i,
                    element
                )
    ...
    function calc_viscous_fluxes!(
            flux_viscous, gradients, u_transformed, mesh::TreeMesh{1},
            equations_parabolic::AbstractEquationsParabolic,
        ...
        @threaded for element in eachelement(dg, cache)
            for i in eachnode(dg)
                ...
                gradients_1_node = get_node_vars(
                    gradients, equations_parabolic, dg, i,
                    element
                )
                ...
                set_node_vars!(
                    flux_viscous, flux_viscous_node, equations_parabolic, dg, i,
                    element
                )
            end
        end
    ...
    function calc_boundary_flux_by_direction_gradient!(
        ...
        @threaded for boundary in first_boundary:last_boundary
            ...
            flux = boundary_condition(
                flux_inner, u_inner, orientations[boundary],
                direction,
    ...
    function calc_boundary_flux_by_direction_divergence!(
        ...
        @threaded for boundary in first_boundary:last_boundary
            ...
            flux = boundary_condition(
                flux_inner, nothing, orientations[boundary],
                direction,
    ...
    function calc_gradient!(
            ...
            @threaded for element in eachelement(dg, cache)
                ...
                for i in eachnode(dg)
                    u_node = get_node_vars(
                        u_transformed, equations_parabolic, dg, i,
                        element
                    )

                    for ii in eachnode(dg)
                        multiply_add_to_node_vars!(
                            gradients, derivative_dhat[ii, i],
        ...
        @trixi_timeit timer() "Jacobian" begin
            apply_jacobian_parabolic!(
                gradients, mesh, equations_parabolic, dg,
                cache_parabolic
            )
        end
    ...
    function create_cache_parabolic(
        ...
        viscous_container = init_viscous_container_1d(
