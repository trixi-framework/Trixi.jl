...
@muladd begin
    ...
    function create_cache(
        ...
        indicator_threaded = [
            A(undef, nnodes(basis), nnodes(basis), nnodes(basis))
        ...
        return (;
            alpha, alpha_tmp, indicator_threaded, modal_threaded, modal_tmp1_threaded,
            modal_tmp2_threaded,
        )
    end
    ...
    function create_cache(
            typ::Type{IndicatorHennemannGassner}, mesh,
    ...
    @inline function calc_indicator_hennemann_gassner!(
            indicator_hg, threshold, parameter_s,
            u,
        ...
        multiply_scalar_dimensionwise!(
            modal, dg.basis.inverse_vandermonde_legendre,
    ...
    function apply_smoothing!(
            mesh::Union{TreeMesh{3}, P4estMesh{3}, T8codeMesh{3}}, alpha,
            alpha_tmp, dg,
            cache
        )
        ...
        for mortar in eachmortar(dg, cache)
            ...
            # Apply smoothing
            alpha[lower_left] = max(
                alpha_tmp[lower_left], 0.5f0 * alpha_tmp[large],
                alpha[lower_left]
            )
            alpha[lower_right] = max(
                alpha_tmp[lower_right], 0.5f0 * alpha_tmp[large],
                alpha[lower_right]
            )
            alpha[upper_left] = max(
                alpha_tmp[upper_left], 0.5f0 * alpha_tmp[large],
                alpha[upper_left]
            )
            alpha[upper_right] = max(
                alpha_tmp[upper_right], 0.5f0 * alpha_tmp[large],
                alpha[upper_right]
            )

            alpha[large] = max(
                alpha_tmp[large], 0.5f0 * alpha_tmp[lower_left],
                alpha[large]
            )
            alpha[large] = max(
                alpha_tmp[large], 0.5f0 * alpha_tmp[lower_right],
                alpha[large]
            )
            alpha[large] = max(
                alpha_tmp[large], 0.5f0 * alpha_tmp[upper_left],
                alpha[large]
            )
            alpha[large] = max(
                alpha_tmp[large], 0.5f0 * alpha_tmp[upper_right],
                alpha[large]
            )
        end
    end
    ...
    # this method is used when the indicator is constructed as for shock-capturing volume integrals
    function create_cache(
            ::Type{IndicatorLöhner}, equations::AbstractEquations{3},
    ...
    # this method is used when the indicator is constructed as for AMR
    function create_cache(
            typ::Type{IndicatorLöhner}, mesh, equations::AbstractEquations{3},
            dg::DGSEM, cache
        )
        create_cache(typ, equations, dg.basis)
    end
    ...
    # this method is used when the indicator is constructed as for shock-capturing volume integrals
    function create_cache(
            ::Type{IndicatorMax}, equations::AbstractEquations{3},
            basis::LobattoLegendreBasis
        )
        alpha = Vector{real(basis)}()
    ...
    # this method is used when the indicator is constructed as for AMR
    function create_cache(
            typ::Type{IndicatorMax}, mesh, equations::AbstractEquations{3},
            dg::DGSEM, cache
        )
