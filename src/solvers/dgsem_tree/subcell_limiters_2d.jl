...
@muladd begin
    ...
    function create_cache(
            limiter::Type{SubcellLimiterIDP}, equations::AbstractEquations{2},
        ...
        return (;
            subcell_limiter_coefficients, idp_bounds_delta_local,
            idp_bounds_delta_global,
        )
    end

    function (limiter::SubcellLimiterIDP)(
            u::AbstractArray{<:Any, 4}, semi, dg::DGSEM, t,
            dt;
            kwargs...
        )
        ...
        if limiter.local_twosided
            @trixi_timeit timer() "local twosided" idp_local_twosided!(
                alpha, limiter,
                u, t, dt, semi
            )
        end
        ...
        if limiter.local_onesided
            @trixi_timeit timer() "local onesided" idp_local_onesided!(
                alpha, limiter,
                u, t, dt, semi
            )
        end
    ...
    @inline function calc_bounds_twosided_interface!(
            var_min, var_max, variable, u, t, semi,
            mesh::TreeMesh2D
        )
        ...
        for interface in eachinterface(dg, cache)
            ...
            for i in eachnode(dg)
                ...
                var_min[index_right..., right] = min(
                    var_min[index_right..., right],
                    var_left
                )
                var_max[index_right..., right] = max(
                    var_max[index_right..., right],
                    var_left
                )
        ...
        for boundary in eachboundary(dg, cache)
            ...
            for i in eachnode(dg)
                ...
                u_outer = get_boundary_outer_state(
                    u_inner, t,
    ...
    @inline function calc_bounds_onesided!(var_minmax, min_or_max, variable, u, t, semi)
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j in eachnode(dg), i in eachnode(dg)
                ...
                if i > 1
                    var_minmax[i - 1, j, element] = min_or_max(
                        var_minmax[
                            i - 1, j,
                            element,
                        ], var
                ...
                if j > 1
                    var_minmax[i, j - 1, element] = min_or_max(
                        var_minmax[
                            i, j - 1,
                            element,
                        ], var
    ...
    @inline function calc_bounds_onesided_interface!(
            var_minmax, min_or_max, variable, u, t,
        ...
        for interface in eachinterface(dg, cache)
            ...
            for i in eachnode(dg)
                ...
                var_left = variable(
                    get_node_vars(u, equations, dg, index_left..., left),
                    equations
                )
                var_right = variable(
                    get_node_vars(u, equations, dg, index_right..., right),
                    equations
                )

                var_minmax[index_right..., right] = min_or_max(
                    var_minmax[
                        index_right...,
                        right,
                    ], var_left
                )
                var_minmax[index_left..., left] = min_or_max(
                    var_minmax[
                        index_left...,
                        left,
                    ], var_right
        ...
        for boundary in eachboundary(dg, cache)
            ...
            for i in eachnode(dg)
                ...
                var_minmax[index..., element] = min_or_max(
                    var_minmax[index..., element],
                    var_outer
                )
            end
        end
    ...
    @inline function idp_local_twosided!(alpha, limiter, u, t, dt, semi, variable)
        ...
        @threaded for element in eachelement(dg, semi.cache)
            for j in eachnode(dg), i in eachnode(dg)
                inverse_jacobian = get_inverse_jacobian(
    ...
    @inline function idp_local_onesided!(
            alpha, limiter, u, t, dt, semi,
            variable, min_or_max
        )
        mesh, equations, dg, cache = mesh_equations_solver_cache(semi)
        (; variable_bounds) = limiter.cache.subcell_limiter_coefficients
        ...
        @threaded for element in eachelement(dg, cache)
            for j in eachnode(dg), i in eachnode(dg)
                ...
                newton_loops_alpha!(
                    alpha, var_minmax[i, j, element], u_local,
    ...
    @inline function idp_positivity!(alpha, limiter, u, dt, semi)
        ...
        for variable in limiter.positivity_variables_cons
            @trixi_timeit timer() "conservative variables" idp_positivity_conservative!(
                alpha,
                limiter,
                u,
                dt,
        ...
        for variable in limiter.positivity_variables_nonlinear
            @trixi_timeit timer() "nonlinear variables" idp_positivity_nonlinear!(
                alpha,
                limiter,
                u, dt,
                semi,
    ...
    @inline function idp_positivity_nonlinear!(alpha, limiter, u, dt, semi, variable)
        ...
        @threaded for element in eachelement(dg, semi.cache)
            for j in eachnode(dg), i in eachnode(dg)
                ...
                # Perform Newton's bisection method to find new alpha
                newton_loops_alpha!(
                    alpha, var_min[i, j, element], u_local, i, j, element,
    ...
    @inline function newton_loops_alpha!(
            alpha, bound, u, i, j, element, variable,
        ...
        antidiffusive_flux = gamma_constant_newton * inverse_jacobian * inverse_weights[i] *
            get_node_vars(
            antidiffusive_flux1_R, equations, dg, i, j,
            element
        )
        newton_loop!(
            alpha, bound, u, i, j, element, variable, min_or_max, initial_check,
        ...
        antidiffusive_flux = -gamma_constant_newton * inverse_jacobian *
            inverse_weights[i] *
            get_node_vars(
            antidiffusive_flux1_L, equations, dg, i + 1, j,
            element
        )
        newton_loop!(
            alpha, bound, u, i, j, element, variable, min_or_max, initial_check,
        ...
        antidiffusive_flux = gamma_constant_newton * inverse_jacobian * inverse_weights[j] *
            get_node_vars(
            antidiffusive_flux2_R, equations, dg, i, j,
            element
        )
        newton_loop!(
            alpha, bound, u, i, j, element, variable, min_or_max, initial_check,
        ...
        antidiffusive_flux = -gamma_constant_newton * inverse_jacobian *
            inverse_weights[j] *
            get_node_vars(
            antidiffusive_flux2_L, equations, dg, i, j + 1,
            element
        )
        newton_loop!(
            alpha, bound, u, i, j, element, variable, min_or_max, initial_check,
    ...
    @inline function newton_loop!(
            alpha, bound, u, i, j, element, variable, min_or_max,
        ...
        for iter in 1:(limiter.max_iterations_newton)
            ...
            if isvalid(u_curr, equations)
                dgoal_dbeta = dgoal_function_newton_idp(
                    variable, u_curr, dt,
    ...
    # Initial checks
    @inline function initial_check_local_onesided_newton_idp(
            ::typeof(min), bound,
            goal, newton_abstol
        )
    ...
    @inline function dgoal_function_newton_idp(
            variable, u, dt, antidiffusive_flux,
            equations
        )
