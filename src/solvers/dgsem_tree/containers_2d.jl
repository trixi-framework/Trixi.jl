...
@muladd begin
    ...
    function Base.resize!(elements::ElementContainer2D, capacity)
        ...
        elements.node_coordinates = unsafe_wrap(
            Array, pointer(_node_coordinates),
        ...
        elements.surface_flux_values = unsafe_wrap(
            Array, pointer(_surface_flux_values),
    ...
    function ElementContainer2D{RealT, uEltype}(
        ...
        return ElementContainer2D{RealT, uEltype}(
            inverse_jacobian, node_coordinates,
    ...
    function init_elements(
        ...
        elements = ElementContainer2D{RealT, uEltype}(
            n_elements, nvariables(equations),
            nnodes(basis)
        )
    ...
    function Base.resize!(interfaces::InterfaceContainer2D, capacity)
        ...
        interfaces.neighbor_ids = unsafe_wrap(
    ...
    function Base.resize!(boundaries::BoundaryContainer2D, capacity)
        ...
        boundaries.node_coordinates = unsafe_wrap(
            Array, pointer(_node_coordinates),
            (2, n_nodes, capacity)
        )
    ...
    function Base.resize!(mortars::L2MortarContainer2D, capacity)
        ...
        mortars.u_upper = unsafe_wrap(
            Array, pointer(_u_upper),
            (2, n_variables, n_nodes, capacity)
        )

        resize!(_u_lower, 2 * n_variables * n_nodes * capacity)
        mortars.u_lower = unsafe_wrap(
            Array, pointer(_u_lower),
            (2, n_variables, n_nodes, capacity)
        )

        resize!(_neighbor_ids, 3 * capacity)
        mortars.neighbor_ids = unsafe_wrap(
            Array, pointer(_neighbor_ids),
            (3, capacity)
        )
    ...
    function L2MortarContainer2D{uEltype}(
        ...
        return L2MortarContainer2D{uEltype}(
            u_upper, u_lower, neighbor_ids, large_sides,
            orientations,
    ...
    function init_mortars!(mortars, elements, mesh::TreeMesh2D)
        ...
        for element in eachelement(elements)
            ...
            for direction in eachdirection(mesh.tree)
                ...
                if direction == 1
                    mortars.neighbor_ids[1, count] = c2e[
                        mesh.tree.child_ids[
                            2,
                            neighbor_cell_id,
                        ],
                    ]
                    mortars.neighbor_ids[2, count] = c2e[
                        mesh.tree.child_ids[
                            4,
                            neighbor_cell_id,
                        ],
    ...
    function MPIInterfaceContainer2D{uEltype}(
        ...
        return MPIInterfaceContainer2D{uEltype}(
            u, local_neighbor_ids, orientations,
            remote_sides,
            _u
        )
    end
    ...
    function Base.resize!(mpi_mortars::MPIL2MortarContainer2D, capacity)
        ...
        resize!(_u_upper, 2 * n_variables * n_nodes * capacity)
        mpi_mortars.u_upper = unsafe_wrap(
            Array, pointer(_u_upper),
            (2, n_variables, n_nodes, capacity)
        )

        resize!(_u_lower, 2 * n_variables * n_nodes * capacity)
        mpi_mortars.u_lower = unsafe_wrap(
            Array, pointer(_u_lower),
            (2, n_variables, n_nodes, capacity)
        )
    ...
    function MPIL2MortarContainer2D{uEltype}(
        ...
        return MPIL2MortarContainer2D{uEltype}(
            u_upper, u_lower, local_neighbor_ids,
    ...
    function ContainerAntidiffusiveFlux2D{uEltype}(
            capacity::Integer, n_variables,
            n_nodes
        ) where {uEltype <: Real}
        nan_uEltype = convert(uEltype, NaN)
        ...
        # Initialize fields with defaults
        _antidiffusive_flux1_L = fill(
            nan_uEltype,
            n_variables * (n_nodes + 1) * n_nodes * capacity
        )
        antidiffusive_flux1_L = unsafe_wrap(
            Array, pointer(_antidiffusive_flux1_L),
            (n_variables, n_nodes + 1, n_nodes, capacity)
        )
        _antidiffusive_flux1_R = fill(
            nan_uEltype,
            n_variables * (n_nodes + 1) * n_nodes * capacity
        )
        antidiffusive_flux1_R = unsafe_wrap(
            Array, pointer(_antidiffusive_flux1_R),
            (n_variables, n_nodes + 1, n_nodes, capacity)
        )

        _antidiffusive_flux2_L = fill(
            nan_uEltype,
            n_variables * n_nodes * (n_nodes + 1) * capacity
        )
        antidiffusive_flux2_L = unsafe_wrap(
            Array, pointer(_antidiffusive_flux2_L),
            (n_variables, n_nodes, n_nodes + 1, capacity)
        )
        _antidiffusive_flux2_R = fill(
            nan_uEltype,
            n_variables * n_nodes * (n_nodes + 1) * capacity
        )
        antidiffusive_flux2_R = unsafe_wrap(
            Array, pointer(_antidiffusive_flux2_R),
            (n_variables, n_nodes, n_nodes + 1, capacity)
        )

        return ContainerAntidiffusiveFlux2D{uEltype}(
            antidiffusive_flux1_L,
    ...
    function Base.resize!(fluxes::ContainerAntidiffusiveFlux2D, capacity)
        ...
        fluxes.antidiffusive_flux1_L = unsafe_wrap(
            Array, pointer(_antidiffusive_flux1_L),
            (
        ...
        fluxes.antidiffusive_flux1_R = unsafe_wrap(
            Array, pointer(_antidiffusive_flux1_R),
            (
                n_variables, n_nodes + 1, n_nodes,
                capacity,
        ...
        fluxes.antidiffusive_flux2_L = unsafe_wrap(
            Array, pointer(_antidiffusive_flux2_L),
            (
        ...
        fluxes.antidiffusive_flux2_R = unsafe_wrap(
            Array, pointer(_antidiffusive_flux2_R),
            (
                n_variables, n_nodes, n_nodes + 1,
                capacity,
    ...
    function ContainerSubcellLimiterIDP2D{uEltype}(
            capacity::Integer, n_nodes,
            bound_keys
        ) where {uEltype <: Real}
        nan_uEltype = convert(uEltype, NaN)
        ...
        for key in bound_keys
            ...
            variable_bounds[key] = unsafe_wrap(
                Array, pointer(_variable_bounds[key]),
                (n_nodes, n_nodes, capacity)
            )
        end

        return ContainerSubcellLimiterIDP2D{uEltype}(
            alpha, alpha1, alpha2,
            variable_bounds,
    ...
    function Base.resize!(container::ContainerSubcellLimiterIDP2D, capacity)
        ...
        container.alpha1 = unsafe_wrap(
            Array, pointer(_alpha1),
            (n_nodes + 1, n_nodes, capacity)
        )
        resize!(_alpha2, n_nodes * (n_nodes + 1) * capacity)
        container.alpha2 = unsafe_wrap(
            Array, pointer(_alpha2),
        ...
        for (key, _) in _variable_bounds
            ...
            container.variable_bounds[key] = unsafe_wrap(
                Array,
