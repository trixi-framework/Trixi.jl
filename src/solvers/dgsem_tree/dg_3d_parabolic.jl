...
function rhs_parabolic!(du, u, t, mesh::TreeMesh{3},
  ...
  @trixi_timeit timer() "transform variables" transform_variables!(u_transformed, u, mesh,
                                                                   equations_parabolic,
                                                                   dg, parabolic_scheme,
                                                                   cache, cache_parabolic)

  # Compute the gradients of the transformed variables
  @trixi_timeit timer() "calculate gradient" calc_gradient!(gradients, u_transformed, t,
  ...
  @trixi_timeit timer() "volume integral" calc_volume_integral!(du, flux_viscous, mesh,
  ...
  @trixi_timeit timer() "boundary flux" calc_boundary_flux_divergence!(cache_parabolic, t,
                                                                       boundary_conditions_parabolic,
                                                                       mesh,
                                                                       equations_parabolic,
                                                                       dg.surface_integral,
  ...
  @trixi_timeit timer() "Jacobian" apply_jacobian!(du, mesh, equations_parabolic, dg,
                                                   cache_parabolic)
  ...
  @threaded for element in eachelement(dg, cache)
    ...
    for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
      ...
      u_transformed_node = gradient_variable_transformation(equations_parabolic)(u_node,
                                                                                 equations_parabolic)
      set_node_vars!(u_transformed, u_transformed_node, equations_parabolic, dg, i, j, k,
                     element)
    end
  end
end
...
function calc_volume_integral!(du, flux_viscous,
  ...
  @threaded for element in eachelement(dg, cache)
    # Calculate volume terms in one element
    for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
      flux_1_node = get_node_vars(flux_viscous_x, equations_parabolic, dg, i, j, k,
                                  element)
      flux_2_node = get_node_vars(flux_viscous_y, equations_parabolic, dg, i, j, k,
                                  element)
      flux_3_node = get_node_vars(flux_viscous_z, equations_parabolic, dg, i, j, k,
                                  element)
  ...
  @threaded for interface in eachinterface(dg, cache)
    ...
    if orientations[interface] == 1
      ...
      for k in eachnode(dg), j in eachnode(dg), v in eachvariable(equations_parabolic)
        ...
        interfaces.u[1, v, j, k, interface] = flux_viscous_x[v, nnodes(dg), j, k,
                                                             left_element]
    ...
    elseif orientations[interface] == 2
      ...
      for k in eachnode(dg), i in eachnode(dg), v in eachvariable(equations_parabolic)
        # OBS! `interfaces.u` stores the interpolated *fluxes* and *not the solution*!
        interfaces.u[1, v, i, k, interface] = flux_viscous_y[v, i, nnodes(dg), k,
                                                             left_element]
      ...
      for j in eachnode(dg), i in eachnode(dg), v in eachvariable(equations_parabolic)
        # OBS! `interfaces.u` stores the interpolated *fluxes* and *not the solution*!
        interfaces.u[1, v, i, j, interface] = flux_viscous_z[v, i, j, nnodes(dg),
                                                             left_element]
  ...
  @threaded for boundary in eachboundary(dg, cache_parabolic)
    ...
    if orientations[boundary] == 1
      ...
      if neighbor_sides[boundary] == 1
        ...
        for k in eachnode(dg), j in eachnode(dg), v in eachvariable(equations_parabolic)
          ...
          boundaries.u[1, v, j, k, boundary] = flux_viscous_x[v, nnodes(dg), j, k,
                                                              element]
        end
    ...
    elseif orientations[boundary] == 2
      ...
      if neighbor_sides[boundary] == 1
        ...
        for k in eachnode(dg), i in eachnode(dg), v in eachvariable(equations_parabolic)
          # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
          boundaries.u[1, v, i, k, boundary] = flux_viscous_y[v, i, nnodes(dg), k,
                                                              element]
        end
      else
        # element in +y direction of boundary
      ...
      if neighbor_sides[boundary] == 1
        ...
        for j in eachnode(dg), i in eachnode(dg), v in eachvariable(equations_parabolic)
          # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
          boundaries.u[1, v, i, j, boundary] = flux_viscous_z[v, i, j, nnodes(dg),
                                                              element]
        end
      else
        # element in +z direction of boundary
...
function calc_viscous_fluxes!(flux_viscous, gradients, u_transformed, mesh::TreeMesh{3},
  ...
  @threaded for element in eachelement(dg, cache)
    for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
      ...
      gradients_1_node = get_node_vars(gradients_x, equations_parabolic, dg, i, j, k,
                                       element)
      gradients_2_node = get_node_vars(gradients_y, equations_parabolic, dg, i, j, k,
                                       element)
      gradients_3_node = get_node_vars(gradients_z, equations_parabolic, dg, i, j, k,
                                       element)
      ...
      set_node_vars!(flux_viscous_x, flux_viscous_node_x, equations_parabolic, dg, i, j,
                     k, element)
      set_node_vars!(flux_viscous_y, flux_viscous_node_y, equations_parabolic, dg, i, j,
                     k, element)
      set_node_vars!(flux_viscous_z, flux_viscous_node_z, equations_parabolic, dg, i, j,
                     k, element)
    end
  end
end
  ...
  @trixi_timeit timer() "volume integral" begin
    ...
    @threaded for element in eachelement(dg, cache)
      ...
      for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
        ...
        for kk in eachnode(dg)
          multiply_add_to_node_vars!(gradients_z, derivative_dhat[kk, k], u_node,
                                     equations_parabolic, dg, i, j, kk, element)
        end
      end
    end
  ...
  @trixi_timeit timer() "surface integral" begin
    ...
    @threaded for element in eachelement(dg, cache)
      for m in eachnode(dg), l in eachnode(dg)
        for v in eachvariable(equations_parabolic)
          # surface at -x
          gradients_x[v, 1, l, m, element] = (gradients_x[v, 1, l, m, element] -
                                              surface_flux_values[v, l, m, 1, element] *
                                              factor_1)

          # surface at +x
          gradients_x[v, nnodes(dg), l, m, element] = (gradients_x[v, nnodes(dg), l, m,
                                                                   element] +
          ...
          # surface at -y
          gradients_y[v, l, 1, m, element] = (gradients_y[v, l, 1, m, element] -
                                              surface_flux_values[v, l, m, 3, element] *
                                              factor_1)

          # surface at +y
          gradients_y[v, l, nnodes(dg), m, element] = (gradients_y[v, l, nnodes(dg), m,
                                                                   element] +
          ...
          # surface at -z
          gradients_z[v, l, m, 1, element] = (gradients_z[v, l, m, 1, element] -
                                              surface_flux_values[v, l, m, 5, element] *
                                              factor_1)

          # surface at +z
          gradients_z[v, l, m, nnodes(dg), element] = (gradients_z[v, l, m, nnodes(dg),
                                                                   element] +
