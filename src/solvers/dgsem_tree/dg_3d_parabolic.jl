...
@muladd begin
    ...
    function transform_variables!(
            u_transformed, u, mesh::Union{TreeMesh{3}, P4estMesh{3}},
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                ...
                set_node_vars!(
                    u_transformed, u_transformed_node, equations_parabolic, dg,
                    i, j, k, element
                )
            end
        end
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            # Calculate volume terms in one element
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                flux_1_node = get_node_vars(
                    flux_viscous_x, equations_parabolic, dg, i, j,
                    k, element
                )
                flux_2_node = get_node_vars(
                    flux_viscous_y, equations_parabolic, dg, i, j,
                    k, element
                )
                flux_3_node = get_node_vars(
                    flux_viscous_z, equations_parabolic, dg, i, j,
                    k, element
                )

                for ii in eachnode(dg)
                    multiply_add_to_node_vars!(
                        du, derivative_dhat[ii, i], flux_1_node,
                ...
                for jj in eachnode(dg)
                    multiply_add_to_node_vars!(
                        du, derivative_dhat[jj, j], flux_2_node,
                ...
                for kk in eachnode(dg)
                    multiply_add_to_node_vars!(
                        du, derivative_dhat[kk, k], flux_3_node,
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            if orientations[interface] == 1
                ...
                for k in eachnode(dg), j in eachnode(dg),
                    ...
                    interfaces_u[1, v, j, k, interface] = flux_viscous_x[
                        v, nnodes(dg), j,
                        k, left_element,
                    ]
                    interfaces_u[2, v, j, k, interface] = flux_viscous_x[
                        v, 1, j, k,
                        right_element,
                    ]
                end
            elseif orientations[interface] == 2
                ...
                for k in eachnode(dg), i in eachnode(dg),
                        v in eachvariable(equations_parabolic)
                    # OBS! `interfaces_u` stores the interpolated *fluxes* and *not the solution*!
                    interfaces_u[1, v, i, k, interface] = flux_viscous_y[
                        v, i, nnodes(dg),
                        k, left_element,
                    ]
                    interfaces_u[2, v, i, k, interface] = flux_viscous_y[
                        v, i, 1, k,
                        right_element,
                    ]
                end
                ...
                for j in eachnode(dg), i in eachnode(dg),
                        v in eachvariable(equations_parabolic)
                    # OBS! `interfaces_u` stores the interpolated *fluxes* and *not the solution*!
                    interfaces_u[1, v, i, j, interface] = flux_viscous_z[
                        v, i, j,
                        nnodes(dg),
                        left_element,
                    ]
                    interfaces_u[2, v, i, j, interface] = flux_viscous_z[
                        v, i, j, 1,
                        right_element,
                    ]
                end
            ...
            if orientations[boundary] == 1
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for k in eachnode(dg), j in eachnode(dg),
                        ...
                        boundaries_u[1, v, j, k, boundary] = flux_viscous_x[
                            v, nnodes(dg),
                            j, k, element,
                        ]
                    end
                    ...
                    for k in eachnode(dg), j in eachnode(dg),
                            v in eachvariable(equations_parabolic)
                        # OBS! `boundaries_u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries_u[2, v, j, k, boundary] = flux_viscous_x[
                            v, 1, j, k,
                            element,
                        ]
                    end
            ...
            elseif orientations[boundary] == 2
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for k in eachnode(dg), i in eachnode(dg),
                            v in eachvariable(equations_parabolic)
                        # OBS! `boundaries_u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries_u[1, v, i, k, boundary] = flux_viscous_y[
                            v, i,
                            nnodes(dg), k,
                            element,
                        ]
                    ...
                    for k in eachnode(dg), i in eachnode(dg),
                            v in eachvariable(equations_parabolic)
                        # OBS! `boundaries_u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries_u[2, v, i, k, boundary] = flux_viscous_y[
                            v, i, 1, k,
                            element,
                        ]
                    end
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for j in eachnode(dg), i in eachnode(dg),
                            v in eachvariable(equations_parabolic)
                        # OBS! `boundaries_u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries_u[1, v, i, j, boundary] = flux_viscous_z[
                            v, i, j,
                            nnodes(dg),
                            element,
                        ]
                    ...
                    for j in eachnode(dg), i in eachnode(dg),
                            v in eachvariable(equations_parabolic)
                        # OBS! `boundaries_u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries_u[2, v, i, j, boundary] = flux_viscous_z[
                            v, i, j, 1,
                            element,
                        ]
                    end
        ...
        @threaded for element in eachelement(dg, cache)
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                ...
                u_node = get_node_vars(
                    u_transformed, equations_parabolic, dg, i, j, k,
                    element
                )
                gradients_1_node = get_node_vars(
                    gradients_x, equations_parabolic, dg, i, j,
                    k, element
                )
                gradients_2_node = get_node_vars(
                    gradients_y, equations_parabolic, dg, i, j,
                    k, element
                )
                gradients_3_node = get_node_vars(
                    gradients_z, equations_parabolic, dg, i, j,
                    k, element
                )
                ...
                flux_viscous_node_z = flux(
                    ...
                    (
                        gradients_1_node, gradients_2_node,
                        gradients_3_node,
                    ), 3, equations_parabolic
                )
                set_node_vars!(
                    flux_viscous_x, flux_viscous_node_x, equations_parabolic, dg,
                    i, j, k, element
                )
                set_node_vars!(
                    flux_viscous_y, flux_viscous_node_y, equations_parabolic, dg,
                    i, j, k, element
                )
                set_node_vars!(
                    flux_viscous_z, flux_viscous_node_z, equations_parabolic, dg,
                    i, j, k, element
                )
            end
        end
    ...
    function calc_boundary_flux_by_direction_gradient!(
            surface_flux_values::AbstractArray{
                <:Any,
                5,
            },
            t,
        ...
        @threaded for boundary in first_boundary:last_boundary
            ...
            for j in eachnode(dg), i in eachnode(dg)
                ...
                x = get_node_coords(
                    node_coordinates, equations_parabolic, dg, i, j,
                    boundary
                )
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            if cache.mortars.large_sides[mortar] == 1 # -> small elements on right side
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for k in eachnode(dg), j in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper_left[2, v, j, k, mortar] = flux_viscous_x[
                                v,
                                1,
                                j,
                                k,
                            ...
                            cache.mortars.u_upper_right[2, v, j, k, mortar] = flux_viscous_x[
                                v,
                                1,
                                j,
                                k,
                            ...
                            cache.mortars.u_lower_left[2, v, j, k, mortar] = flux_viscous_x[
                                v,
                                1,
                                j,
                                k,
                            ...
                            cache.mortars.u_lower_right[2, v, j, k, mortar] = flux_viscous_x[
                                v,
                                1,
                                j,
                                k,
                ...
                elseif cache.mortars.orientations[mortar] == 2
                    ...
                    for k in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper_left[2, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                1,
                                k,
                            ...
                            cache.mortars.u_upper_right[2, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                1,
                                k,
                            ...
                            cache.mortars.u_lower_left[2, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                1,
                                k,
                            ...
                            cache.mortars.u_lower_right[2, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                1,
                                k,
                    ...
                    for j in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper_left[2, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                1,
                            ...
                            cache.mortars.u_upper_right[2, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                1,
                            ...
                            cache.mortars.u_lower_left[2, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                1,
                            ...
                            cache.mortars.u_lower_right[2, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                1,
                ...
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for k in eachnode(dg), j in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper_left[1, v, j, k, mortar] = flux_viscous_x[
                                v,
                                nnodes(dg),
                                j,
                                k,
                            ...
                            cache.mortars.u_upper_right[1, v, j, k, mortar] = flux_viscous_x[
                                v,
                                nnodes(dg),
                                j,
                                k,
                            ...
                            cache.mortars.u_lower_left[1, v, j, k, mortar] = flux_viscous_x[
                                v,
                                nnodes(dg),
                                j,
                                k,
                            ...
                            cache.mortars.u_lower_right[1, v, j, k, mortar] = flux_viscous_x[
                                v,
                                nnodes(dg),
                                j,
                                k,
                ...
                elseif cache.mortars.orientations[mortar] == 2
                    ...
                    for k in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper_left[1, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                nnodes(dg),
                                k,
                            ...
                            cache.mortars.u_upper_right[1, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                nnodes(dg),
                                k,
                            ...
                            cache.mortars.u_lower_left[1, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                nnodes(dg),
                                k,
                            ...
                            cache.mortars.u_lower_right[1, v, i, k, mortar] = flux_viscous_y[
                                v,
                                i,
                                nnodes(dg),
                                k,
                    ...
                    for j in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper_left[1, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                nnodes(dg),
                            ...
                            cache.mortars.u_upper_right[1, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                nnodes(dg),
                            ...
                            cache.mortars.u_lower_left[1, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                nnodes(dg),
                            ...
                            cache.mortars.u_lower_right[1, v, i, j, mortar] = flux_viscous_z[
                                v,
                                i,
                                j,
                                nnodes(dg),
            ...
            if cache.mortars.large_sides[mortar] == 1 # -> large element on left side
                ...
                if cache.mortars.orientations[mortar] == 1
                    ...
                    element_solutions_to_mortars!(
                        cache.mortars, mortar_l2, leftright,
    ...
    function calc_mortar_flux!(
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            calc_fstar!(
                fstar_upper_left, equations_parabolic, surface_flux, dg,
                u_upper_left, mortar,
                orientation
            )
            calc_fstar!(
                fstar_upper_right, equations_parabolic, surface_flux, dg,
                u_upper_right,
                mortar, orientation
            )
            calc_fstar!(
                fstar_lower_left, equations_parabolic, surface_flux, dg,
                u_lower_left, mortar,
                orientation
            )
            calc_fstar!(
                fstar_lower_right, equations_parabolic, surface_flux, dg,
                u_lower_right,
                mortar, orientation
            )
    ...
    @inline function calc_fstar!(
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            u_ll, u_rr = get_surface_node_vars(
                u_interfaces, equations_parabolic, dg, i, j,
                interface
            )
            # TODO: parabolic; only BR1 at the moment
    ...
    function calc_gradient!(
        ...
        @trixi_timeit timer() "volume integral" begin
            ...
            @threaded for element in eachelement(dg, cache)
                ...
                for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                    ...
                    for ii in eachnode(dg)
                        multiply_add_to_node_vars!(
                            gradients_x, derivative_dhat[ii, i],
                    ...
                    for jj in eachnode(dg)
                        multiply_add_to_node_vars!(
                            gradients_y, derivative_dhat[jj, j],
                    ...
                    for kk in eachnode(dg)
                        multiply_add_to_node_vars!(
                            gradients_z, derivative_dhat[kk, k],
        ...
        @trixi_timeit timer() "Jacobian" begin
            apply_jacobian_parabolic!(
                gradients_x, mesh, equations_parabolic, dg,
                cache_parabolic
            )
            apply_jacobian_parabolic!(
                gradients_y, mesh, equations_parabolic, dg,
                cache_parabolic
            )
            apply_jacobian_parabolic!(
                gradients_z, mesh, equations_parabolic, dg,
                cache_parabolic
            )
        end
    ...
    function create_cache_parabolic(
        ...
        elements = init_elements(
            leaf_cell_ids, mesh, equations_hyperbolic, dg.basis, RealT,
            uEltype
        )
        ...
        viscous_container = init_viscous_container_3d(
