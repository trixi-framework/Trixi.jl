...
@muladd begin
    ...
    function create_cache(
            mesh::Union{
                TreeMesh{2}, StructuredMesh{2}, StructuredMeshView{2},
                UnstructuredMesh2D,
    ...
    function create_cache(
            mesh::Union{
                TreeMesh{2}, StructuredMesh{2}, UnstructuredMesh2D,
                P4estMesh{2}, T8codeMesh{2},
            }, equations,
        ...
        return (;
            cache..., element_ids_dg, element_ids_dgfv,
    ...
    function create_cache(
        ...
        MA2d = MArray{
            Tuple{nvariables(equations), nnodes(mortar_l2)}, uEltype, 2,
...
=#
    ...
    @inline function fv_kernel!(
        ...
        calcflux_fv!(
            fstar1_L, fstar1_R, fstar2_L, fstar2_R, u, mesh,
            ...
            if cache.mortars.large_sides[mortar] == 1 # -> small elements on right side
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for l in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper[2, v, l, mortar] = u[
                                v, 1, l,
                                upper_element,
                            ]
                            cache.mortars.u_lower[2, v, l, mortar] = u[
                                v, 1, l,
                                lower_element,
                            ]
                        end
                ...
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for l in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper[1, v, l, mortar] = u[
                                v, nnodes(dg), l,
                                upper_element,
                            ]
                            cache.mortars.u_lower[1, v, l, mortar] = u[
                                v, nnodes(dg), l,
                                lower_element,
                            ]
                        end
    ...
    function calc_mortar_flux!(
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            calc_fstar!(
                fstar_upper, equations, surface_flux, dg, u_upper, mortar,
                orientation
            )
            calc_fstar!(
                fstar_lower, equations, surface_flux, dg, u_lower, mortar,
                orientation
            )
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            if large_sides[mortar] == 1 # -> small elements on right side
                for i in eachnode(dg)
                    ...
                    u_upper_ll, u_upper_rr = get_surface_node_vars(
                        u_upper, equations, dg,
                        i, mortar
                    )
                    u_lower_ll, u_lower_rr = get_surface_node_vars(
                        u_lower, equations, dg,
                        i, mortar
                    )
                    # Call pointwise nonconservative term
                    noncons_upper = nonconservative_flux(
                    ...
                    multiply_add_to_node_vars!(
                        fstar_upper, 0.5f0, noncons_upper, equations,
                        dg, i
                    )
                    multiply_add_to_node_vars!(
                        fstar_lower, 0.5f0, noncons_lower, equations,
                        dg, i
                    )
                end
                ...
                for i in eachnode(dg)
                    ...
                    # Call pointwise nonconservative term
                    noncons_upper = nonconservative_flux(
                        u_upper_rr, u_upper_ll,
                        orientation, equations
                    )
                    noncons_lower = nonconservative_flux(
                        u_lower_rr, u_lower_ll,
                        orientation, equations
                    )
                    # Add to primary and secondary temporary storage
                    multiply_add_to_node_vars!(
    ...
    @inline function calc_fstar!(
            destination::AbstractArray{<:Any, 2}, equations,
