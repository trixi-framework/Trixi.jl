...
@muladd begin
            ...
            for (index, mortar) in enumerate(mpi_cache.mpi_neighbor_mortars[rank])
                ...
                for position in cache.mpi_mortars.local_neighbor_positions[mortar]
                    ...
                    elseif position == 2 # upper element
                        ...
                        @views send_buffer[first_lower:last_lower] .= vec(
                            cache.mpi_mortars.u_lower[
                                leftright,
                                :,
                                :,
                        ...
                        @views send_buffer[first_upper:last_upper] .= vec(
                            cache.mpi_mortars.u_upper[
                                leftright,
                                :,
                                :,
    ...
    function create_cache(
        ...
        mpi_cache = init_mpi_cache(
            mesh, elements, mpi_interfaces, mpi_mortars,
    ...
    function init_mpi_cache(
            mesh, elements, mpi_interfaces, mpi_mortars, nvars, nnodes,
            uEltype
        )
        mpi_cache = MPICache(uEltype)

        init_mpi_cache!(
            mpi_cache, mesh, elements, mpi_interfaces, mpi_mortars, nvars,
            nnodes, uEltype
        )
        return mpi_cache
    end
    ...
    function init_mpi_neighbor_connectivity(
            elements, mpi_interfaces, mpi_mortars,
            mesh::TreeMesh2D
        )
        tree = mesh.tree
        ...
        for mortar in 1:nmpimortars(mpi_mortars)
            ...
            if 3 in local_neighbor_positions # large element is on this rank
                large_element_id = local_neighbor_ids[
                    findfirst(
                        pos -> pos == 3,
            ...
            neighbor_cell_id = tree.neighbor_ids[
    ...
    function rhs!(
            du, u, t,
            mesh::Union{
                ParallelTreeMesh{2}, ParallelP4estMesh{2},
        ...
        @threaded for interface in eachmpiinterface(dg, cache)
            ...
            if mpi_interfaces.orientations[interface] == 1 # interface in x-direction
                if mpi_interfaces.remote_sides[interface] == 1 # local element in positive direction
                    ...
                    for j in eachnode(dg), v in eachvariable(equations)
                        mpi_interfaces.u[1, v, j, interface] = u[
                            v, nnodes(dg), j,
                            local_element,
                        ]
                    end
                ...
                if mpi_interfaces.remote_sides[interface] == 1 # local element in positive direction
                    ...
                    for i in eachnode(dg), v in eachvariable(equations)
                        mpi_interfaces.u[1, v, i, interface] = u[
                            v, i, nnodes(dg),
                            local_element,
                        ]
                    end
                    ...
                    if mpi_mortars.large_sides[mortar] == 1 # -> small elements on right side
                            ...
                            if position == 1
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_lower[2, v, l, mortar] = u[
                                            v, 1, l,
                                            element,
                                        ]
                                    end
                                ...
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_upper[2, v, l, mortar] = u[
                                            v, 1, l,
                                            element,
                                        ]
                                    end
                        ...
                        if mpi_mortars.orientations[mortar] == 1
                            ...
                            if position == 1
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_lower[1, v, l, mortar] = u[
                                            v,
                                            nnodes(dg),
                                            l, element,
                                        ]
                                ...
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_upper[1, v, l, mortar] = u[
                                            v,
                                            nnodes(dg),
                                            l, element,
                                        ]
                    ...
                    if mpi_mortars.large_sides[mortar] == 1 # -> large element on left side
                        ...
                        if mpi_mortars.orientations[mortar] == 1
                            ...
                            element_solutions_to_mortars!(
                                mpi_mortars, mortar_l2, leftright,
                                mortar, u_large
                            )
                        else
                            # L2 mortars in y-direction
