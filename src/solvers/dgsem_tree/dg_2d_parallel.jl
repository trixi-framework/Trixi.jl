...
@muladd begin
  ...
  function init_mpi_neighbor_connectivity(elements, mpi_interfaces, mpi_mortars,
    ...
    mpi_neighbor_ranks = vcat(neighbor_ranks_interface, neighbor_ranks_mortar...) |> sort |>
                         unique
  ...
  function init_mpi_data_structures(mpi_neighbor_interfaces, mpi_neighbor_mortars, ndims,
                                    nvars, n_nodes)
    ...
    for index in 1:length(mpi_neighbor_interfaces)
      mpi_send_buffers[index] = Vector{Float64}(undef,
                                                length(mpi_neighbor_interfaces[index]) *
                                                data_size +
                                                length(mpi_neighbor_mortars[index]) * 4 *
                                                data_size)
      mpi_recv_buffers[index] = Vector{Float64}(undef,
                                                length(mpi_neighbor_interfaces[index]) *
                                                data_size +
                                                length(mpi_neighbor_mortars[index]) * 4 *
                                                data_size)
