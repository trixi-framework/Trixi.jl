...
@muladd begin
    ...
    function create_cache(
            typ::Type{LimiterType},
            semi
        ) where {LimiterType <: AbstractSubcellLimiter}
    ...
    ***Conservative variables*** to be limited are passed as a vector of strings, e.g.
    ...
    """
    struct SubcellLimiterIDP{
            RealT <: Real, LimitingVariablesNonlinear,
    ...
    function SubcellLimiterIDP(
            equations::AbstractEquations, basis;
            local_twosided_variables_cons = String[],
            positivity_variables_cons = String[],
            positivity_variables_nonlinear = [],
            positivity_correction_factor = 0.1,
        ...
        positivity = (
            length(positivity_variables_cons) +
        ...
        local_twosided_variables_cons_ = get_variable_index.(
            local_twosided_variables_cons,
            equations
        )
        positivity_variables_cons_ = get_variable_index.(
        ...
        if local_twosided
            for v in local_twosided_variables_cons_
                v_string = string(v)
                bound_keys = (
                    bound_keys..., Symbol(v_string, "_min"),
        ...
        SubcellLimiterIDP{
            typeof(positivity_correction_factor),
            typeof(positivity_variables_nonlinear),
            typeof(local_onesided_variables_nonlinear_),
            typeof(cache),
        }(
            local_twosided, local_twosided_variables_cons_,
    ...
    function get_node_variables!(
            node_variables, limiter::SubcellLimiterIDP,
