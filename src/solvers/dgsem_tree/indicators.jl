...
function IndicatorNeuralNetwork(equations::AbstractEquations, basis;
  ...
  IndicatorNeuralNetwork{IndicatorType,typeof(alpha_max),typeof(variable),typeof(network),
                         typeof(cache)}(indicator_type, alpha_max, alpha_min,
                                        alpha_smooth, alpha_continuous, alpha_amr,
                                        variable,
                                        network, cache)
end
...
function Base.show(io::IO, ::MIME"text/plain", indicator::IndicatorNeuralNetwork)
  ...
  if get(io, :compact, false)
    ...
    setup = ["indicator type" => indicator.indicator_type,
             "max. α" => indicator.alpha_max,
             "min. α" => indicator.alpha_min,
             "smooth α" => (indicator.alpha_smooth ? "yes" : "no"),
