...
@muladd begin
    ...
    function rhs_parabolic!(
            du, u, t, mesh::Union{TreeMesh{2}, TreeMesh{3}},
    ...
    function transform_variables!(
            u_transformed, u, mesh::Union{TreeMesh{2}, P4estMesh{2}},
            equations_parabolic::AbstractEquationsParabolic,
            dg::DG, parabolic_scheme, cache, cache_parabolic
        )
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            if orientations[interface] == 1
                ...
                for j in eachnode(dg), v in eachvariable(equations_parabolic)
                    ...
                    interfaces_u[1, v, j, interface] = flux_viscous_x[
                        v, nnodes(dg), j,
                        left_element,
                    ]
                    interfaces_u[2, v, j, interface] = flux_viscous_x[
                        v, 1, j,
                        right_element,
                    ]
                end
                ...
                for i in eachnode(dg), v in eachvariable(equations_parabolic)
                    # OBS! `interfaces_u` stores the interpolated *fluxes* and *not the solution*!
                    interfaces_u[1, v, i, interface] = flux_viscous_y[
                        v, i, nnodes(dg),
                        left_element,
                    ]
                    interfaces_u[2, v, i, interface] = flux_viscous_y[
                        v, i, 1,
                        right_element,
                    ]
                end
            ...
            if orientations[boundary] == 1
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for l in eachnode(dg), v in eachvariable(equations_parabolic)
                        ...
                        boundaries_u[1, v, l, boundary] = flux_viscous_x[
                            v, nnodes(dg), l,
                            element,
                        ]
                    end
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for l in eachnode(dg), v in eachvariable(equations_parabolic)
                        # OBS! `boundaries_u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries_u[1, v, l, boundary] = flux_viscous_y[
                            v, l, nnodes(dg),
                            element,
                        ]
                    end
        ...
        @threaded for element in eachelement(dg, cache)
            for j in eachnode(dg), i in eachnode(dg)
                ...
                flux_viscous_node_x = flux(
                    u_node, (gradients_1_node, gradients_2_node), 1,
                    equations_parabolic
                )
                flux_viscous_node_y = flux(
                    u_node, (gradients_1_node, gradients_2_node), 2,
                    equations_parabolic
                )
                set_node_vars!(
            ...
            if cache.mortars.large_sides[mortar] == 1 # -> small elements on right side
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for l in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper[2, v, l, mortar] = flux_viscous_x[
                                v, 1, l,
                                upper_element,
                            ]
                            cache.mortars.u_lower[2, v, l, mortar] = flux_viscous_x[
                                v, 1, l,
                                lower_element,
                            ]
                        end
                    ...
                    for l in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper[2, v, l, mortar] = flux_viscous_y[
                                v, l, 1,
                                upper_element,
                            ]
                            cache.mortars.u_lower[2, v, l, mortar] = flux_viscous_y[
                                v, l, 1,
                                lower_element,
                            ]
                        end
                ...
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for l in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper[1, v, l, mortar] = flux_viscous_x[
                                v,
                                nnodes(dg),
                                l,
                                upper_element,
                            ]
                            cache.mortars.u_lower[1, v, l, mortar] = flux_viscous_x[
                                v,
                                nnodes(dg),
                                l,
                                lower_element,
                    ...
                    for l in eachnode(dg)
                        for v in eachvariable(equations_parabolic)
                            cache.mortars.u_upper[1, v, l, mortar] = flux_viscous_y[
                                v, l,
                                nnodes(dg),
                                upper_element,
                            ]
                            cache.mortars.u_lower[1, v, l, mortar] = flux_viscous_y[
                                v, l,
                                nnodes(dg),
                                lower_element,
                            ]
    ...
    function calc_mortar_flux!(
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            calc_fstar!(
                fstar_upper, equations_parabolic, surface_flux, dg, u_upper, mortar,
                orientation
            )
            calc_fstar!(
                fstar_lower, equations_parabolic, surface_flux, dg, u_lower, mortar,
                orientation
            )
    ...
    @inline function calc_fstar!(
        ...
        for i in eachnode(dg)
            ...
            u_ll, u_rr = get_surface_node_vars(
                u_interfaces, equations_parabolic, dg, i,
                interface
            )
            # TODO: parabolic; only BR1 at the moment
