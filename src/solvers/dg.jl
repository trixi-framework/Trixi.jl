...
@muladd begin
    ...
    function get_element_variables!(
            element_variables, u, mesh, equations,
            volume_integral::AbstractVolumeIntegral, dg, cache
        )
        nothing
    end

    function get_node_variables!(
            node_variables, mesh, equations,
            volume_integral::AbstractVolumeIntegral, dg, cache
        )
        nothing
    end
    ...
    function get_element_variables!(
        ...
        get_element_variables!(
            element_variables, volume_integral.indicator,
            volume_integral
        )
    end
    ...
    function VolumeIntegralSubcellLimiting(
            limiter; volume_flux_dg,
            volume_flux_fv
        )
    ...
    function get_node_variables!(
        ...
        get_node_variables!(
            node_variables, volume_integral.limiter, volume_integral,
            equations
        )
    end
    ...
    function get_element_variables!(element_variables, u, mesh, equations, dg::DG, cache)
        get_element_variables!(
            element_variables, u, mesh, equations, dg.volume_integral,
            dg, cache
        )
    end
    ...
    const MeshesDGSEM = Union{
        TreeMesh, StructuredMesh, StructuredMeshView,
        UnstructuredMesh2D,
    ...
    @inline function multiply_add_to_node_vars!(
            u, factor, u_node, equations, solver::DG,
            indices...
        )
        for v in eachvariable(equations)
    ...
    @inline function wrap_array(
            u_ode::AbstractVector, mesh::AbstractMesh, equations,
            dg::DGSEM, cache
        )
        @boundscheck begin
            @assert length(u_ode) ==
        ...
        if _PREFERENCE_POLYESTER && LoopVectorization.check_args(u_ode)
            ...
            unsafe_wrap(
                Array{eltype(u_ode), ndims(mesh) + 2}, pointer(u_ode),
                (
    ...
    function compute_coefficients!(
            u, func, t, mesh::AbstractMesh{1}, equations, dg::DG,
            cache
        )
        @threaded for element in eachelement(dg, cache)
            for i in eachnode(dg)
                x_node = get_node_coords(
                    cache.elements.node_coordinates, equations, dg, i,
                    element
                )
    ...
    function compute_coefficients!(
            u, func, t, mesh::AbstractMesh{2}, equations, dg::DG,
            cache
        )
        @threaded for element in eachelement(dg, cache)
    ...
    function compute_coefficients!(
            u, func, t, mesh::AbstractMesh{3}, equations, dg::DG,
            cache
        )
        @threaded for element in eachelement(dg, cache)
