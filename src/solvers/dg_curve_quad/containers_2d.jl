...
struct GeneralElementContainer2D{RealT<:Real, NNODES, NVARS}
  ...
  surface_flux_values::SArray{Tuple{NVARS, NNODES, 4}, RealT} # [variables, i, local sides]
end
...
struct GeneralInterfaceContainer2D{RealT<:Real, NNODES, NVARS}
  edge_type        ::String
  start_idx        ::Int64
  inc_idx          ::Int64
  element_ids      ::SVector{2, Int64}
  element_side_ids ::SVector{2, Int64}
  surface_u_values ::SArray{Tuple{NVARS, NNODES, 4}, RealT} # [variables, i, local sides]
end
...
function GeneralInterfaceContainer2D(RealT, edge_information, nvars, poly_deg)
...
# Check the boundaries/orientation along each edge
  ...
  surface_u_values = zeros( nvars , poly_deg + 1 , 4 )

  return GeneralInterfaceContainer2D{RealT, poly_deg+1, nvars}( edge_type, start_idx, inc_idx,
