...
#
# TODO: this should throw an error if one tries to create an unstructured mesh for NDIMS ≠ 2
#

# readin all the information from the mesh file into a string array
  file_lines = readlines(open(filename))

# read in the number of nodes, number of edges, number of elements and local polynomial degree
...
# declare memory for pieces of the mesh
...
# pull the (x,y) values of these corners out of the nodes array
  ...
  # pull the information to cehck if boundary is curved in order to readin additional data or construct needed edge data
    file_idx += 1
    current_line    = split(file_lines[file_idx])
...
# construct quad geometry using only corner information
      ...
      for i in 1:4
        if curved_check[i] == 0
# when curved_check[i] is 0 then the "curve" from cornerNode(i) to cornerNode(i+1) is a straight line
          ...
          for k in 1:mesh_poly_deg + 1
            file_idx      += 1
            current_line   = split(file_lines[file_idx])
            CurveVals[k,1] = parse(Float64,current_line[1])
            CurveVals[k,2] = parse(Float64,current_line[2])
          end
        end
# construct the curve interpolant for the current side
        GammaCurves[i] = GammaCurve(RealT, mesh_poly_deg, CurveVals[:,1], CurveVals[:,2])
# indexing update that contains a "flip" to make sure that the element orientation is correct
...
# finally read in the boundary names where "---" means an internal connection
      file_idx  += 1
      bndy_names = split(file_lines[file_idx])
# construct quad geometry using all the curve information
