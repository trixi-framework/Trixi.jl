...
function bilin_spline(path::String)
  ...
  y = [parse(Float64, val) for val in lines[(7+num_elements_x):(5+num_elements_x+num_elements_y)]]
  z_tmp = [parse(Float64, val) for val in lines[(7+num_elements_x+num_elements_y):end]]

  z = transpose(reshape(z_tmp, (num_elements_x, num_elements_y)))

  bi_lin_spline(x, y, Matrix(z))
end

#################################
### Bicubic Spline simplified ###
#################################
