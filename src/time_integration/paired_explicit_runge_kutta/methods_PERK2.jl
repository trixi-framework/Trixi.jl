...
@muladd begin
    ...
    function compute_PairedExplicitRK2_butcher_tableau(
            num_stages, eig_vals, tspan,
            bS, cS; verbose = false
        )
        ...
        dteps = 1.0e-9 # Hyperparameter of the optimization, might be too large for systems requiring very small timesteps

        num_eig_vals, eig_vals = filter_eig_vals(eig_vals; verbose)

        monomial_coeffs, dt_opt = bisect_stability_polynomial(
            consistency_order,
        ...
        monomial_coeffs = undo_normalization!(
            monomial_coeffs, consistency_order,
            num_stages
        )
    ...
    function PairedExplicitRK2(
            num_stages, base_path_monomial_coeffs::AbstractString,
            bS = 1.0, cS = 0.5
        )
        a_matrix, c = compute_PairedExplicitRK2_butcher_tableau(
            num_stages,
            base_path_monomial_coeffs,
            bS, cS
        )
    ...
    # semidiscretization
    function PairedExplicitRK2(
            num_stages, tspan, semi::AbstractSemidiscretization;
            verbose = false,
            bS = 1.0, cS = 0.5
        )
    ...
    # list of eigenvalues
    function PairedExplicitRK2(
            num_stages, tspan, eig_vals::Vector{ComplexF64};
            verbose = false,
            bS = 1.0, cS = 0.5
        )
        a_matrix, c = compute_PairedExplicitRK2_butcher_tableau(
    ...
    function PairedExplicitRKOptions(callback, tspan; maxiters = typemax(Int), kwargs...)
        ...
        PairedExplicitRKOptions{typeof(callback), typeof(tstops_internal)}(
            callback,
            false, Inf,
            maxiters,
            tstops_internal
    ...
    mutable struct PairedExplicitRK2Integrator{
    ...
    function init(
            ode::ODEProblem, alg::PairedExplicitRK2;
        ...
        integrator = PairedExplicitRK2Integrator(
            u0, du, u_tmp, t0, tdir, dt, dt, iter,
            ode.p,
            (prob = ode,), ode.f, alg,
            PairedExplicitRKOptions(
                callback,
    ...
    function step!(integrator::PairedExplicitRK2Integrator)
        ...
        @trixi_timeit timer() "Paired Explicit Runge-Kutta ODE integration step" begin
            ...
            # k2
            integrator.f(
                integrator.du, integrator.u_tmp, prob.p,
