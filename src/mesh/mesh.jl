...
function TreeMesh(coordinates_min::NTuple{NDIMS,Real}, coordinates_max::NTuple{NDIMS,Real};
  ...
  # Create mesh
  mesh = @_timeit timer() "creation" TreeMesh{NDIMS, TreeType}(n_cells_max, domain_center, domain_length, periodicity)
...
function initialize!(mesh::TreeMesh, initial_refinement_level,
  ...
  @_timeit timer() "initial refinement" refine_uniformly!(mesh.tree, initial_refinement_level)

  # Apply refinement patches
  @_timeit timer() "refinement patches" for patch in refinement_patches
  ...
  @_timeit timer() "coarsening patches" for patch in coarsening_patches
    mpi_isparallel() && error("non-uniform meshes not supported in parallel")
    # TODO: Taal refactor, use multiple dispatch
    if patch.type == "box"
