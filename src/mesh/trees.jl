...
function refine_unbalanced!(t::Tree, cell_ids)
  ...
  for (count, original_cell_id) in enumerate(sort(unique(cell_ids)))
    ...
    for child in 1:n_children
      ...
      t.domain_id[child_id] = t.domain_id[cell_id] + 1
...
function invalidate!(t::Tree, first::Int, last::Int)
  ...
  t.domain_id[first:last] .= typemin(Int)
end
...
function raw_copy!(target::Tree, source::Tree, first::Int,
  ...
  copy_data!(target.domain_id, source.domain_id, first, last, destination)
end
...
function reset_data_structures!(t::Tree{D}) where D
  ...
  t.domain_id = Vector{Int}(undef, t.capacity + 1)
