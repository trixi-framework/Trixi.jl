...
@muladd begin
    ...
    function analyze_convergence(
        ...
        errorsmatrix = Dict(
            kind => transpose(reshape(error, (nvariables, iterations)))
        ...
        eocs = Dict(
            kind => log.(error[2:end, :] ./ error[1:(end - 1), :]) ./ log(1 / 2)
    ...
    function extract_initial_resolution(elixir, kwargs)
        ...
        try
            ...
            initial_refinement_level = TrixiBase.find_assignment(
                expr,
            ...
            if isa(e, ArgumentError)
                try
                    ...
                    cells_per_dimension = eval(
                        TrixiBase.find_assignment(
                            expr,
                            :cells_per_dimension
                        )
                    )
                    ...
                    if isa(e2, ArgumentError)
                        throw(
                            ArgumentError(
                                "`convergence_test` requires the elixir to define " *
    ...
    function include_refined(
            mod, elixir, cells_per_dimension::NTuple{NDIMS, Int}, iter;
            kwargs
        ) where {NDIMS}
