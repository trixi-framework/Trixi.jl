...
@muladd begin
    ...
    function load_pointerwrapper_sc(
            ::Type{T}, sc_array::PointerWrapper{sc_array},
            i::Integer = 1
        ) where {T}
    ...
    # 2D
    function new_p4est(
            connectivity::PointerOrWrapper{p4est_connectivity_t},
    ...
    # 3D
    function new_p4est(
            connectivity::PointerOrWrapper{p8est_connectivity_t},
            initial_refinement_level
        )
        comm = P4est.uses_mpi() ? mpi_comm() : 0 # Use Trixi.jl's MPI communicator if p4est supports MPI
        p8est_new_ext(
            comm, connectivity, 0, initial_refinement_level, true,
    ...
    # 2D
    function refine_p4est!(
            p4est::PointerOrWrapper{p4est_t}, recursive, refine_fn_c,
            init_fn_c
        )
    ...
    # 3D
    function refine_p4est!(
            p8est::PointerOrWrapper{p8est_t}, recursive, refine_fn_c,
            init_fn_c
        )
    ...
    # 2D
    function coarsen_p4est!(
            p4est::PointerOrWrapper{p4est_t}, recursive, coarsen_fn_c,
            init_fn_c
        )
    ...
    # 3D
    function coarsen_p4est!(
            p8est::PointerOrWrapper{p8est_t}, recursive, coarsen_fn_c,
            init_fn_c
        )
    ...
    # 2D
    function ghost_is_valid_p4est(
            p4est::PointerOrWrapper{p4est_t},
    ...
    # 3D
    function ghost_is_valid_p4est(
            p4est::PointerOrWrapper{p8est_t},
    ...
    function iterate_p4est(
            p4est::PointerOrWrapper{p4est_t}, user_data;
            ghost_layer = C_NULL,
    ...
    # 3D
    function iterate_p4est(
            p8est::PointerOrWrapper{p8est_t}, user_data;
            ghost_layer = C_NULL,
            iter_volume_c = C_NULL, iter_face_c = C_NULL
        )
        if user_data === C_NULL
    ...
    # 2D version
    function load_pointerwrapper_side(
            info::PointerWrapper{p4est_iter_face_info_t},
            i::Integer = 1
        )
    ...
    # 3D version
    function load_pointerwrapper_side(
            info::PointerWrapper{p8est_iter_face_info_t},
            i::Integer = 1
        )
