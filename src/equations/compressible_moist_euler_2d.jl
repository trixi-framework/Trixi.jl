...
@doc raw"""
CompressibleMoistEulerEquations2D(; g, RealT)

The moist compressible Euler equations
```math
...
water vapor ``\rho_v`` and liquid water ``\rho_l``, ``v_1``,`v_2` the velocities, ``E`` the specific total energy **rather than** specific internal energy,
 and
```math
p = (rho_d R_v + rho_v R_v) * T
```
the pressure. This assumes that dry air and water vapor behave like an ideal gas with gas constants ``R_d`` (dry air) and ``R_v`` (vapor).
"""
...
@inline function flux(u, orientation::Integer, equations::CompressibleMoistEulerEquations2D)
  rho, rho_v1, rho_v2, rho_E, rho_qv, rho_ql = u
  _, v1, v2, p, qv, ql = cons2prim(u, equations)
...
# Note: These entropy variables are derived from the entropy that is used here
# which is not an mathematical entropy since it's Hessian has rank 0.
