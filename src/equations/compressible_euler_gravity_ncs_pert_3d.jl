...
#! format: noindent
@doc raw"""
    CompressibleEulerEquationsGravityNCSPert3D(gamma)
...
"""
struct CompressibleEulerEquationsGravityNCSPert3D{RealT <: Real} <:
       AbstractCompressibleEulerEquations{3, 5}
    gamma::RealT               # ratio of specific heats
    inv_gamma_minus_one::RealT # = inv(gamma - 1); can be used to write slow divisions as fast multiplications

    function CompressibleEulerEquationsGravityNCSPert3D(gamma)
        γ, inv_gamma_minus_one = promote(gamma, inv(gamma - 1))
        new{typeof(γ)}(γ, inv_gamma_minus_one)
    end
end

have_nonconservative_terms(::CompressibleEulerEquationsGravityNCSPert3D) = True()
have_aux_node_vars(::CompressibleEulerEquationsGravityNCSPert3D) = True()
n_aux_node_vars(::CompressibleEulerEquationsGravityNCSPert3D) = 6

function cons2aux(u, aux, equations::CompressibleEulerEquationsGravityNCSPert3D)
    return aux
end

varnames(::typeof(cons2aux),
         ::CompressibleEulerEquationsGravityNCSPert3D) = ("rho_steady",
                                                          "rho_v1_steady",
                                                          "rho_v2_steady",
                                                          "rho_v3_steady",
                                                          "rho_e_steady",
...
@inline function cons2prim(u, aux,
                           equations::CompressibleEulerEquationsGravityNCSPert3D)
...
@inline function cons2prim_total(u, aux,
    ...
    return cons2prim(u_total, aux, equations)
end

varnames(::typeof(cons2prim_total),
         ::CompressibleEulerEquationsGravityNCSPert3D) = ("rho_total",
                                                      "v1_total",
                                                      "v2_total",
                                                      "v3_total",
                                                      "p_total")
...
@inline function cons2prim_pert(u, aux,
    ...
    u_prim_steady = cons2prim(aux, aux, equations)
    return u_prim_total - u_prim_steady
end

varnames(::typeof(cons2prim_pert),
         ::CompressibleEulerEquationsGravityNCSPert3D) = ("rho_pert",
                                                      "v1_pert",
                                                      "v2_pert",
                                                      "v3_pert",
                                                      "p_pert")
...
@inline function boundary_condition_slip_wall(u_inner, aux_inner,
    ...
    rho_local, v_normal, v_tangent1, v_tangent2, p_local = cons2prim(u_local,
                                                                            aux_inner,
                                                                            equations)
...
"""
    boundary_condition_slip_wall(u_inner, aux_inner, orientation, direction, x, t,
                                 surface_flux_function, equations::CompressibleEulerEquationsGravityNCSPert3D)
