...
@inline function Base.real(::IdealGlmMhdMulticomponentEquations2D{NVARS,NCOMP,RealT}) where {NVARS,
                                                                                             NCOMP,
                                                                                             RealT}
  RealT
end
...
function initial_condition_convergence_test(x, t,
  ...
  rho = 1
  prim_rho = SVector{ncomponents(equations),real(equations)}(2^(i - 1) * (1 - 2) / (1 -
...
function flux_derigs_etal(u_ll, u_rr, orientation::Integer,
  ...
  rhok_mean = SVector{ncomponents(equations),real(equations)}(ln_mean(u_ll[i + 8],
                                                              ...
                                                              for i in
                                                                  eachcomponent(equations))
  rhok_avg = SVector{ncomponents(equations),real(equations)}(0.5 *
                                                             (u_ll[i + 8] + u_rr[i + 8])
                                                             for i in
                                                                 eachcomponent(equations))
  ...
  T_ll = (rho_e_ll - 0.5 * rho_ll * (vel_norm_ll) - 0.5 * mag_norm_ll - 0.5 * psi_ll^2) /
         help1_ll
  T_rr = (rho_e_rr - 0.5 * rho_rr * (vel_norm_rr) - 0.5 * mag_norm_rr - 0.5 * psi_rr^2) /
         help1_rr
...
@inline function cons2entropy(u, equations::IdealGlmMhdMulticomponentEquations2D)
  ...
  entrop_rho = SVector{ncomponents(equations),real(equations)}(-1.0 * (cv[i] * log(T) -
                                                                       gas_constants[i] *
                                                                       log(u[i + 8])) +
                                                               gas_constants[i] + cv[i] -
                                                               (v_square / (2 * T))
                                                               for i in
                                                                   eachcomponent(equations))
