...
"""
@inline function source_terms_convergence_test(u, x, t, equations::TrafficFlowLWREquations1D)
  ...
  f = 1/L
  omega = 2 * pi * f
  du = omega * cos(omega * (x[1] - t)) * (-1 -2 * equations.v_max * sin(omega * (x[1] - t)) - 3 * equations.v_max)
...
@inline function max_abs_speed_naive(u_ll, u_rr, orientation::Integer, equations::TrafficFlowLWREquations1D)
  λ_max = max(abs(equations.v_max * (1.0 - 2 * u_ll[1])), 
              abs(equations.v_max * (1.0 - 2 * u_rr[1])))
end

# Calculate minimum and maximum wave speeds for HLL-type fluxes
@inline function min_max_speed_naive(u_ll, u_rr, orientation::Integer, equations::TrafficFlowLWREquations1D)
...
@inline energy_total(u, equations::TrafficFlowLWREquations1D) = energy_total(u[1], equations)
