...
@doc raw"""
    CompressibleEulerEquationsNCGravity3D(gamma)
...
"""
struct CompressibleEulerEquationsNCGravity3D{RealT <: Real} <:
       AbstractCompressibleEulerEquations{3, 5}
    gamma::RealT               # ratio of specific heats
    inv_gamma_minus_one::RealT # = inv(gamma - 1); can be used to write slow divisions as fast multiplications

    function CompressibleEulerEquationsNCGravity3D(gamma)
        γ, inv_gamma_minus_one = promote(gamma, inv(gamma - 1))
        new{typeof(γ)}(γ, inv_gamma_minus_one)
    end
end

have_nonconservative_terms(::CompressibleEulerEquationsNCGravity3D) = True()
function varnames(::typeof(cons2cons), ::CompressibleEulerEquationsNCGravity3D)
    ("rho", "rho_v1", "rho_v2", "rho_v3", "rho_e", "phi")
end
function varnames(::typeof(cons2prim), ::CompressibleEulerEquationsNCGravity3D)
    ("rho", "v1", "v2", "v3", "p", "phi")
end

have_aux_node_vars(::CompressibleEulerEquationsNCGravity3D) = True()
n_aux_node_vars(::CompressibleEulerEquationsNCGravity3D) = 1

function cons2aux(u, aux, equations::CompressibleEulerEquationsNCGravity3D)
    return SVector(aux[1])
end

varnames(::typeof(cons2aux), ::CompressibleEulerEquationsNCGravity3D) =("geopotential")
...
@inline function boundary_condition_slip_wall(u_inner, aux_inner,
    ...
    rho_local, v_normal, v_tangent1, v_tangent2, p_local = cons2prim(u_local, aux_inner,
                                                                        equations)
...
"""
    boundary_condition_slip_wall(u_inner, orientation, direction, x, t,
                                 surface_flux_function, equations::CompressibleEulerEquationsNCGravity3D)
...
@inline function flux(u, normal_direction::AbstractVector,
                      equations::CompressibleEulerEquationsNCGravity3D)
    rho_e = u[5]
    rho, v1, v2, v3, p = cons2prim(u, aux, equations)
...
@inline function flux_shima_etal(u_ll, u_rr, aux_ll, aux_rr, orientation::Integer,
    ...
    rho_ll, v1_ll, v2_ll, v3_ll, p_ll = cons2prim(u_ll, aux_ll, equations)
    rho_rr, v1_rr, v2_rr, v3_rr, p_rr = cons2prim(u_rr, aux_rr, equations)
...
@inline function cons2prim(u, aux, equations::CompressibleEulerEquationsNCGravity3D)
    rho, rho_v1, rho_v2, rho_v3, rho_e = u
...
@inline function density(u, equations::CompressibleEulerEquationsNCGravity3D)
    rho = u[1]
    return rho
end

@inline function velocity(u, equations::CompressibleEulerEquationsNCGravity3D)
    rho = u[1]
    v1 = u[2] / rho
    v2 = u[3] / rho
    v3 = u[4] / rho
...
@inline function pressure(u, equations::CompressibleEulerEquationsNCGravity3D)
    rho, rho_v1, rho_v2, rho_v3, rho_e = u
    p = (equations.gamma - 1) *
        (rho_e - 0.5f0 * (rho_v1^2 + rho_v2^2 + rho_v3^2) / rho)
    return p
end
...
@inline function entropy(cons, equations::CompressibleEulerEquationsNCGravity3D)
    entropy_math(cons, equations)
end

# Calculate total energy for a conservative state `cons`
@inline energy_total(cons, ::CompressibleEulerEquationsNCGravity3D) = cons[5]
...
@inline function (dissipation::DissipationLocalLaxFriedrichs)(u_ll, u_rr,
    ...
    return SVector(diss[1], diss[2], diss[3], diss[4], diss[5])
end
end # @muladd
