...
```
"""
struct CompressibleNavierStokesVarMuDiffusion1D{GradientVariables, RealT <: Real,
                                                E <:
...
# default to primitive gradient variables
function CompressibleNavierStokesVarMuDiffusion1D(equations::CompressibleEulerEquations1D;
    ...
    CompressibleNavierStokesVarMuDiffusion1D{typeof(gradient_variables), typeof(gamma),
...
function flux(u, gradients, orientation::Integer,
              equations::CompressibleNavierStokesVarMuDiffusion1D)
    # Here, `u` is assumed to be the "transformed" variables specified by `gradient_variable_transformation`.
    rho, v1, _ = convert_transformed_to_primitive(u, equations)
    # Here `gradients` is assumed to contain the gradients of the primitive variables (rho, v1, v2, T)
    # either computed directly or reverse engineered from the gradient of the entropy variables
...
function cons2entropy(u,
                      equations::Union{CompressibleNavierStokesDiffusion1D,
                                       CompressibleNavierStokesVarMuDiffusion1D})
...
@inline function convert_transformed_to_primitive(u_transformed,
                                                  equations::Union{CompressibleNavierStokesDiffusion1D{GradientVariablesPrimitive},
                                                                   CompressibleNavierStokesVarMuDiffusion1D{GradientVariablesPrimitive}})
...
@inline function convert_derivative_to_primitive(u, gradient,
                                                 ::Union{CompressibleNavierStokesDiffusion1D{GradientVariablesPrimitive},
                                                         CompressibleNavierStokesVarMuDiffusion1D{GradientVariablesPrimitive}})
    return gradient
end
