...
@muladd begin
    ...
    struct CompressibleNavierStokesDiffusion3D{
            GradientVariables, RealT <: Real, Mu,
    ...
    function CompressibleNavierStokesDiffusion3D(
        ...
        CompressibleNavierStokesDiffusion3D{
            typeof(gradient_variables), typeof(gamma),
            typeof(mu),
            typeof(equations),
        }(
    ...
    function flux(
            u, gradients, orientation::Integer,
        ...
        _, dv1dx, dv2dx, dv3dx, dTdx = convert_derivative_to_primitive(
            u, gradients[1],
            equations
        )
        _, dv1dy, dv2dy, dv3dy, dTdy = convert_derivative_to_primitive(
            u, gradients[2],
            equations
        )
        _, dv1dz, dv2dz, dv3dz, dTdz = convert_derivative_to_primitive(
            u, gradients[3],
            equations
        )
    ...
    @inline function vorticity(u, gradients, equations::CompressibleNavierStokesDiffusion3D)
        ...
        _, dv1dx, dv2dx, dv3dx, _ = convert_derivative_to_primitive(
            u, gradients[1],
            equations
        )
        _, dv1dy, dv2dy, dv3dy, _ = convert_derivative_to_primitive(
            u, gradients[2],
            equations
        )
        _, dv1dz, dv2dz, dv3dz, _ = convert_derivative_to_primitive(
            u, gradients[3],
            equations
        )
    ...
    @inline function (
            boundary_condition::BoundaryConditionNavierStokesWall{
                <:NoSlip,
                <:Adiabatic,
            }
        )(
    ...
    @inline function (
        ...
        normal_heat_flux = boundary_condition.boundary_condition_heat_flux.boundary_value_normal_flux_function(
            x,
            t,
            equations
        )
        ...
        return SVector(
            flux_inner[1], flux_inner[2], flux_inner[3], flux_inner[4],
            normal_energy_flux
        )
    end
    ...
    @inline function (
        ...
        return SVector(
            w_inner[1], -v1 * negative_rho_inv_p, -v2 * negative_rho_inv_p,
