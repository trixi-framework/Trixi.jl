...
#! format: noindent
@doc raw"""
    CompressibleEulerEquationsFullPerturbationGravity3D(gamma)
...
"""
struct CompressibleEulerEquationsFullPerturbationGravity3D{RealT <: Real} <:
       AbstractCompressibleEulerEquations{3, 5}
    gamma::RealT               # ratio of specific heats
    inv_gamma_minus_one::RealT # = inv(gamma - 1); can be used to write slow divisions as fast multiplications

    function CompressibleEulerEquationsFullPerturbationGravity3D(gamma)
        γ, inv_gamma_minus_one = promote(gamma, inv(gamma - 1))
        new{typeof(γ)}(γ, inv_gamma_minus_one)
    end
end

have_nonconservative_terms(::CompressibleEulerEquationsFullPerturbationGravity3D) = True()
have_aux_node_vars(::CompressibleEulerEquationsFullPerturbationGravity3D) = True()
n_aux_node_vars(::CompressibleEulerEquationsFullPerturbationGravity3D) = 6

function cons2aux(u, aux, equations::CompressibleEulerEquationsFullPerturbationGravity3D)
    return aux
end

varnames(::typeof(cons2aux), ::CompressibleEulerEquationsFullPerturbationGravity3D) = ("rho_steady",
                                                                                   "rho_v1_steady",
                                                                                   "rho_v2_steady",
                                                                                   "rho_v3_steady",
                                                                                   "rho_e_steady",
...
varnames(::typeof(cons2prim_total), ::CompressibleEulerEquationsFullPerturbationGravity3D) = ("rho_total",
                                                                                          "v1_total",
                                                                                          "v2_total",
                                                                                          "v3_total",
                                                                                          "p_total")
...
varnames(::typeof(cons2prim_pert), ::CompressibleEulerEquationsFullPerturbationGravity3D) = ("rho_pert",
                                                                                         "v1_pert",
                                                                                         "v2_pert",
                                                                                         "v3_pert",
                                                                                         "p_pert")
...
@inline function boundary_condition_slip_wall(u_inner, aux_inner,
    ...
    p_star_total = calc_pstar(u_total, aux_inner, normal, tangent1, tangent2, equations,
                              gamma, inv_gamma_minus_one)
    
    # compute boundary conditions for steady conserved quantities
    p_star_steady = calc_pstar(aux_inner, aux_inner, normal, tangent1, tangent2, equations,
                               gamma, inv_gamma_minus_one)
    
    return (SVector(0,
                    (p_star_total - p_star_steady) * normal_direction[1],
                    (p_star_total - p_star_steady) * normal_direction[2],
                    (p_star_total - p_star_steady) * normal_direction[3],
                    0),
            SVector(0, 0, 0, 0, 0))
end

@inline function calc_pstar(u, aux, normal, tangent1, tangent2, equations,
                            gamma, inv_gamma_minus_one)
    # rotate the internal solution state
    u_rot = rotate_to_x(u, normal, tangent1, tangent2, equations)

    # compute the primitive variables
    rho_local, v_normal, v_tangent1, v_tangent2, p_local = cons2prim_geopot(u_rot, aux, equations)
...
"""
    boundary_condition_slip_wall(u_inner, aux_inner, orientation, direction, x, t,
                                 surface_flux_function, equations::CompressibleEulerEquationsFullPerturbationGravity3D)
...
@inline function flux(u, aux, normal_direction::AbstractVector,
                      equations::CompressibleEulerEquationsFullPerturbationGravity3D)
    u_total = cons2cons_total(u, aux, equations)
    flux_total = calc_flux(u_total, aux, normal_direction, equations)
    flux_steady = calc_flux(aux, aux, normal_direction, equations)
    return flux_total - flux_steady
end

@inline function calc_flux(u, aux, normal_direction::AbstractVector,
                           equations::CompressibleEulerEquationsFullPerturbationGravity3D)
    rho, _, _, _, rho_e = u
    _, v1, v2, v3, p = cons2prim_geopot(u, aux, equations)
...
@inline function flux_shima_etal(u_ll, u_rr, aux_ll, aux_rr,
    ...
    flux_total = calc_flux_shima_etal(u_ll_total, u_rr_total, aux_ll, aux_rr,
                                      normal_direction, equations)
    flux_steady = calc_flux_shima_etal(aux_ll, aux_rr, aux_ll, aux_rr,
                                       normal_direction, equations)
    return flux_total - flux_steady
end

@inline function calc_flux_shima_etal(u_ll, u_rr, aux_ll, aux_rr,
                                      normal_direction::AbstractVector,
                                      equations::CompressibleEulerEquationsFullPerturbationGravity3D)
    # Unpack left and right state
    rho_ll, v1_ll, v2_ll, v3_ll, p_ll = cons2prim_geopot(u_ll, aux_ll, equations)
    rho_rr, v1_rr, v2_rr, v3_rr, p_rr = cons2prim_geopot(u_rr, aux_rr, equations)
    ...
    f5 = (f1 * (velocity_square_avg + phi_avg) +
...
@inline function flux_kennedy_gruber(u_ll, u_rr, aux_ll, aux_rr,
    ...
    flux_total = calc_flux_kennedy_gruber(u_ll_total, u_rr_total, aux_ll, aux_rr,
                                      normal_direction, equations)
    flux_steady = calc_flux_kennedy_gruber(aux_ll, aux_rr, aux_ll, aux_rr,
                                       normal_direction, equations)
    return flux_total - flux_steady
end

@inline function calc_flux_kennedy_gruber(u_ll, u_rr, aux_ll, aux_rr,
                                         normal_direction::AbstractVector,
                                         equations::CompressibleEulerEquationsFullPerturbationGravity3D)
    # Unpack left and right state
    rho_e_ll = u_ll[5]
...
@inline function flux_ranocha(u_ll, u_rr, aux_ll, aux_rr,
    ...
    flux_total = calc_flux_ranocha(u_ll_total, u_rr_total, aux_ll, aux_rr,
                                   normal_direction, equations)
    flux_steady = calc_flux_ranocha(aux_ll, aux_rr, aux_ll, aux_rr,
                                    normal_direction, equations)
    return flux_total - flux_steady
end

@inline function calc_flux_ranocha(u_ll, u_rr, aux_ll, aux_rr,
                                   normal_direction::AbstractVector,
                                   equations::CompressibleEulerEquationsFullPerturbationGravity3D)
    # Unpack left and right state
    rho_ll, v1_ll, v2_ll, v3_ll, p_ll = cons2prim_geopot(u_ll, aux_ll, equations)
    ...
    f3 = f1 * v2_avg + p_avg * normal_direction[2]
    f4 = f1 * v3_avg + p_avg * normal_direction[3]
    f5 = (f1 * (velocity_square_avg + phi_avg + inv_rho_p_mean * equations.inv_gamma_minus_one)
          +
          0.5 * (p_ll * v_dot_n_rr + p_rr * v_dot_n_ll))
...
@inline function (flux_lmars::FluxLMARS)(u_ll, u_rr, aux_ll, aux_rr,
    ...
    flux_total = calc_flux_lmars(u_ll_total, u_rr_total, aux_ll, aux_rr, c,
                                   normal_direction, equations)
    flux_steady = calc_flux_lmars(aux_ll, aux_rr, aux_ll, aux_rr, c,
                                    normal_direction, equations)
    return flux_total - flux_steady
end
    
@inline function calc_flux_lmars(u_ll, u_rr, aux_ll, aux_rr, c,
                                 normal_direction::AbstractVector,
                                 equations::CompressibleEulerEquationsFullPerturbationGravity3D)
    # Unpack left and right state
    rho_ll, v1_ll, v2_ll, v3_ll, p_ll = cons2prim_geopot(u_ll, aux_ll, equations)
    ...
    if v >= 0
        f1, f2, f3, f4, f5 = u_ll * v
        f5 = f5 + p_ll * v
    else
        f1, f2, f3, f4, f5 = u_rr * v
        f5 = f5 + p_rr * v
    end
