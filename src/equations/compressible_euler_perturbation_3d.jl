...
@doc raw"""
    CompressibleEulerEquationsPerturbation3D(gamma)

The compressible Euler equations [`CompressibleEulerEquationsPerturbation3D`](@ref), formulated using a steady background state and perturbations.
...
"""
struct CompressibleEulerEquationsPerturbation3D{RealT <: Real} <:
       AbstractCompressibleEulerEquations{3, 5}
    equations_total::CompressibleEulerEquations3D{RealT}

    function CompressibleEulerEquationsPerturbation3D(gamma)
        γ, inv_gamma_minus_one = promote(gamma, inv(gamma - 1))
        new{typeof(γ)}(CompressibleEulerEquations3D(gamma))
    end
end

have_aux_node_vars(::CompressibleEulerEquationsPerturbation3D) = True()
n_aux_node_vars(::CompressibleEulerEquationsPerturbation3D) = 5

function cons2aux(u, aux, equations::CompressibleEulerEquationsPerturbation3D)
...
@inline function max_abs_speed_naive(u_ll, u_rr, aux_ll, aux_rr,
    ...
    u_ll_total = cons2cons_total(u_ll, aux_ll, equations)
    u_rr_total = cons2cons_total(u_rr, aux_rr, equations)
    return max_abs_speed_naive(u_ll_total, u_rr_total, normal_direction,
                               equations.equations_total)
end
...
@inline function flux(u, aux, orientation::Integer,
    ...
    return _flux - SVector(0, f2, f3, f4, 0)
end

@inline function flux(u, aux, normal_direction::AbstractVector,
                      equations::CompressibleEulerEquationsPerturbation3D)
    u_total = cons2cons_total(u, aux, equations)
    _flux = flux(u_total, normal_direction, equations.equations_total)
...
@inline function flux_kennedy_gruber(u_ll, u_rr, aux_ll, aux_rr,
    ...
    u_ll_total = cons2cons_total(u_ll, aux_ll, equations)
    u_rr_total = cons2cons_total(u_rr, aux_rr, equations)
    flux = flux_kennedy_gruber(u_ll_total, u_rr_total, normal_direction,
                               equations.equations_total)
