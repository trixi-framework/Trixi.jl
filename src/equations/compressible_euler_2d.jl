...
@muladd begin
    ...
    @inline function boundary_condition_slip_wall(
        ...
        if v_normal <= 0
            ...
            p_star = p_local *
                (1 + 0.5f0 * (equations.gamma - 1) * v_normal / sound_speed)^(
                2 *
                    equations.gamma *
    ...
    @inline function boundary_condition_slip_wall(
        ...
        return boundary_condition_slip_wall(
            u_inner, normal_direction, direction,
    ...
    @inline function splitting_steger_warming(
            u, ::Val{:plus}, orientation::Integer,
            equations::CompressibleEulerEquations2D
        )
        rho, rho_v1, rho_v2, rho_e = u
        v1 = rho_v1 / rho
    ...
    @inline function splitting_steger_warming(
            u, ::Val{:minus}, orientation::Integer,
            equations::CompressibleEulerEquations2D
        )
        rho, rho_v1, rho_v2, rho_e = u
        v1 = rho_v1 / rho
    ...
    """
    @inline function splitting_drikakis_tsangaris(
            u, orientation_or_normal_direction,
            equations::CompressibleEulerEquations2D
        )
        fm = splitting_drikakis_tsangaris(
            u, Val{:minus}(), orientation_or_normal_direction,
            equations
        )
        fp = splitting_drikakis_tsangaris(
            u, Val{:plus}(), orientation_or_normal_direction,
            equations
        )
        return fm, fp
    end
    ...
    @inline function splitting_vanleer_haenel(
        ...
        fm = splitting_vanleer_haenel(
            u, Val{:minus}(), orientation_or_normal_direction,
            equations
        )
        fp = splitting_vanleer_haenel(
            u, Val{:plus}(), orientation_or_normal_direction,
            equations
        )
        return fm, fp
    ...
    @inline function splitting_lax_friedrichs(
        ...
        fm = splitting_lax_friedrichs(
            u, Val{:minus}(), orientation_or_normal_direction,
            equations
        )
        fp = splitting_lax_friedrichs(
            u, Val{:plus}(), orientation_or_normal_direction,
            equations
        )
        return fm, fp
    ...
    function flux_hllc(
        ...
        elseif Ssr <= 0
            ...
            SStar = (
                rho_ll * v_dot_n_ll * sMu_L - rho_rr * v_dot_n_rr * sMu_R +
                    (p_rr - p_ll) * norm_sq
            ) / (rho_ll * sMu_L - rho_rr * sMu_R)
            if Ssl <= 0 <= SStar
                densStar = rho_ll * sMu_L / (Ssl - SStar)
