...
function Equations.riemann_fv!(surface_flux::AbstractMatrix{Float64},
...
function Equations.riemann_fv_diss!(surface_flux::AbstractArray{Float64, 1},
...
#  if equation.surface_flux_type == :laxfriedrichs
    ...
    surface_flux[1] =  - 1/2 * λ_max * (rho_rr    - rho_ll)
    surface_flux[2] =  - 1/2 * λ_max * (rho_v1_rr - rho_v1_ll)
    surface_flux[3] =  - 1/2 * λ_max * (rho_v2_rr - rho_v2_ll)
    surface_flux[4] =  - 1/2 * λ_max * (rho_e_rr  - rho_e_ll)
end

# Calculate flux across interface with different states on both sides (pointwise version)
function Equations.riemann_fv!(surface_flux::AbstractArray{Float64, 1},
                            rho_ll, rho_v1_ll, rho_v2_ll, rho_e_ll,
                            rho_rr, rho_v1_rr, rho_v2_rr, rho_e_rr,
                            equation::Euler, orientation::Int)
  # Calculate primitive variables and speed of sound
...
function Equations.cons2entropy(gamma::Float64, cons::A, entropy::A, ::Val{N})  where A<:AbstractArray{T,3} where {T,N}
  ...
  for j = 1:N
    for i = 1:N
      v[1,i,j] = cons[2,i,j] / cons[1,i,j] 
      v[2,i,j] = cons[3,i,j] / cons[1,i,j] 
      v_square[i,j] = v[1,i,j]*v[1,i,j]+v[2,i,j]*v[2,i,j]
      p[i,j] = ((gamma - 1)
                          * (cons[4,i,j] - 
         	     1/2 * (cons[2,i,j] * v[1,i,j] +
                             cons[3,i,j] * v[2,i,j])))
      s[i,j] = log(p[i,j]) - gamma*log(cons[1,i,j])
      rho_p[i,j] = cons[1,i,j] / p[i,j] 

      entropy[1,i,j] = (gamma - s[i,j])/(gamma-1) -
                            0.5*rho_p[i,j]*v_square[i,j] 
      entropy[2,i,j] = rho_p[i,j]*v[1,i,j]
      entropy[3,i,j] = rho_p[i,j]*v[2,i,j]
      entropy[4,i,j] = -rho_p[i,j]
    end
  end
end
