...
default_analysis_quantities(::IdealGlmMhdEquations2D) = (:l2_error, :linf_error, :dsdu_ut, :l2_divb, :linf_divb)
default_analysis_integrals(::IdealGlmMhdEquations2D)  = (entropy_timederivative, Val(:l2_divb), Val(:linf_divb))


# Set initial conditions at physical location `x` for time `t`
function initial_condition_constant(x, t, equation::IdealGlmMhdEquations2D)
  rho = 1.0
  rho_v1 = 0.1
  rho_v2 = -0.2
  rho_v3 = -0.5
...
function initial_condition_convergence_test(x, t, equation::IdealGlmMhdEquations2D)
...
function initial_condition_orszag_tang(x, t, equation::IdealGlmMhdEquations2D)
...
function initial_condition_rotor(x, t, equation::IdealGlmMhdEquations2D)
  # setup taken from Derigs et al. DMV article (2018)
...
function initial_condition_blast_wave(x, t, equation::IdealGlmMhdEquations2D)
  # setup taken from Derigs et al. DMV article (2018)
...
function initial_condition_ec_test(x, t, equation::IdealGlmMhdEquations2D)
...
# function source_terms_WHATEVER(ut, u, x, element_id, t, n_nodes, equation::IdealGlmMhdEquations2D)


# Calculate 1D flux in for a single point
@inline function calcflux(u, orientation, equation::IdealGlmMhdEquations2D)
...
"""
function flux_derigs_etal(u_ll, u_rr, orientation, equation::IdealGlmMhdEquations2D)
...
function flux_lax_friedrichs(u_ll, u_rr, orientation, equation::IdealGlmMhdEquations2D)
  rho_ll, rho_v1_ll, rho_v2_ll, rho_v3_ll, rho_e_ll, B1_ll, B2_ll, B3_ll, psi_ll = u_ll
  rho_rr, rho_v1_rr, rho_v2_rr, rho_v3_rr, rho_e_rr, B1_rr, B2_rr, B3_rr, psi_rr = u_rr
...
"""
    flux_hll(u_ll, u_rr, orientation, equation::IdealGlmMhdEquations2D)
...
"""
function flux_hll(u_ll, u_rr, orientation, equation::IdealGlmMhdEquations2D)
  rho_ll, rho_v1_ll, rho_v2_ll, rho_v3_ll, rho_e_ll, B1_ll, B2_ll, B3_ll, psi_ll = u_ll
  rho_rr, rho_v1_rr, rho_v2_rr, rho_v3_rr, rho_e_rr, B1_rr, B2_rr, B3_rr, psi_rr = u_rr
...
@inline function max_abs_speeds(u, equation::IdealGlmMhdEquations2D)
  ...
  return (sqrt(v1^2 + v2^2 + v3^2) + cf_max), 0.0
end
...
@inline function density(u, equation::IdealGlmMhdEquations2D)
  rho, rho_v1, rho_v2, rho_v3, rho_e, B1, B2, B3, psi = u
  return rho
end

@inline function pressure(u, equation::IdealGlmMhdEquations2D)
  rho, rho_v1, rho_v2, rho_v3, rho_e, B1, B2, B3, psi = u
...
@inline function density_pressure(u, equation::IdealGlmMhdEquations2D)
  rho, rho_v1, rho_v2, rho_v3, rho_e, B1, B2, B3, psi = u
  p = (equation.gamma - 1)*(rho_e - 0.5 * (rho_v1^2 + rho_v2^2 + rho_v3^2) / rho
                                  - 0.5 * (B1^2 + B2^2 + B3^2)
                                  - 0.5 * psi^2)
...
"""
@inline function calc_fast_wavespeed_roe(u_ll, u_rr, direction, equation::IdealGlmMhdEquations2D)
  rho_ll, rho_v1_ll, rho_v2_ll, rho_v3_ll, rho_e_ll, B1_ll, B2_ll, B3_ll, psi_ll = u_ll
  rho_rr, rho_v1_rr, rho_v2_rr, rho_v3_rr, rho_e_rr, B1_rr, B2_rr, B3_rr, psi_rr = u_rr
...
@inline function entropy_math(cons, equation::IdealGlmMhdEquations2D)
...
@inline entropy(cons, equation::IdealGlmMhdEquations2D) = entropy_math(cons, equation)
...
@inline function energy_kinetic(cons, equation::IdealGlmMhdEquations2D)
...
@inline function energy_internal(cons, equation::IdealGlmMhdEquations2D)
