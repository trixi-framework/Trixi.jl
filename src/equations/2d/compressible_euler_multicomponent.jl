...
struct CompressibleEulerMulticomponentEquations2D <: AbstractCompressibleEulerMulticomponentEquations{2, 5}
  # CHRIS: Hier muss gamma ein Array der Größe 2 sein
  gamma::Float64
end

function CompressibleEulerMulticomponentEquations2D()
  # CHRIS: Gamma ist nun ein Array, muss mehrere Werte aus Parameter File lesen
  gamma = parameter("gamma", 1.4)

  # CHRIS: Hier vllt extra Parameter 'K_numb_of_comp'

  # CHRIS: Hier muss das Array übergeben werden
  CompressibleEulerMulticomponentEquations2D(gamma)
end
...
# CHRIS: !!! Am besten erstmal für ein testcase entscheiden !!! z.B. eins aus Paper und einmal Convergence Test

# Set initial conditions at physical location `x` for time `t`
# function initial_conditions_density_pulse(x, t, equation::CompressibleEulerMulticomponentEquations2D)
#   # CHRIS: rho als Array? oder mehrere rhos?
#   rho = 1 + exp(-(x[1]^2 + x[2]^2))/2
#   v1 = 1
#   v2 = 1
#   # CHRIS: gleiches Problem
#   rho_v1 = rho * v1
#   rho_v2 = rho * v2
#   # CHRIS: wird p vorgegeben? oder auf rho bestimmt?
#   p = 1
#   # CHRIS: muss anders bestimmt werden, da verschiedene rho
#   rho_e = p/(equation.gamma - 1) + 1/2 * rho * (v1^2 + v2^2)
#   # CHRIS: Return muss verschiedene rhos beinhalten
#   return @SVector [rho, rho_v1, rho_v2, rho_e]
# end

"""
    initial_conditions_density_wave(x, t, equation::CompressibleEulerMulticomponentEquations2D)

test case for stability of EC fluxes from paper: https://arxiv.org/pdf/2007.09026.pdf
domain [-1, 1]^2
mesh = 4x4, polydeg = 5
"""
# function initial_conditions_density_wave(x, t, equation::CompressibleEulerMulticomponentEquations2D)
#   v1 = 0.1
#   v2 = 0.2
...
# function initial_conditions_pressure_pulse(x, t, equation::CompressibleEulerMulticomponentEquations2D)
#   rho = 1
#   v1 = 1
#   v2 = 1
#   rho_v1 = rho * v1
...
# function initial_conditions_density_pressure_pulse(x, t, equation::CompressibleEulerMulticomponentEquations2D)
#   rho = 1 + exp(-(x[1]^2 + x[2]^2))/2
#   v1 = 1
#   v2 = 1
#   rho_v1 = rho * v1
...
function initial_conditions_convergence_test(x, t, equation::CompressibleEulerMulticomponentEquations2D)
  ...
  gamma   = (rho1/rho*csv1*gamma1 + rho2/rho*csv2*gamma2)/(rho1/rho*csv1 + rho2/rho*csv2)
...
function initial_conditions_multicomponent_convergence_test(x, t, equation::CompressibleEulerMulticomponentEquations2D)
  rho    = 2 + 0.5*sin(2*pi*(x[1]-t)) + 0.5*cos(2*pi*(x[2]-t))
  rho_v1 = 0.5 * (2 + 0.5*sin(2*pi*(x[1]-t)) + 0.5*cos(2*pi*(x[2]-t)))
  rho_v2 = 0.5 * (2 + 0.5*sin(2*pi*(x[1]-t)) + 0.5*cos(2*pi*(x[2]-t)))
  rho_e  = 0.25 * (2 + 0.5*sin(2*pi*(x[1]-t)) + 0.5*cos(2*pi*(x[2]-t))) + 1/(equation.gamma - 1)

  return @SVector [rho, rho_v1, rho_v2, rho_e]
end

# function initial_conditions_isentropic_vortex(x, t, equation::CompressibleEulerMulticomponentEquations2D)
...
# function initial_conditions_blast_wave(x, t, equation::CompressibleEulerMulticomponentEquations2D)
...
# function initial_conditions_sedov_blast_wave(x, t, equation::CompressibleEulerMulticomponentEquations2D)
#   # Set up polar coordinates
#   inicenter = SVector(0.0, 0.0)
#   x_norm = x[1] - inicenter[1]
#   y_norm = x[2] - inicenter[2]
...
# function initial_conditions_medium_sedov_blast_wave(x, t, equation::CompressibleEulerMulticomponentEquations2D)
#   # Set up polar coordinates
#   inicenter = SVector(0.0, 0.0)
#   x_norm = x[1] - inicenter[1]
#   y_norm = x[2] - inicenter[2]
...
# function initial_conditions_khi(x, t, equation::CompressibleEulerMulticomponentEquations2D)
...
# function initial_conditions_blob(x, t, equation::CompressibleEulerMulticomponentEquations2D)
...
# function initial_conditions_jeans_instability(x, t, equation::CompressibleEulerMulticomponentEquations2D)
...
# function initial_conditions_eoc_test_coupled_euler_gravity(x, t, equation::CompressibleEulerMulticomponentEquations2D)
...
# function initial_conditions_sedov_self_gravity(x, t, equation::CompressibleEulerMulticomponentEquations2D)
#   # Set up polar coordinates
#   r = sqrt(x[1]^2 + x[2]^2)
...
#                                               equation::CompressibleEulerMulticomponentEquations2D)
...
# function source_terms_eoc_test_coupled_euler_gravity(ut, u, x, element_id, t, n_nodes, equation::CompressibleEulerMulticomponentEquations2D)
...
# function source_terms_eoc_test_euler(ut, u, x, element_id, t, n_nodes, equation::CompressibleEulerMulticomponentEquations2D)
#   # Same settings as in `initial_conditions_eoc_test_coupled_euler_gravity`
#   c = 2.0
#   A = 0.1
#   G = 1.0
...
# function source_terms_harmonic(ut, u, x, element_id, t, n_nodes, equation::CompressibleEulerMulticomponentEquations2D)
...
@inline function calcflux(u, orientation, equation::CompressibleEulerMulticomponentEquations2D)
  ...
  p = (equation.gamma - 1) * (rho_e - 0.5 * rho * ((rho_v1/rho)^2 + (rho_v2/rho)^2))
...
"""
    function flux_shima_etal(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
...
Nao Shima, Yuichi Kuya, Yoshiharu Tamaki, Soshi Kawai (JCP 2020)
  ...
  compressible flows
"""
# @inline function flux_shima_etal(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
...
"""
    flux_kennedy_gruber(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
...
"""
# @inline function flux_kennedy_gruber(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
#   # Unpack left and right state
#   rho_ll, rho_v1_ll, rho_v2_ll, rho_e_ll = u_ll
#   rho_rr, rho_v1_rr, rho_v2_rr, rho_e_rr = u_rr
...
"""
    flux_chandrashekar(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
...
"""
    flux_ranocha(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
...
"""
# @inline function flux_ranocha(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
#   # Unpack left and right state
#   rho_ll, rho_v1_ll, rho_v2_ll, rho_e_ll = u_ll
#   rho_rr, rho_v1_rr, rho_v2_rr, rho_e_rr = u_rr
...
# function flux_hll(u_ll, u_rr, orientation, equation::CompressibleEulerMulticomponentEquations2D)
#   # Calculate primitive variables and speed of sound
#   rho_ll, rho_v1_ll, rho_v2_ll, rho_e_ll = u_ll
#   rho_rr, rho_v1_rr, rho_v2_rr, rho_e_rr = u_rr
...
# This Method has to be adapted completely for multicomponent!
...
# @inline energy_total(cons, ::CompressibleEulerMulticomponentEquations2D) = cons[4]


# Calculate kinetic energy for a conservative state `cons`
# @inline function energy_kinetic(cons, equation::CompressibleEulerMulticomponentEquations2D)
#   return 0.5 * (cons[2]^2 + cons[3]^2)/cons[1]
# end


# Calculate internal energy for a conservative state `cons`
# @inline function energy_internal(cons, equation::CompressibleEulerMulticomponentEquations2D)
