...
layer ``h_1`` and the respecitve velocities ``v_1`` and ``v_2``. The gravitational constant is 
denoted by `g`, the layer densitites by ``\rho_1``and ``\rho_2`` and the (possibly) variable bottom 
topography function ``b(x)``. Conservative variable water height ``h_2`` is measured from the bottom
topography ``b`` and ``h_1`` relative to ``h_2``, therefore one also defines the total water heights
as ``H2 = h2 + b`` and ``H1 = h1 + h2 + b``.
...
@inline function source_terms_convergence_test(u, x, t, equations::TwoLayerShallowWaterEquations2D)
  ...
  du2 = (5.0(-0.1ω*cos(t + ω*x[1])*cos(t + ω*x[2]) - 0.1ω*sin(t + ω*x[1])*sin(t + ω*x[2]))*(4.0 +
        0.2cos(t + ω*x[1])*sin(t + ω*x[2]) - 0.2sin(t + ω*x[1])*cos(t + ω*x[2])) +
        0.008999999999999994ω*cos(t + ω*x[1])*cos(t + ω*x[2]) + 0.008999999999999994ω*sin(t +
        ω*x[1])*sin(t + ω*x[2]) + 0.1ω*(20.0 + cos(t + ω*x[1])*sin(t + ω*x[2]) - sin(t +
        ω*x[1])*cos(t + ω*x[2]))*cos(t + ω*x[1])*cos(t + ω*x[2]))
  du3 = (5.0(0.1ω*cos(t + ω*x[1])*cos(t + ω*x[2]) + 0.1ω*sin(t + ω*x[1])*sin(t + ω*x[2]))*(4.0 +
        0.2cos(t + ω*x[1])*sin(t + ω*x[2]) - 0.2sin(t + ω*x[1])*cos(t + ω*x[2])) +
        0.009999999999999995ω*cos(t + ω*x[1])*cos(t + ω*x[2]) + 0.009999999999999995ω*sin(t +
        ω*x[1])*sin(t + ω*x[2]) - 0.1ω*(20.0 + cos(t + ω*x[1])*sin(t + ω*x[2]) - sin(t +
...
that contains the gradient of the bottom topography [`TwoLayerShallowWaterEquations2D`](@ref).
...
@inline function flux_nonconservative_wintermeyer_etal(u_ll, u_rr, orientation::Integer,
  ...
  # Bottom gradient nonconservative term: (0, g*h1*(b+h2)_x, g*h1*(b+h2)_y , 0, g*h2*(b+r*h1)_x, g*h2*(b+h1)_y, 0)
  if orientation == 1
    f = SVector(z,
...
the gradients of the bottom topography and the layer heights [`TwoLayerShallowWaterEquations2D`](@ref).
...
@inline function flux_nonconservative_fjordholm_etal(u_ll, u_rr, orientation::Integer,
  ...
  # Bottom gradient nonconservative term: (0, g*h1*(b+h2)_x, g*h1*(b+h2)_y, 0, g*h2*(b+r*h1)_x,
  #                                        g*h2*(b+r*h1)_x, 0)
...
# !! In contrast to general usage the entropy variables are denoted with q instead of w, because 
# w is already in use for speeds in y-Direction !!
...
@inline function energy_total(cons, equations::TwoLayerShallowWaterEquations2D)
  ...
  e = 0.5 * ρ1 * (h1_v1^2 / h1 + h1_w1^2 / h1 + g * h1^2) + 0.5 * ρ2 * (h2_v2^2 / h2 + h2_w2^2 / h2 + g * h2^2) + g*ρ2*h2*b + g*ρ1*h1*(h2 + b)
  return e
end
...
@inline function energy_kinetic(u, equations::TwoLayerShallowWaterEquations2D)
  ...
  0.5 * ρ1 * h1_v1^2 / h1 + 0.5 * ρ1 * h1_w1^2 / h1 + 0.5 * ρ2 * h2_v2^2 / h2 + 0.5 * ρ2 * h2_w2^2 / h2
  return 
end
