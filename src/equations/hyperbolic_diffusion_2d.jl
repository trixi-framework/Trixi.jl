...
@inline function max_abs_speed_naive(u_ll, u_rr, orientation::Integer,
                                     equations::HyperbolicDiffusionEquations2D)
    NaNMath.sqrt(equations.nu * equations.inv_Tr)
end

@inline function max_abs_speed_naive(u_ll, u_rr, normal_direction::AbstractVector,
                                     equations::HyperbolicDiffusionEquations2D)
    NaNMath.sqrt(equations.nu * equations.inv_Tr) * norm(normal_direction)
end
...
@inline function flux_godunov(u_ll, u_rr, normal_direction::AbstractVector,
    ...
    f1 = 1 / 2 * (f_ll[1] + f_rr[1]) -
         ...
         NaNMath.sqrt(normal_direction[1]^2 + normal_direction[2]^2)
    f2 = 1 / 2 * (f_ll[2] + f_rr[2]) -
...
@inline function max_abs_speeds(eq::HyperbolicDiffusionEquations2D)
    λ = NaNMath.sqrt(eq.nu * eq.inv_Tr)
    return λ, λ
end
