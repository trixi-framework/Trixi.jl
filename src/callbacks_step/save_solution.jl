...
@muladd begin
    ...
    function Base.show(io::IO, cb::DiscreteCallback{<:Any, <:SaveSolutionCallback})
        ...
        print(
            io, "SaveSolutionCallback(interval=", save_solution_callback.interval_or_dt,
            ")"
        )
    end
    ...
    function (solution_callback::SaveSolutionCallback)(integrator)
        ...
        @trixi_timeit timer() "I/O" begin
            ...
            @trixi_timeit timer() "save mesh" save_mesh(
                semi,
    ...
    @inline function save_solution_file(
            semi::AbstractSemidiscretization, u_ode,
            solution_callback,
            integrator; system = ""
        )
            ...
            if callbacks isa CallbackSet
                foreach(callbacks.continuous_callbacks) do cb
                    get_element_variables!(
                        element_variables, u_ode, semi, cb;
        ...
        @trixi_timeit timer() "get node variables" get_node_variables!(
            node_variables,
            semi
        )

        @trixi_timeit timer() "save solution" save_solution_file(
            u_ode, t, dt, iter, semi,
            solution_callback,
            element_variables,
            node_variables,
            system = system
    ...
    @inline function save_solution_file(
        ...
        save_solution_file(
            u, t, dt, iter, mesh, equations, solver, cache,
            solution_callback,
            element_variables,
