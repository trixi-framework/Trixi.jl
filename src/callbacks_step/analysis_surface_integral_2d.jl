...
@muladd begin
    ...
    function (lift_coefficient::LiftCoefficientPressure)(
            u, normal_direction, x, t,
            equations
        )
    ...
    function (drag_coefficient::DragCoefficientPressure)(
            u, normal_direction, x, t,
            equations
        )
        p = pressure(u, equations)
    ...
    function viscous_stress_tensor(
            u, normal_direction, equations_parabolic,
            gradients_1, gradients_2
        )
        _, dv1dx, dv2dx, _ = convert_derivative_to_primitive(
            u, gradients_1,
            equations_parabolic
        )
        _, dv1dy, dv2dy, _ = convert_derivative_to_primitive(
            u, gradients_2,
            equations_parabolic
        )
    ...
    function viscous_stress_vector(
            u, normal_direction, equations_parabolic,
            gradients_1, gradients_2
        )
        ...
        tau_11, tau_12, tau_22 = viscous_stress_tensor(
            u, normal_direction,
            equations_parabolic,
            gradients_1, gradients_2
        )
    ...
    function (lift_coefficient::LiftCoefficientShearStress)(
            u, normal_direction, x, t,
            equations_parabolic,
            gradients_1, gradients_2
        )
        visc_stress_vector = viscous_stress_vector(
            u, normal_direction, equations_parabolic,
            gradients_1, gradients_2
        )
        @unpack psi, rhoinf, uinf, linf = lift_coefficient.force_state
    ...
    function (drag_coefficient::DragCoefficientShearStress)(
            u, normal_direction, x, t,
            equations_parabolic,
            gradients_1, gradients_2
        )
    ...
    function analyze(
            surface_variable::AnalysisSurfaceIntegral, du, u, t,
            mesh::P4estMesh{2},
        ...
        for boundary in indices
            ...
            for node_index in index_range
                u_node = Trixi.get_node_vars(
                    cache.boundaries.u, equations, dg, node_index,
                    boundary
                )
                ...
                x = get_node_coords(
                    node_coordinates, equations, dg, i_node, j_node,
                    element
                )
    ...
    function analyze(
            surface_variable::AnalysisSurfaceIntegral{Variable},
            du, u, t, mesh::P4estMesh{2},
            equations, equations_parabolic,
            dg::DGSEM, cache, semi,
            cache_parabolic
        ) where {Variable <: VariableViscous}
        @unpack boundaries = cache
        @unpack surface_flux_values, node_coordinates, contravariant_vectors = cache.elements
        @unpack weights = dg.basis
        ...
        for boundary in indices
            ...
            for node_index in index_range
                ...
                gradients_1 = get_node_vars(
                    gradients_x, equations_parabolic, dg, i_node,
                    j_node, element
                )
                gradients_2 = get_node_vars(
                    gradients_y, equations_parabolic, dg, i_node,
                    j_node, element
                )
                ...
                # function `variable` and the division of the normal scaling factor
                # `norm(normal_direction)` is then accounted for with the `dS` quantity.
                surface_integral += variable(
                    u_node, normal_direction, x, t,
                    equations_parabolic,
                    gradients_1, gradients_2
                ) * dS
