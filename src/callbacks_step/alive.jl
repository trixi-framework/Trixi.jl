...
@muladd begin
    ...
    function AliveCallback(;
        ...
        DiscreteCallback(
            alive_callback, alive_callback, # the first one is the condition, the second the affect!
    ...
    function (alive_callback::AliveCallback)(integrator)
        ...
        if isfinished(integrator) && mpi_isroot()
            println("─"^100)
            println(
                "Trixi.jl simulation finished.  Final time: ", integrator.t,
