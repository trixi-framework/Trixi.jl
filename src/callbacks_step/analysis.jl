...
@muladd begin
    ...
    mutable struct AnalysisCallback{
            Analyzer, AnalysisIntegrals, InitialStateIntegrals,
            Cache,
        }
        start_time::Float64
    ...
    function AnalysisCallback(
            mesh, equations::AbstractEquations, solver, cache;
            interval = 0,
            save_analysis = false,
            output_directory = "out",
            analysis_filename = "analysis.dat",
            extra_analysis_errors = Symbol[],
            analysis_errors = union(
        ...
        cache_analysis = create_cache_analysis(
            analyzer, mesh, equations, solver, cache,
            RealT, uEltype
        )
    ...
    function initialize!(
            cb::DiscreteCallback{Condition, Affect!}, u_ode, t,
            integrator
    ...
    function initialize!(
            cb::DiscreteCallback{Condition, Affect!}, u_ode, du_ode, t,
            integrator, semi
        ) where {Condition, Affect! <: AnalysisCallback}
    ...
    function (analysis_callback::AnalysisCallback)(u_ode, du_ode, integrator, semi)
        ...
        @trixi_timeit timer() "analyze solution" begin
            ...
            mpi_println("─"^100)
            mpi_println(
                " Simulation running '", get_name(equations), "' with ",
                summary(solver)
            )
            mpi_println("─"^100)
            mpi_println(
    ...
    function (analysis_callback::AnalysisCallback)(io, du, u, u_ode, t, semi)
        ...
        if :l2_error_primitive in analysis_errors ||
            ...
            l2_error_prim, linf_error_prim = calc_error_norms(
                cons2prim, u_ode, t, analyzer,
                semi, cache_analysis
            )
    ...
    function print_amr_information(callbacks, mesh, solver, cache)
        ...
        elements_per_level = get_elements_per_level(
            min_level, max_level, mesh, solver,
            cache
        )
    ...
    function get_elements_per_level(min_level, max_level, mesh::TreeMesh, solver, cache)
        levels = [
            mesh.tree.levels[cache.elements.cell_ids[element]]
    ...
    function analyze_integrals(
            analysis_integrals::NTuple{N, Any}, io, du, u, t,
            semi
        ) where {N}
    ...
    function (cb::DiscreteCallback{Condition, Affect!})(sol) where {
        ...
        l2_error, linf_error = calc_error_norms(
            sol.u[end], sol.t[end], analyzer, semi,
            cache_analysis
        )
    ...
    function analyze(
            quantity::typeof(enstrophy), du, u, t,
        ...
        analyze(
            quantity, du, u, t, mesh, equations, equations_parabolic, solver, cache,
            cache_parabolic
        )
    end
    function analyze(
            quantity, du, u, t, mesh, equations, equations_parabolic, solver,
            cache, cache_parabolic
        )
        integrate(
            quantity, u, mesh, equations, equations_parabolic, solver, cache,
...
# in the case of AMR.
function analyze(
        quantity::AnalysisSurfaceIntegral, du, u, t,
...
function analyze(
        quantity::AnalysisSurfaceIntegral{Variable},
        du, u, t,
        semi::SemidiscretizationHyperbolicParabolic
    ) where {
        Variable <:
    ...
    cache_parabolic = semi.cache_parabolic
    analyze(
        quantity, du, u, t, mesh, equations, equations_parabolic, solver, cache, semi,
        cache_parabolic
    )
end
