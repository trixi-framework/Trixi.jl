...
@muladd begin
    ...
    function save_solution_file(
        ...
        if HDF5.has_parallel()
            save_solution_file_parallel(
                data, time, dt, timestep, n_vars, mesh, equations,
    ...
    function save_solution_file_parallel(
        ...
        cum_element_counts = append!(
            zeros(eltype(element_counts), 1),
        ...
        h5open(filename, "w", mpi_comm()) do file
            ...
            for v in 1:n_vars
                ...
                var = create_dataset(
                    file, "/variables_$v", datatype(eltype(data)),
    ...
    function save_solution_file_on_root(
        ...
        h5open(filename, "w") do file
            ...
            for v in 1:n_vars
                ...
                MPI.Gatherv!(
                    vec(data[v, .., :]), MPI.VBuffer(recv, node_counts),
                    mpi_root(), mpi_comm()
                )
            ...
            for (v, (key, element_variable)) in enumerate(element_variables)
                ...
                MPI.Gatherv!(
                    element_variable, MPI.VBuffer(recv, element_counts),
                    mpi_root(), mpi_comm()
                )
