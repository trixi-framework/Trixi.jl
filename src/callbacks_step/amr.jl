...
@muladd begin
    ...
    """
    uses_amr(cb) = false
    function uses_amr(cb::DiscreteCallback{Condition, Affect!}) where {
            Condition,
            Affect! <:
            AMRCallback,
        }
    ...
    function get_element_variables!(
            element_variables, u, mesh, equations, solver, cache,
    ...
    function initialize!(
            cb::DiscreteCallback{Condition, Affect!}, u, t,
            integrator
    ...
    @inline function (amr_callback::AMRCallback)(
        ...
        # it when doing AMR while still dispatching on the `mesh` etc.
        amr_callback(
            u_ode, mesh_equations_solver_cache(semi)..., semi.cache_parabolic,
            semi, t, iter; kwargs...
        )
    end
    ...
    function (amr_callback::AMRCallback)(
            u_ode::AbstractVector, mesh::TreeMesh,
        ...
        @trixi_timeit timer() "refine" if !only_coarsen && !isempty(to_refine)
            # refine mesh
            refined_original_cells = @trixi_timeit timer() "mesh" refine!(
                mesh.tree,
                to_refine
            )

            # Find all indices of elements whose cell ids are in refined_original_cells
            elements_to_refine = findall(
                in(refined_original_cells),
                cache.elements.cell_ids
            )

            # refine solver
            @trixi_timeit timer() "solver" refine!(
                u_ode, adaptor, mesh, equations, dg,
        ...
        @trixi_timeit timer() "coarsen" if !only_refine && !isempty(to_coarsen)
            ...
            coarsened_original_cells = @trixi_timeit timer() "mesh" coarsen!(
                mesh.tree,
                to_coarsen
            )
    ...
    function (amr_callback::AMRCallback)(
            u_ode::AbstractVector, mesh::P4estMesh,
    ...
    function cfunction(::typeof(copy_to_quad_iter_volume), ::Val{2})
        @cfunction(
            copy_to_quad_iter_volume, Cvoid,
    ...
    function (amr_callback::AMRCallback)(
            u_ode::AbstractVector, mesh::T8codeMesh,
            equations, dg::DG, cache, semi,
            t, iter;
            only_refine = false, only_coarsen = false,
            passive_args = ()
        ...
        u = wrap_array(u_ode, mesh, equations, dg, cache)
        indicators = @trixi_timeit timer() "indicator" controller(
            u, mesh, equations, dg,
            cache, t = t, iter = iter
        )
        ...
        @trixi_timeit timer() "adapt" begin
            ...
            if has_changed
                @trixi_timeit timer() "solver" adapt!(
                    u_ode, adaptor, mesh, equations, dg,
                    cache, difference
                )
            end
    ...
    function get_element_variables!(
            element_variables, indicator::AbstractIndicator,
            ::AMRCallback
        )
    ...
    function cfunction(::typeof(extract_levels_iter_volume), ::Val{2})
        @cfunction(
            extract_levels_iter_volume, Cvoid,
            (Ptr{p4est_iter_volume_info_t}, Ptr{Cvoid})
        )
    end
    # 3D
    ...
    struct ControllerThreeLevelCombined{
            RealT <: Real, IndicatorPrimary, IndicatorSecondary,
            Cache,
        }
        base_level::Int
        med_level::Int
    ...
    function ControllerThreeLevelCombined(
        ...
        med_threshold, max_threshold, max_threshold_secondary = promote(
            med_threshold,
            max_threshold,
        ...
        ControllerThreeLevelCombined{
            typeof(max_threshold), typeof(indicator_primary),
            typeof(indicator_secondary), typeof(cache),
        }(
            base_level,
            med_level,
            max_level,
    ...
    function get_element_variables!(
        ...
        get_element_variables!(
            element_variables, controller.indicator_primary,
            amr_callback
        )
    end
