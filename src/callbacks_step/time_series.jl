...
@muladd begin
    ...
    mutable struct TimeSeriesCallback{
            RealT <: Real, uEltype <: Real, SolutionVariables,
            VariableNames, Cache,
        }
        interval::Int
    ...
    function TimeSeriesCallback(
            mesh, equations, solver, cache, point_coordinates;
            interval::Integer = 1,
        ...
        time = Vector{RealT}()
        step = Vector{Int}()
        time_series_cache = create_cache_time_series(
            point_coordinates_, mesh, solver,
            cache
        )
        ...
        return DiscreteCallback(
            condition, time_series_callback,
    ...
    function TimeSeriesCallback(
        ...
        point_coordinates_ = Matrix{eltype(eltype(point_coordinates))}(
            undef, n_points,
            ndims(mesh)
        )
        ...
        return TimeSeriesCallback(
            mesh, equations, solver, cache, point_coordinates_;
            kwargs...
        )
    end
    ...
    function (time_series_callback::TimeSeriesCallback)(integrator)
        ...
        if integrator.stats.naccept % interval == 0
            @trixi_timeit timer() "time series" begin
                ...
                @unpack (
                    point_data, solution_variables,
                    variable_names, time_series_cache,
