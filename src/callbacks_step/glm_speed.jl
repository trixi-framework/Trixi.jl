...
@muladd begin
    ...
    function Base.show(io::IO, cb::DiscreteCallback{<:Any, <:GlmSpeedCallback})
        ...
        print(
            io, "GlmSpeedCallback(glm_scale=", glm_scale, ", cfl=", cfl, "semi_indices=",
            semi_indices, ")"
        )
    end
    ...
    function GlmSpeedCallback(; glm_scale = 0.5, cfl, semi_indices = Int[])
        ...
        DiscreteCallback(
            glm_speed_callback, glm_speed_callback, # the first one is the condition, the second the affect!
