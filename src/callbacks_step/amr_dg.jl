...
function rebalance_solver!(u_ode::AbstractVector, mesh::ParallelP4estMesh, equations,
  ...
  global_first_quadrant = unsafe_wrap(Array, unsafe_load(mesh.p4est).global_first_quadrant,
                                      mpi_nranks() + 1)
    ...
    @trixi_timeit timer() "exchange data" begin
      ...
      for old_element_id in 1:old_n_elements
        ...
        if !(global_first_quadrant[mpi_rank() + 1] <= global_quad_id <
          ...
          request = MPI.Isend(@view(old_u[:, .., old_element_id]), dest, global_quad_id,
                              mpi_comm())
      ...
      for element in eachelement(dg, cache)
        ...
        if old_global_first_quadrant[mpi_rank() + 1] <= global_quad_id <
          ...
          src = findfirst(r -> old_global_first_quadrant[r] <= global_quad_id <
