...
@muladd begin
    ...
    function calc_error_norms(
            ...
            dg::DGMulti{NDIMS}, cache, cache_analysis
        ) where {NDIMS}
        rd = dg.basis
        md = mesh.md
    ...
    function integrate(
            func::Func, u,
            mesh::DGMultiMesh,
            equations, dg::DGMulti, cache; normalize = true
        ) where {Func}
        rd = dg.basis
        md = mesh.md
        @unpack u_values = cache
    ...
    function compute_local_divergence!(
            local_divergence, element, vector_field,
        ...
        for i in eachdim(mesh)
            for j in eachdim(mesh)
                ...
                mul!(
                    local_divergence, jth_ref_derivative_matrix, vector_field[i],
    ...
    function integrate(
        ...
        local_gradient_quadrature_values = ntuple(
            _ -> similar(cache_parabolic.local_u_values_threaded),
            3
        )
        ...
        for e in eachelement(mesh, dg)
            ...
            apply_to_each_field(
                mul_by!(dg.basis.Vq), gradient_x_quadrature_values,
                view(gradients_x, :, e)
            )
            apply_to_each_field(
                mul_by!(dg.basis.Vq), gradient_y_quadrature_values,
                view(gradients_y, :, e)
            )
            apply_to_each_field(
                mul_by!(dg.basis.Vq), gradient_z_quadrature_values,
                view(gradients_z, :, e)
            )
