...
@muladd begin
    ...
    function max_dt(
            u, t, mesh::Union{StructuredMesh{3}, P4estMesh{3}, T8codeMesh{3}},
            constant_speed::False, equations, dg::DG, cache
        )
        # to avoid a division by zero if the speed vanishes everywhere,
        # e.g. for steady-state linear advection
    ...
    function max_dt(
        ...
        for element in eachelement(dg, cache)
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                ...
                lambda1_transformed = abs(
                    Ja11 * max_lambda1 + Ja12 * max_lambda2 +
                        Ja13 * max_lambda3
                )
                Ja21, Ja22, Ja23 = get_contravariant_vector(
                    2, contravariant_vectors, i, j,
                ...
                lambda2_transformed = abs(
                    Ja21 * max_lambda1 + Ja22 * max_lambda2 +
                        Ja23 * max_lambda3
                )
                Ja31, Ja32, Ja33 = get_contravariant_vector(
                    3, contravariant_vectors, i, j,
                ...
                lambda3_transformed = abs(
                    Ja31 * max_lambda1 + Ja32 * max_lambda2 +
                        Ja33 * max_lambda3
                )
    ...
    function max_dt(
        ...
        dt = invoke(
            max_dt,
            Tuple{
                typeof(u), typeof(t), T8codeMesh{3},
                typeof(constant_speed), typeof(equations), typeof(dg),
                typeof(cache),
            },
            u, t, mesh, constant_speed, equations, dg, cache
