...
@muladd begin
    ...
    function create_cache(
            ::Type{AveragingCallback}, mesh::TreeMesh{2},
    ...
    function calc_mean_values!(
            mean_values, averaging_callback_cache, u, u_prev,
        ...
        @threaded for element in eachelement(dg, cache)
            for j in eachnode(dg), i in eachnode(dg)
                vorticity = calc_vorticity_node(
                    u, mesh, equations, dg, cache, i, j,
                    element
                )
                ...
                u_node_prim = cons2prim(
                    get_node_vars(u, equations, dg, i, j, element),
                    equations
                )
                u_prev_node_prim = cons2prim(
                    get_node_vars(
                        u_prev, equations, dg, i, j,
                        element
                    ), equations
                )
