...
@muladd begin
    ...
    function StepsizeCallback(; cfl::Real = 1.0)
        ...
        DiscreteCallback(
            stepsize_callback, stepsize_callback, # the first one is the condition, the second the affect!
    ...
    @inline function (stepsize_callback::StepsizeCallback)(integrator)
        ...
        if !integrator.opts.adaptive
            ...
            dt = @trixi_timeit timer() "calculate dt" calculate_dt(
                u_ode, t, cfl_number,
                semi
            )
    ...
    function (cb::DiscreteCallback{Condition, Affect!})(ode::ODEProblem) where {
            Condition,
            Affect! <:
            StepsizeCallback,
        }
