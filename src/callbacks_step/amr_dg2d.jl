...
@muladd begin
    ...
    function rebalance_solver!(
            u_ode::AbstractVector, mesh::TreeMesh{2}, equations,
            ...
            @trixi_timeit timer() "exchange data" begin
                ...
                for old_element_id in 1:old_n_elements
                    ...
                    if !(cell_id in leaf_cell_ids)
                        ...
                        request = MPI.Isend(
                            @view(old_u[:, .., old_element_id]), dest,
                            cell_id, mpi_comm()
                        )
    ...
    function refine!(
            u_ode::AbstractVector, adaptor, mesh::Union{TreeMesh{2}, P4estMesh{2}},
        ...
        GC.@preserve old_u_ode old_inverse_jacobian begin
            ...
            for old_element_id in 1:old_n_elements
                if needs_refinement[old_element_id]
                    ...
                    refine_element!(
                        u, element_id, old_u, old_element_id,
    ...
    function refine_element!(
            u::AbstractArray{<:Any, 4}, element_id,
    ...
    function adapt!(
            u_ode::AbstractVector, adaptor, mesh::T8codeMesh{2}, equations,
        ...
        GC.@preserve old_u_ode begin
            ...
            while old_index <= old_nelems && new_index <= new_nelems
                ...
                elseif difference[old_index] < 0 # Coarsen.
                    ...
                    for v in eachvariable(equations)
                        u[v, .., new_index] .*= (
                            4 .* cache.elements.inverse_jacobian[
                                ..,
                                new_index,
                            ]
                        )
    ...
    function create_cache(
            ::Type{ControllerThreeLevelCombined}, mesh::TreeMesh{2},
