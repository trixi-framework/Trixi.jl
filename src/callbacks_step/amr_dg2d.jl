    ...
    for v in 1:_nvariables
        for j in 1:_nnodes, i in 1:_nnodes
            ...
            for l in 1:_nnodes, k in 1:_nnodes
                acc += old_u[v, k, l, lower_left_id] * reverse_lower[i, k] *
                       reverse_lower[j, l]
            end

            for l in 1:_nnodes, k in 1:_nnodes
                acc += old_u[v, k, l, lower_right_id] * reverse_upper[i, k] *
                       reverse_lower[j, l]
            end

            for l in 1:_nnodes, k in 1:_nnodes
                acc += old_u[v, k, l, upper_left_id] * reverse_lower[i, k] *
                       reverse_upper[j, l]
            end

            for l in 1:_nnodes, k in 1:_nnodes
                acc += old_u[v, k, l, upper_right_id] * reverse_upper[i, k] *
                       reverse_upper[j, l]
            end
...
function refine!(::Nothing, elements_to_refine, adaptor,
                 ...
                 equations, dg, cache, u_ode, limiter!)
