...
@muladd begin
    ...
    function get_elements_by_coordinates!(
        ...
        for index in eachindex(element_ids)
            ...
            distances, indices = calc_minimum_surface_distance(
                point,
            ...
            candidates = findall(
                abs.(minimum(distances) .- distances) .<
            ...
            for element in eachindex(candidates)
                bary_center = SVector(
                    bary_centers[1, candidates[element]],
    ...
    @inline function calc_bary_centers!(bary_centers, dg, cache)
        ...
        @views for element in eachelement(dg, cache)
            bary_centers[1, element] = sum(
                cache.elements.node_coordinates[
                    1, :, :,
                    element,
                ]
            ) / n^2
            bary_centers[2, element] = sum(
                cache.elements.node_coordinates[
                    2, :, :,
                    element,
                ]
    ...
    function calc_minimum_surface_distance(
    ...
    function calc_interpolating_polynomials!(
            interpolating_polynomials, coordinates,
            element_ids,
        ...
        for index in eachindex(element_ids)
            ...
            if !mesh.element_is_curved[element]
                ...
                x_check = straight_side_quad_map(
                    unit_coordinates[1], unit_coordinates[2],
                    corners
                )
                ...
                unit_coordinates = invert_transfinite_interpolation(
                    mesh, x,
                    view(
                        mesh.surface_curves,
                        :, element
    ...
    @inline function invert_bilinear_interpolation(
            mesh::UnstructuredMesh2D, point,
            element_corners
        )
    ...
    @inline function invert_transfinite_interpolation(
            mesh::UnstructuredMesh2D, point,
    ...
    function record_state_at_points!(
            point_data, u, solution_variables,
