...
struct SemidiscretizationHyperbolic{Mesh,Equations,InitialCondition,BoundaryConditions,
  ...
  function SemidiscretizationHyperbolic{Mesh,Equations,InitialCondition,
                                        BoundaryConditions,SourceTerms,Solver,Cache}(mesh::Mesh,
                                                                                     equations::Equations,
                                                                                     initial_condition::InitialCondition,
                                                                                     boundary_conditions::BoundaryConditions,
                                                                                     source_terms::SourceTerms,
...
function SemidiscretizationHyperbolic(mesh, equations, initial_condition, solver;
  ...
  SemidiscretizationHyperbolic{typeof(mesh),typeof(equations),typeof(initial_condition),
...
function remake(semi::SemidiscretizationHyperbolic; uEltype=real(semi.solver),
  ...
  SemidiscretizationHyperbolic(mesh, equations, initial_condition, solver; source_terms,
                               boundary_conditions, uEltype)
end
...
function Base.show(io::IO, ::MIME"text/plain", semi::SemidiscretizationHyperbolic)
  ...
  if get(io, :compact, false)
    ...
    summary_line(io, "equations", nameof(typeof(semi.equations)))
    summary_line(io, "initial condition", semi.initial_condition)

    print_boundary_conditions(io, semi)

    summary_line(io, "source terms", semi.source_terms)
    summary_line(io, "solver", nameof(typeof(semi.solver)))
...
const SemiHypMeshBCSolver{Mesh,BoundaryConditions,Solver} = SemidiscretizationHyperbolic{Mesh,
                                                                                         Equations,
                                                                                         InitialCondition,
                                                                                         BoundaryConditions,
                                                                                         SourceTerms,
