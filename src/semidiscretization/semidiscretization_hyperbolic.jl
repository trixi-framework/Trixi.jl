...
@muladd begin
    ...
    struct SemidiscretizationHyperbolic{
            Mesh, Equations, InitialCondition,
            BoundaryConditions,
        ...
        function SemidiscretizationHyperbolic{
            ...
            new(
                mesh, equations, initial_condition, boundary_conditions, source_terms,
    ...
    function SemidiscretizationHyperbolic(
        ...
        cache = (;
            create_cache(mesh, equations, solver, RealT, uEltype)...,
            initial_cache...,
        )
        _boundary_conditions = digest_boundary_conditions(
            boundary_conditions, mesh, solver,
            cache
        )
    ...
    function remake(
            semi::SemidiscretizationHyperbolic; uEltype = real(semi.solver),
            mesh = semi.mesh,
    ...
    function digest_boundary_conditions(
            boundary_conditions::AbstractArray, mesh, solver,
            cache
        )
    ...
    function check_periodicity_mesh_boundary_conditions(
            mesh::Union{
                P4estMesh,
                UnstructuredMesh2D,
                T8codeMesh,
    ...
    function check_periodicity_mesh_boundary_conditions(
        ...
        check_periodicity_mesh_boundary_conditions_x(
            mesh, boundary_conditions[1],
    ...
    function check_periodicity_mesh_boundary_conditions(
        ...
        check_periodicity_mesh_boundary_conditions_y(
            mesh, boundary_conditions[3],
    ...
    function check_periodicity_mesh_boundary_conditions(
        ...
        check_periodicity_mesh_boundary_conditions_z(
            mesh, boundary_conditions[5],
...
const SemiHypMeshBCSolver{Mesh, BoundaryConditions, Solver} =
    ...
    function print_boundary_conditions(io, semi::SemiHypMeshBCSolver{<:Any, <:NamedTuple})
        ...
        for (i, bc_name) in enumerate(bc_names)
            summary_line(
                increment_indent(io), String(bc_name),
    ...
    function calc_error_norms(
            func, u_ode, t, analyzer, semi::SemidiscretizationHyperbolic,
            cache_analysis
        )
        ...
        calc_error_norms(
            func, u, t, analyzer, mesh, equations, initial_condition, solver,
    ...
    function rhs!(du_ode, u_ode, semi::SemidiscretizationHyperbolic, t)
        ...
        @trixi_timeit timer() "rhs!" rhs!(
            du, u, t, mesh, equations, initial_condition,
