...
mesh and solvers with a shared basis. The coupling region is described by a function `source_region`
that maps the coordinates of a single node to `true` or `false` depending on whether the point lies
within the coupling region or not. A weighting function `weights` that maps coordinates to weights
is applied to the acoustic source terms.
