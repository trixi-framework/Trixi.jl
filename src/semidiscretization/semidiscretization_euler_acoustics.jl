...
@muladd begin
    ...
    function SemidiscretizationEulerAcoustics(
        ...
        cache = create_cache(
            SemidiscretizationEulerAcoustics, source_region, weights,
            mesh_equations_solver_cache(semi_acoustics)...
        )

        return SemidiscretizationEulerAcoustics{
            typeof(semi_acoustics), typeof(semi_euler),
            typeof(cache),
        }(
            semi_acoustics, semi_euler,
            cache
        )
    end

    function create_cache(
            ::Type{SemidiscretizationEulerAcoustics}, source_region, weights,
    ...
    function get_coupled_element_ids(source_region, equations, dg::DGSEM, cache)
        ...
        for element in eachelement(dg, cache)
            for j in eachnode(dg), i in eachnode(dg)
                x = get_node_coords(
                    cache.elements.node_coordinates, equations, dg, i, j,
                    element
                )
                if source_region(x)
    ...
    function precompute_weights(
            source_region, weights, coupled_element_ids, equations,
            dg::DGSEM, cache
        )
    ...
    function Base.show(
        ...
        if get(io, :compact, false)
            ...
            summary_line(
                io, "semidiscretization acoustics",
    ...
    function rhs!(du_ode, u_ode, semi::SemidiscretizationEulerAcoustics, t)
        ...
        @trixi_timeit timer() "add acoustic source terms" begin
            add_acoustic_source_terms!(
                du_acoustics, acoustic_source_terms,
    ...
    function add_acoustic_source_terms!(
            du_acoustics, acoustic_source_terms, source_weights,
