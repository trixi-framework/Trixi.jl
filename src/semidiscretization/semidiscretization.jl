...
@muladd begin
    ...
    function integrate_via_indices(
            func::Func, u_ode, semi::AbstractSemidiscretization,
            args...; normalize = true
        ) where {Func}
        ...
        integrate_via_indices(
            func, u, mesh, equations, solver, cache, args...,
            normalize = normalize
        )
    end
    ...
    function calc_error_norms(
            u_ode, t, analyzer, semi::AbstractSemidiscretization,
            cache_analysis
        )
    ...
    function semidiscretize(
            semi::AbstractSemidiscretization, tspan;
            reset_threads = true
        )
    ...
    function jacobian_ad_forward(semi::AbstractSemidiscretization, t0, _u0_ode::StructArray)
        u0_ode_plain = similar(
            _u0_ode, eltype(eltype(_u0_ode)),
    ...
    # handle arrays of `SVector`s.
    function jacobian_ad_forward(
            semi::AbstractSemidiscretization, t0,
