...
@muladd begin
    ...
    function AnalysisCallbackCoupled(semi_coupled, callbacks...)
        ...
        DiscreteCallback(
            condition, analysis_callback_coupled,
    ...
    function initialize!(
            cb_coupled::DiscreteCallback{Condition, Affect!}, u_ode_coupled, t,
            integrator
        ) where {Condition, Affect! <: AnalysisCallbackCoupled}
        analysis_callback_coupled = cb_coupled.affect!
    ...
    function (cb::DiscreteCallback{Condition, Affect!})(sol) where {
            Condition,
            Affect! <:
        ...
        for i in eachsystem(semi_coupled)
            ...
            l2_error, linf_error = calc_error_norms(
                u_ode, sol.t[end], analyzer, semi,
                cache_analysis
            )
    ...
    function save_mesh(semi::SemidiscretizationCoupled, output_directory, timestep = 0)
        for i in eachsystem(semi)
            ...
            if mesh.unsaved_changes
                mesh.current_filename = save_mesh_file(
                    mesh, output_directory; system = i,
                    timestep = timestep
                )
    ...
    @inline function save_solution_file(
            semi::SemidiscretizationCoupled, u_ode,
            solution_callback,
            integrator
        )
        @unpack semis = semi
        ...
        for i in eachsystem(semi)
            ...
            save_solution_file(
                semis[i], u_ode_slice, solution_callback, integrator,
                system = i
            )
        end
    end
    ...
    function update_cleaning_speed!(
        ...
        for (semi_index, semi) in enumerate(semi_coupled.semis)
            if (
                    typeof(semi.equations) <: AbstractIdealGlmMhdEquations &&
    ...
    mutable struct BoundaryConditionCoupled{
        ...
        function BoundaryConditionCoupled(
                other_semi_index, indices, uEltype,
                coupling_converter
            )
            ...
            new{
                NDIMS, other_semi_index, NDIMS * 2 - 1, uEltype, typeof(indices),
                typeof(coupling_converter),
            }(
                u_boundary,
                other_orientation,
    ...
    function (boundary_condition::BoundaryConditionCoupled)(
            u_inner, orientation, direction,
            cell_indices,
        ...
        u_boundary = SVector(
            ntuple(
                v -> boundary_condition.u_boundary[
                    v,
                    surface_node_indices...,
                    cell_indices...,
                ],
        ...
        if surface_flux_function isa Tuple
            ...
            if iseven(direction) # u_inner is "left" of boundary, u_boundary is "right" of boundary
                flux = (
                    surface_flux_function[1](
                        u_inner, u_boundary, orientation,
                        equations
                    ) +
                        0.5f0 *
                        surface_flux_function[2](
                        u_inner, u_boundary, orientation,
                        equations
                    )
                )
                ...
                flux = (
                    surface_flux_function[1](
                        u_boundary, u_inner, orientation,
                        equations
                    ) +
                        0.5f0 *
                        surface_flux_function[2](
    ...
    function allocate_coupled_boundary_conditions(semi::AbstractSemidiscretization)
        ...
        for direction in 1:n_boundaries
            ...
            allocate_coupled_boundary_condition(
    ...
    function allocate_coupled_boundary_condition(
            boundary_condition, direction, mesh,
            equations,
            solver
        )
    ...
    # In 2D
    function allocate_coupled_boundary_condition(
            boundary_condition::BoundaryConditionCoupled{2},
        ...
        boundary_condition.u_boundary = Array{uEltype, 3}(
    ...
    function copy_to_coupled_boundary!(
            u_ode, semi_coupled, semi, i, n_boundaries,
        ...
        if i < n_boundaries
            copy_to_coupled_boundary!(
                u_ode, semi_coupled, semi, i + 1, n_boundaries,
                boundary_conditions...
            )
        end
    end
    ...
    function copy_to_coupled_boundary!(
        ...
        copy_to_coupled_boundary!(
            u_ode, semi_coupled, semi, 1, length(boundary_conditions),
            boundary_conditions...
        )
    end
    ...
    function copy_to_coupled_boundary!(
        ...
        u_other = wrap_array(
            u_ode_other, mesh_other, equations_other, solver_other,
            cache_other
        )
        ...
        @threaded for i in eachindex(cells)
            ...
            for element_id in eachnode(solver_other)
                ...
                u_node_other = get_node_vars(
                    u_other, equations_other, solver_other, i_node,
                    j_node, linear_indices[i_cell, j_cell]
                )
                u_node_converted = coupling_converter(
                    x_other, u_node_other,
                    equations_other,
                    equations_own
                )
    ...
    @inline function calc_boundary_flux_by_direction!(
        ...
        normal = sign_jacobian *
            ...
            orientation, contravariant_vectors,
        ...
        flux = sign_jacobian * boundary_condition(
            u_inner, normal, direction, cell_indices,
