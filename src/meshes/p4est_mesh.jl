...
@muladd begin
    ...
    mutable struct P4estMesh{NDIMS, RealT <: Real, IsParallel, P, Ghost, NDIMSP2, NNODES} <:
        ...
        function P4estMesh{NDIMS}(
            ...
            mesh = new{
                NDIMS, eltype(tree_node_coordinates), typeof(is_parallel),
                typeof(p4est_pw), typeof(ghost_pw), NDIMS + 2, length(nodes),
            }(
                p4est_pw,
                is_parallel,
                ghost_pw,
    ...
    function P4estMesh(
        ...
        calc_tree_node_coordinates!(
            tree_node_coordinates, nodes, mapping,
            trees_per_dimension
        )
        ...
        return P4estMesh{NDIMS}(
            p4est, tree_node_coordinates, nodes,
    ...
    function structured_boundary_names!(
            boundary_names, trees_per_dimension::NTuple{2},
            periodicity
        )
    ...
    # 3D version
    function structured_boundary_names!(
            boundary_names, trees_per_dimension::NTuple{3},
            periodicity
        )
        linear_indices = LinearIndices(trees_per_dimension)
    ...
    function P4estMesh{NDIMS}(
        ...
        if header == " File created by HOHQMesh"
            ...
            p4est, tree_node_coordinates, nodes, boundary_names = p4est_mesh_from_hohqmesh_abaqus(
                meshfile,
            ...
            p4est, tree_node_coordinates, nodes, boundary_names = p4est_mesh_from_standard_abaqus(
                meshfile,
                mapping,
                polydeg,
                initial_refinement_level,
    ...
    function p4est_mesh_from_hohqmesh_abaqus(
            meshfile, initial_refinement_level,
            n_dimensions, RealT
        )
        connectivity, tree_node_coordinates, nodes, boundary_names = p4est_connectivity_from_hohqmesh_abaqus(
            meshfile,
            initial_refinement_level,
            n_dimensions,
            RealT
    ...
    function p4est_mesh_from_standard_abaqus(
            meshfile, mapping, polydeg,
            initial_refinement_level, n_dimensions, RealT,
            boundary_symbols
        )
        connectivity, tree_node_coordinates, nodes, boundary_names = p4est_connectivity_from_standard_abaqus(
            meshfile,
            mapping,
            polydeg,
            initial_refinement_level,
    ...
    # [`HOHQMesh.jl`](https://github.com/trixi-framework/HOHQMesh.jl).
    function p4est_connectivity_from_hohqmesh_abaqus(
            meshfile, initial_refinement_level,
            n_dimensions, RealT
        )
        ...
        file_idx = calc_tree_node_coordinates!(
            tree_node_coordinates, file_lines, nodes,
            vertices, RealT
        )
    ...
    function p4est_connectivity_from_standard_abaqus(
            meshfile, mapping, polydeg,
        ...
        vertices = unsafe_wrap(Array, connectivity_pw.vertices, (3, n_vertices))
        tree_to_vertex = unsafe_wrap(
            Array, connectivity_pw.tree_to_vertex,
        ...
        calc_tree_node_coordinates!(
            tree_node_coordinates, nodes, mapping, vertices,
            tree_to_vertex
        )
        ...
        if boundary_symbols === nothing
            ...
            assign_boundaries_standard_abaqus!(
                boundary_names, n_trees,
    ...
    function assign_boundaries_standard_abaqus!(
            boundary_names, n_trees,
            element_node_matrix, node_set_dict,
            ::Val{2}
        ) # 2D version
    ...
    function P4estMeshCubedSphere(
            trees_per_face_dimension, layers, inner_radius, thickness;
            polydeg, RealT = Float64,
            initial_refinement_level = 0, unsaved_changes = true,
            p4est_partition_allow_for_coarsening = true
        )
        ...
        tree_node_coordinates = Array{RealT, 5}(
            undef, 3,
            ntuple(_ -> length(nodes), 3)...,
            n_trees
        )
        calc_tree_node_coordinates!(
            tree_node_coordinates, nodes, trees_per_face_dimension,
            layers,
    ...
    function connectivity_structured(n_cells_x, n_cells_y, n_cells_z, periodicity)
        ...
        connectivity = p8est_connectivity_new_copy(
            n_vertices, n_trees, n_corners, n_edges,
            vertices, tree_to_vertex,
            tree_to_tree, tree_to_face,
    ...
    function connectivity_cubed_sphere(trees_per_face_dimension, layers)
        ...
        linear_indices = LinearIndices(
            (
                trees_per_face_dimension, trees_per_face_dimension,
                layers, 6,
            )
        )
        ...
        for direction in 1:6
            for cell_z in 1:n_cells_z, cell_y in 1:n_cells_y, cell_x in 1:n_cells_x
                ...
                if cell_x > 1 # Connect to tree at the same face
                    tree_to_tree[1, tree] = linear_indices[
                ...
                if cell_x < n_cells_x # Connect to tree at the same face
                    tree_to_tree[2, tree] = linear_indices[
                        cell_x + 1, cell_y, cell_z,
                        direction,
                    ] - 1
                    tree_to_face[2, tree] = 0
                ...
                if cell_y > 1 # Connect to tree at the same face
                    tree_to_tree[3, tree] = linear_indices[
                ...
                if cell_y < n_cells_y # Connect to tree at the same face
                    tree_to_tree[4, tree] = linear_indices[
                        cell_x, cell_y + 1, cell_z,
                        direction,
                    ] - 1
                    tree_to_face[4, tree] = 2
                ...
                # Negative z-direction
                if cell_z > 1
                    tree_to_tree[5, tree] = linear_indices[
                ...
                # Positive z-direction
                if cell_z < n_cells_z
                    tree_to_tree[6, tree] = linear_indices[
                        cell_x, cell_y, cell_z + 1,
                        direction,
                    ] - 1
                    tree_to_face[6, tree] = 4
    ...
    function map_node_coordinates!(node_coordinates::AbstractArray{<:Any, 4}, mapping)
        for tree in axes(node_coordinates, 4),
            ...
            node_coordinates[:, i, j, tree] .= mapping(
                node_coordinates[1, i, j, tree],
        ...
        for tree in 1:n_trees
            ...
            if sum(curved_check) == 0
                ...
                for i in 1:4
                    ...
                    surface_curves[i] = CurvedSurfaceT(
                        nodes, bary_weights,
                        copy(curve_values)
                    )
    ...
    function cfunction(::typeof(save_original_id_iter_volume), ::Val{2})
        @cfunction(
            save_original_id_iter_volume, Cvoid,
    ...
    function cfunction(::typeof(collect_changed_iter_volume), ::Val{2})
        @cfunction(
            collect_changed_iter_volume, Cvoid,
            (Ptr{p4est_iter_volume_info_t}, Ptr{Cvoid})
        )
    end
    # 3D
