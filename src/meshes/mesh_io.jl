...
@muladd begin
    ...
    function save_mesh_file(
            mesh::Union{TreeMesh, P4estMesh, T8codeMesh}, output_directory,
            timestep = 0
        )
    ...
    function save_mesh_file(
            mesh::TreeMesh, output_directory, timestep,
            mpi_parallel::False
        )
    ...
    # `StructuredMeshView`
    function save_mesh_file(
            mesh::StructuredMesh, output_directory; system = "",
            timestep = 0
        )
        # Create output directory (if it does not exist)
        mkpath(output_directory)
    ...
    # these attributes for plotting purposes
    function save_mesh_file(
            mesh::P4estMesh, output_directory, timestep,
            mpi_parallel::False
        )
        # Create output directory (if it does not exist)
        mkpath(output_directory)
    ...
    function load_mesh_serial(mesh_file::AbstractString; n_cells_max, RealT)
        ...
        elseif mesh_type in ("StructuredMesh", "StructuredMeshView")
            ...
            mesh = StructuredMesh(
                size, mapping; RealT = RealT, unsaved_changes = false,
    ...
    function load_mesh!(mesh::ParallelTreeMesh, mesh_file::AbstractString)
        ...
        if mpi_isroot()
            h5open(mesh_file, "r") do file
                ...
                @views MPI.Bcast!(
                    mesh.tree.neighbor_ids[:, 1:n_cells], mpi_root(),
                    mpi_comm()
                )
                @views MPI.Bcast!(mesh.tree.levels[1:n_cells], mpi_root(), mpi_comm())
                @views MPI.Bcast!(
                    mesh.tree.coordinates[:, 1:n_cells], mpi_root(),
                    mpi_comm()
                )
            end
        else # non-root ranks
            # Set domain information
            mesh.tree.center_level_0 = MPI.Bcast!(
                collect(mesh.tree.center_level_0),
                mpi_root(), mpi_comm()
            )
            mesh.tree.length_level_0 = MPI.Bcast!(
                collect(mesh.tree.length_level_0),
                mpi_root(), mpi_comm()
            )[1]
            mesh.tree.periodicity = Tuple(
                MPI.Bcast!(
                    collect(mesh.tree.periodicity),
                    mpi_root(), mpi_comm()
                )
            )
