...
@muladd begin
    ...
    mutable struct TreeMesh{NDIMS, TreeType <: AbstractTree{NDIMS}} <: AbstractMesh{NDIMS}
        ...
        function TreeMesh{NDIMS, TreeType}(n_cells_max::Integer) where {
                NDIMS,
                TreeType <:
                AbstractTree{NDIMS},
            }
    ...
    function TreeMesh{NDIMS, TreeType}(
        ...
        TreeMesh{NDIMS, TreeType}(
            n_cells_max, SVector{NDIMS, Float64}(domain_center),
    ...
    function initialize!(
            mesh::TreeMesh, initial_refinement_level,
    ...
    function Base.show(
        ...
        if get(io, :compact, false)
            ...
            summary_box(
                io, "TreeMesh{" * string(NDIMS) * ", " * string(TreeType) * "}",
                setup
            )
        end
    end
