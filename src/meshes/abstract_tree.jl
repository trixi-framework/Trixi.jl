...
@muladd begin
    ...
    function refine_box!(
            t::AbstractTree{NDIMS}, coordinates_min,
            coordinates_max
        ) where {NDIMS}
        for dim in 1:NDIMS
    ...
    function refine_box!(t::AbstractTree{1}, coordinates_min::Real, coordinates_max::Real)
        return refine_box!(
            t, [convert(Float64, coordinates_min)],
    ...
    function refine_sphere!(
            t::AbstractTree{NDIMS}, center::SVector{NDIMS},
            radius
        ) where {NDIMS}
    ...
    function refine_sphere!(
            t::AbstractTree{NDIMS}, center::NTuple{NDIMS},
            radius
        ) where {NDIMS}
    ...
    function coarsen_box!(
            t::AbstractTree{NDIMS}, coordinates_min::AbstractArray{Float64},
    ...
    function child_coordinates(
            ::AbstractTree{NDIMS}, parent_coordinates,
            parent_length::Number, child::Int
        ) where {NDIMS}
        # Calculate length of child cells
        child_length = parent_length / 2
        return SVector(
            ntuple(
                d -> parent_coordinates[d] +
