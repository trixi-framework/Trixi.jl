...
@muladd begin
    ...
    function StructuredMesh(
            cells_per_dimension, mapping; RealT = Float64,
            periodicity = true, unsaved_changes = true,
            mapping_as_string = mapping2string(
                mapping,
        ...
        return StructuredMesh{NDIMS, RealT}(
            Tuple(cells_per_dimension), mapping,
    ...
    """
    function StructuredMesh(
            cells_per_dimension, faces::Tuple; RealT = Float64,
            periodicity = true
        )
        NDIMS = length(cells_per_dimension)
        ...
        return StructuredMesh(
            cells_per_dimension, mapping; RealT = RealT,
    ...
    function validate_faces(faces::NTuple{6, Any})
        @assert (
            faces[1](-1, -1) ≈
                faces[3](-1, -1) ≈
                faces[5](-1, -1)
        ) "faces[1](-1, -1), faces[3](-1, -1) and faces[5](-1, -1) need to match at (-1, -1, -1) corner"

        @assert (
            faces[2](-1, -1) ≈
                faces[3](1, -1) ≈
                faces[5](1, -1)
        ) "faces[2](-1, -1), faces[3](1, -1) and faces[5](1, -1) need to match at (1, -1, -1) corner"

        @assert (
            faces[1](1, -1) ≈
                faces[4](-1, -1) ≈
                faces[5](-1, 1)
        ) "faces[1](1, -1), faces[4](-1, -1) and faces[5](-1, 1) need to match at (-1, 1, -1) corner"

        @assert (
            faces[2](1, -1) ≈
                faces[4](1, -1) ≈
                faces[5](1, 1)
        ) "faces[2](1, -1), faces[4](1, -1) and faces[5](1, 1) need to match at (1, 1, -1) corner"

        @assert (
            faces[1](-1, 1) ≈
                faces[3](-1, 1) ≈
                faces[6](-1, -1)
        ) "faces[1](-1, 1), faces[3](-1, 1) and faces[6](-1, -1) need to match at (-1, -1, 1) corner"

        @assert (
            faces[2](-1, 1) ≈
                faces[3](1, 1) ≈
                faces[6](1, -1)
        ) "faces[2](-1, 1), faces[3](1, 1) and faces[6](1, -1) need to match at (1, -1, 1) corner"

        @assert (
            faces[1](1, 1) ≈
                faces[4](-1, 1) ≈
                faces[6](-1, 1)
        ) "faces[1](1, 1), faces[4](-1, 1) and faces[6](-1, 1) need to match at (-1, 1, 1) corner"

        @assert (
            faces[2](1, 1) ≈
                faces[4](1, 1) ≈
                faces[6](1, 1)
        ) "faces[2](1, 1), faces[4](1, 1) and faces[6](1, 1) need to match at (1, 1, 1) corner"
    end
