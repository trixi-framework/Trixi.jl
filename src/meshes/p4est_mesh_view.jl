...
function extract_boundaries(mesh::P4estMeshView{2},
    ...
    boundaries.node_coordinates = unsafe_wrap(Array, pointer(boundaries._node_coordinates),
                                              (n_dims,
                                               ntuple(_ -> n_nodes, n_dims - 1)...,
                                               capacity))

    resize!(boundaries._normal_directions, n_dims * n_nodes^(n_dims - 1) * capacity)
    boundaries.normal_directions = unsafe_wrap(Array, pointer(boundaries._normal_directions),
                                               (n_dims,
                                                ntuple(_ -> n_nodes, n_dims - 1)...,
                                                capacity))
