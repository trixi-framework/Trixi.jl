    ...
    for element in eachelement(solver, cache)
        ...
        for k in eachnode(solver), j in eachnode(solver), i in eachnode(solver)
            ...
            for variable in 1:n_vars
                element_conservative_values[variable, i, j, k] = u_node[variable]
            end
        end
        ...
        # 'r1', 'r2', and 'r3' are the global indices for the current element that it is mapped onto
...
function compute_kinetic_energy_spectrum(u, mesh::DGMultiMesh{3},
    ...
    q = cons2prim.(u_values, Ref(equations))
