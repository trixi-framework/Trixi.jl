...
@muladd begin
    ...
    struct PlotData2DTriangulated{
            DataType, NodeType, FaceNodeType, FaceDataType,
    ...
    function Base.show(io::IO, pds::PlotDataSeries)
        ...
        print(
            io, "PlotDataSeries{", typeof(pds.plot_data), "}(<plot_data>, ",
            pds.variable_id, ")"
        )
    end
    ...
    function PlotData2DCartesian(
            u, mesh::TreeMesh, equations, solver, cache;
            solution_variables = nothing,
        ...
        unstructured_data = get_unstructured_data(
            u, solution_variables_, mesh, equations,
            solver, cache
        )
        ...
        return PlotData2DCartesian(
            x, y, data, variable_names, mesh_vertices_x,
            mesh_vertices_y,
    ...
    function PlotData2D(
            u::Array{<:SVector, 2}, mesh, equations, dg::DGMulti, cache;
            solution_variables = nothing, nvisnodes = 2 * nnodes(dg)
        )
        nvars = length(first(u))
        u_structarray = StructArray{eltype(u)}(
            ntuple(
                _ -> zeros(eltype(first(u)), size(u)),
                nvars
            )
        )
        ...
        return PlotData2D(
            u_structarray, mesh, equations, dg, cache;
    ...
    function PlotData2D(
        ...
        for e in eachelement(mesh, dg, cache)
            ...
            StructArrays.foreachfield(
                interpolate_to_plotting_points!, view(u_plot, :, e),
                view(u, :, e)
            )
        ...
        return PlotData2DTriangulated(
            x_plot, y_plot, u_plot, t, x_face, y_face, face_data,
            variable_names
        )
    end
    ...
    function PlotData2DTriangulated(
            u, mesh, equations, dg::DGSEM, cache;
            solution_variables = nothing,
    ...
    function ScalarPlotData2D(
            u, mesh, equations, dg::DGMulti, cache;
        ...
        return PlotData2DTriangulated(
            x_plot, y_plot, ScalarData(u_plot), t,
            x_face, y_face, face_data, variable_name
        )
    end

    function ScalarPlotData2D(
            u, mesh, equations, dg::DGSEM, cache; variable_name = nothing,
            nvisnodes = 2 * nnodes(dg)
        )
        n_nodes_2d = nnodes(dg)^ndims(mesh)
        n_elements = nelements(dg, cache)
    ...
    function PlotData1D(
        ...
        if ndims(mesh) == 1
            x, data, mesh_vertices_x = get_data_1d(
                original_nodes, unstructured_data,
                nvisnodes
            )
            orientation_x = 1
    ...
    function PlotData1D(
            u, mesh, equations, dg::DGMulti{1}, cache;
            solution_variables = nothing
        )
        solution_variables_ = digest_solution_variables(equations, solution_variables)
        variable_names = SVector(varnames(solution_variables_, equations))
        ...
        if u isa StructArray
            ...
            x_plot = vec(x)
            data_plot = permutedims(
                reinterpret(reshape, eltype(eltype(data)), vec(data)),
                (2, 1)
            )
        end
