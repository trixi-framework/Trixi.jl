...
"""
function generate_plotting_triangulation(sol::TrixiODESolution, variable_to_plot::Int; nvisnodes=10)
    ...
    # make solution
    u = sol.u[end]
    u = wrap_array(u, mesh, equations, dg, cache)
...
function generate_plotting_wireframe(sol::TrixiODESolution, variable_to_plot::Int; nvisnodes=5)
    ...
    function face_first_reshape(x, num_nodes_1D, num_nodes, num_elements)
        num_reference_faces = 4 # hardcoded for quadrilateral elements
...
function trixi_plot(sol::TrixiODESolution; variable_to_plot_in = 1, nvisnodes=5)
    ...
    plotting_mesh = Makie.@lift(generate_plotting_triangulation(sol, $(menu.selection), nvisnodes=nvisnodes))
