...
@muladd begin
        ...
        for e in 1:num_elements
            for i in 1:num_reference_plotting_triangles
                tp[:, i + (e - 1) * num_reference_plotting_triangles] .= @views t[i, :] .+
                    (e - 1) *
                    num_plotting_points
                zp[:, i + (e - 1) * num_reference_plotting_triangles] .= @views u_plot[
                    t[
                        i,
                        :,
                    ],
    ...
    function mesh_plotting_wireframe(
            u::StructArray, mesh, equations, dg::DGMulti, cache;
        ...
        vandermonde_matrix_1D = StartUpDG.vandermonde(
            Line(), N,
            StartUpDG.nodes(
                Line(),
                num_face_points - 1
    ...
    function mesh_plotting_wireframe(
            u::StructArray, mesh, equations, dg::DGSEM, cache;
            nvisnodes = 2 * nnodes(dg)
        )
        ...
        x = reshape(
            view(cache.elements.node_coordinates, 1, :, :, :), n_nodes_2d,
            n_elements
        )
        y = reshape(
            view(cache.elements.node_coordinates, 2, :, :, :), n_nodes_2d,
            n_elements
        )

        # extract indices of local face nodes for wireframe plotting
        Fmask = get_face_node_indices(r, s, dg)
        plotting_interp_matrix1D = face_plotting_interpolation_matrix(
            dg;
            nvisnodes = nvisnodes
        )
    ...
    function mesh_plotting_wireframe(
            u::ScalarData, mesh, equations, dg::DGSEM, cache;
            nvisnodes = 2 * nnodes(dg)
        )
    ...
    function mesh_plotting_wireframe(
            u::ScalarData, mesh, equations, dg::DGMulti, cache;
            nvisnodes = 2 * nnodes(dg)
        )
        @unpack md = mesh
        rd = dg.basis
    ...
    function get_data_2d(
        ...
        if grid_lines
            mesh_vertices_x, mesh_vertices_y = calc_vertices(
                coordinates, levels,
                length_level_0
            )
        else
    ...
    function get_data_1d(original_nodes, unstructured_data, nvisnodes)
        ...
        interpolated_nodes = Array{eltype(original_nodes), 2}(
            undef, max_nvisnodes,
            n_elements
        )
        interpolated_data = Array{eltype(unstructured_data), 3}(
            undef, max_nvisnodes,
            n_elements, n_vars
        )
    ...
    function get_unstructured_data(u, solution_variables, mesh, equations, solver, cache)
        if solution_variables === cons2cons
            ...
            n_vars = length(
                solution_variables(
                    get_node_vars(u, equations, solver),
                    equations
                )
            )
    ...
    function cell2node(cell_centered_data)
        ...
        node_centered_data = [
            Matrix{Float64}(undef, resolution_out, resolution_out)
    ...
    function unstructured_3d_to_2d(
        ...
        for element_id in 1:n_elements
            ...
            if haskey(vandermonde_to_2d, normalized_intercept)
                ...
                vandermonde = polynomial_interpolation_matrix(
                    nodes_in,
                    [normalized_intercept]
                )
    ...
    function unstructured_2d_to_1d(
            original_nodes, unstructured_data, nvisnodes, slice,
            point
        )
        if slice === :x
            slice_dimension = 2
    ...
    function unstructured_2d_to_1d_curve(
            original_nodes, unstructured_data, nvisnodes,
        ...
        for element in 1:n_points_curve
            ...
            vandermonde_x = polynomial_interpolation_matrix(
                nodes_in,
                normalized_coordinates[1]
            )
            vandermonde_y = polynomial_interpolation_matrix(
                nodes_in,
            ...
            for v in 1:n_variables
                for i in 1:n_nodes
                    temp_data[i, element, v] = (
                        vandermonde_y * unstructured_data[
                            i, :,
                            element_ids[element],
                            v,
                        ]
    ...
    function unstructured_3d_to_1d_curve(
        ...
        for element in 1:n_points_curve
            ...
            vandermonde_z = polynomial_interpolation_matrix(
                nodes_in,
            ...
            for v in 1:n_variables
                for i in 1:n_nodes
                    for ii in 1:n_nodes
                        temp_data[i, ii, element, v] = (
                            vandermonde_z * unstructured_data[
                                i,
                                ii,
                                :,
                                element_ids[element],
    ...
    function tetrahedron_interpolation(
            x_coordinates_in, y_coordinates_in, z_coordinates_in,
    ...
    function get_value_at_point(point, nodes, data)
        ...
        @views coordinates_tetrahedron[:, 1] = nodes[
        ...
        for i in 1:4
            ...
            while true
                ...
                @views coordinates_tetrahedron[:, i] = nodes[
                    :, index[1], index[2],
                    index[3], closest_element,
                ]
                @views value_tetrahedron[:, i] = data[
                    :, index[1], index[2], index[3],
                    closest_element,
                ]
    ...
    function unstructured_3d_to_1d(
        ...
        for element_id in 1:n_elements
            ...
            max_coordinate = original_nodes[
                :, n_nodes_in, n_nodes_in, n_nodes_in,
                element_id,
            ]
            element_length = max_coordinate - min_coordinate
            ...
            # Construct vandermonde matrix for interpolation of each 2D element to a 1D element.
            normalized_intercept = (
                point[other_dimensions] .-
                    min_coordinate[other_dimensions]
            ) /
                element_length[1] * 2 .- 1
            vandermonde_i = polynomial_interpolation_matrix(
                nodes_in,
                normalized_intercept[1]
            )
            vandermonde_ii = polynomial_interpolation_matrix(
                nodes_in,
                ...
                for i in 1:n_nodes_in
                    for ii in 1:n_nodes_in
                        temp_unstructured_data[i, ii, new_id, v] = (
                            vandermonde_ii * unstructured_data[
                                ii,
                                :,
                                i,
                                element_id,
                    ...
                    new_unstructured_data[i, new_id, v] = (
                        vandermonde_i * temp_unstructured_data[
                            i,
                            :,
                            new_id,
                            v,
    ...
    function unstructured2structured(
            unstructured_data, normalized_coordinates,
            levels, resolution, nvisnodes_per_level
        )
        # Extract data shape information
        n_nodes_in, _, n_elements, n_variables = size(unstructured_data)
        ...
        lower_left_index = element2index(
            normalized_coordinates, levels, resolution,
            nvisnodes_per_level
        )
    ...
    function element2index(normalized_coordinates, levels, resolution, nvisnodes_per_level)
        ...
        for element_id in 1:n_elements
            ...
            lower_left_coordinate[1, element_id] = (
                normalized_coordinates[1, element_id] -
                    (nvisnodes - 1) / 2 * dx
            )
            lower_left_coordinate[2, element_id] = (
                normalized_coordinates[2, element_id] -
                    (nvisnodes - 1) / 2 * dx
            )
        end
    ...
    function coordinate2index(coordinate, resolution::Integer)
        ...
        # Find index
        id_x = searchsortedfirst.(
            Ref(mesh_coordinates), coordinate[1, :],
            lt = (x, y) -> x .< y .- dx / 2
        )
        id_y = searchsortedfirst.(
            Ref(mesh_coordinates), coordinate[2, :],
            lt = (x, y) -> x .< y .- dx / 2
        )
    ...
    function calc_vertices(node_coordinates, mesh)
        ...
        for cell_y in axes(mesh, 2)
            ...
            for cell_x in axes(mesh, 1)
                for node in 1:(n_nodes - 1)
                    x[i, line_index] = node_coordinates[
                        1, node, end,
                        linear_indices[cell_x, cell_y],
                    ]
                    y[i, line_index] = node_coordinates[
                        2, node, end,
                        linear_indices[cell_x, cell_y],
                    ]
    ...
    function triangle_interpolation(
            x_coordinates_in, y_coordinates_in, values_in,
            coordinate_out
        )
