...
@testset "TreeMesh1D" begin
    ...
    @testset "Additional tests in 1D" begin
        @testset "compressible Euler" begin
            ...
            @test isapprox(
                Trixi.entropy_thermodynamic([1.0, 2.0, 20.0], eqn),
                1.9740810260220094
            )
    ...
    @trixi_testset "Nonconservative terms in 1D (linear advection)" begin
        ...
        struct NonconservativeLinearAdvectionEquation <: AbstractEquations{
                1, #= spatial dimension =#
                2,
            } #= two variables (u,a) =#
        end

        Trixi.varnames(::typeof(cons2cons), ::NonconservativeLinearAdvectionEquation) = (
            "scalar",
            "advection_velocity",
        )
        ...
        function flux_nonconservative(
                u_mine, u_other, orientation,
        ...
        # characteristics
        function initial_condition_sine(
                x, t,
                equation::NonconservativeLinearAdvectionEquation
            )
        ...
        mesh = TreeMesh(
            -Float64(π), Float64(π), # min/max coordinates
        ...
        solver = DGSEM(
            polydeg = 3, surface_flux = surface_flux,
        ...
        sol = solve(
            ode, Tsit5(), abstol = 1.0e-6, reltol = 1.0e-6,
