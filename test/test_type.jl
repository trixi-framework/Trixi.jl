...
@testset "Test Type Stability" begin
    ...
    @timed_testset "Acoustic Perturbation 2D" begin
        for RealT in (Float32, Float64)
            ...
            equations = @inferred AcousticPerturbationEquations2D(
                v_mean_global,
                c_mean_global,
                rho_mean_global
            )
            ...
            for orientation in orientations
                for direction in directions
                    @test eltype(
                        @inferred boundary_condition_wall(
                            u_inner, orientation,
                            direction, x, t,
            ...
            @test eltype(
                @inferred boundary_condition_slip_wall(
                    u_inner,
    ...
    @timed_testset "Compressible Euler 1D" begin
        for RealT in (Float32, Float64)
            ...
            @test eltype(@inferred initial_condition_weak_blast_wave(x, t, equations)) ==
                RealT
            @test eltype(
                @inferred initial_condition_eoc_test_coupled_euler_gravity(
                    x, t,
                    equations
                )
            ) ==
            ...
            @test eltype(
                eltype(
                    @inferred splitting_steger_warming(
                        u, orientation,
                        equations
                    )
                )
            ...
            @test eltype(
                eltype(
                    @inferred splitting_vanleer_haenel(
                        u, orientation,
                        equations
                    )
                )
            ...
            @test eltype(
                eltype(
                    @inferred splitting_coirier_vanleer(
                        u, orientation,
                        equations
                    )
                )
            ...
            @test eltype(@inferred min_max_speed_davis(u_ll, u_rr, orientation, equations)) ==
                RealT
            @test eltype(
                @inferred min_max_speed_einfeldt(
                    u_ll, u_rr, orientation,
                    equations
                )
            ) ==
    ...
    @timed_testset "Compressible Euler 2D" begin
        for RealT in (Float32, Float64)
            ...
            x = SVector(zero(RealT), zero(RealT))
            t = zero(RealT)
            u = u_ll = u_rr = u_inner = cons = SVector(
                one(RealT), one(RealT), one(RealT),
                one(RealT)
            )
            orientations = [1, 2]
            ...
            @test eltype(@inferred source_terms_convergence_test(u, x, t, equations)) ==
                RealT
            @test eltype(
                @inferred source_terms_eoc_test_coupled_euler_gravity(
                    u, x, t,
                    equations
                )
            ) ==
            ...
            @test eltype(
                eltype(
                    @inferred splitting_drikakis_tsangaris(
                        u, normal_direction,
                        equations
                    )
                )
            ...
            @test eltype(
                eltype(
                    @inferred splitting_lax_friedrichs(
                        u, normal_direction,
                        equations
                    )
                )
            ...
            @test eltype(@inferred Trixi.gradient_conservative(pressure, u, equations)) ==
                RealT
            @test eltype(
                @inferred Trixi.gradient_conservative(
                    Trixi.entropy_math, u,
                    equations
                )
            ) == RealT
            @test eltype(
                @inferred Trixi.gradient_conservative(
                    Trixi.entropy_guermond_etal,
                    u,
                    equations
                )
            ) == RealT
    ...
    @timed_testset "Compressible Euler Multicomponent 1D" begin
        for RealT in (Float32, Float64)
            ...
            equations = @inferred CompressibleEulerMulticomponentEquations1D(
                gammas = gammas,
    ...
    @timed_testset "Compressible Euler Multicomponent 2D" begin
        for RealT in (Float32, Float64)
            gammas = (RealT(1.4), RealT(1.4))
            gas_constants = (RealT(0.4), RealT(0.4))
            equations = @inferred CompressibleEulerMulticomponentEquations2D(
                gammas = gammas,
                gas_constants = gas_constants
            )
            ...
            t = zero(RealT)
            u = u_ll = u_rr = SVector(
                one(RealT), one(RealT), one(RealT), one(RealT),
                one(RealT)
            )
            orientations = [1, 2]
            normal_direction = SVector(one(RealT), zero(RealT))
    ...
    @timed_testset "Compressible Euler Quasi 1D" begin
        for RealT in (Float32, Float64)
            ...
            @test eltype(@inferred flux(u, orientation, equations)) == RealT
            @test eltype(
                @inferred flux_nonconservative_chan_etal(
                    u_ll, u_rr, orientation,
                    equations
                )
            ) == RealT
    ...
    @timed_testset "Compressible Navier Stokes Diffusion 1D" begin
        for RealT in (Float32, Float64)
            ...
            mu = RealT(0.01)
            equations_parabolic_primitive = @inferred CompressibleNavierStokesDiffusion1D(
                equations,
                mu = mu,
                Prandtl = prandtl_number,
                gradient_variables = GradientVariablesPrimitive()
            )
            equations_parabolic_entropy = @inferred CompressibleNavierStokesDiffusion1D(
                equations,
                mu = mu,
                Prandtl = prandtl_number,
            ...
            x = SVector(zero(RealT))
            t = zero(RealT)
            u = u_inner = u_transformed = flux_inner = SVector(
            ...
            for equations_parabolic in (
                ...
                @test eltype(
                    @inferred Trixi.convert_transformed_to_primitive(
                        u_transformed,
                        equations_parabolic
                    )
                ) ==
                    RealT
                @test eltype(
                    @inferred Trixi.convert_derivative_to_primitive(
                        u, gradients,
                        equations_parabolic
                    )
                ) ==
                ...
                for direction in directions
                    @test eltype(
                        @inferred boundary_condition_right(
                            flux_inner, u_inner,
                    ...
                    @test eltype(
                        ...
                        RealT
                    @test eltype(
                        @inferred boundary_condition_left(
                            flux_inner, u_inner,
                            orientation, direction,
                            x,
                            t, operator_gradient,
    ...
    @timed_testset "Compressible Navier Stokes Diffusion 2D" begin
        for RealT in (Float32, Float64)
            ...
            prandtl_number = RealT(0.72)
            mu = RealT(0.01)
            equations_parabolic_primitive = @inferred CompressibleNavierStokesDiffusion2D(
                equations,
                mu = mu,
                Prandtl = prandtl_number,
                gradient_variables = GradientVariablesPrimitive()
            )
            equations_parabolic_entropy = @inferred CompressibleNavierStokesDiffusion2D(
                equations,
                mu = mu,
                Prandtl = prandtl_number,
                gradient_variables = GradientVariablesEntropy()
            ...
            x = SVector(zero(RealT), zero(RealT))
            t = zero(RealT)
            u = w_inner = u_transformed = flux_inner = normal = SVector(
                one(RealT),
                zero(RealT),
                zero(RealT),
                zero(RealT)
    ...
    @timed_testset "Compressible Navier Stokes Diffusion 3D" begin
        for RealT in (Float32, Float64)
            ...
            prandtl_number = RealT(0.72)
            mu = RealT(0.01)
            equations_parabolic_primitive = @inferred CompressibleNavierStokesDiffusion3D(
                equations,
                mu = mu,
                Prandtl = prandtl_number,
                gradient_variables = GradientVariablesPrimitive()
            )
            equations_parabolic_entropy = @inferred CompressibleNavierStokesDiffusion3D(
                equations,
                mu = mu,
                Prandtl = prandtl_number,
                gradient_variables = GradientVariablesEntropy()
    ...
    @timed_testset "Hyperbolic Diffusion 1D" begin
        for RealT in (Float32, Float64)
            ...
            @test eltype(
                @inferred Trixi.initial_condition_eoc_test_coupled_euler_gravity(
                    x,
                    t,
                    equations
                )
            ...
            for direction in directions
                @test eltype(
                    @inferred boundary_condition_poisson_nonperiodic(
                        u_inner,
                        orientation,
                        direction,
                        x, t,
    ...
    @timed_testset "Hyperbolic Diffusion 3D" begin
        for RealT in (Float32, Float64)
            ...
            x = SVector(zero(RealT), zero(RealT), zero(RealT))
            t = zero(RealT)
            u = du = u_ll = u_rr = u_inner = SVector(
                one(RealT), one(RealT), one(RealT),
                one(RealT)
            )
            orientations = [1, 2, 3]
    ...
    @timed_testset "Ideal Glm Mhd 1D" begin
        for RealT in (Float32, Float64)
            ...
            x = SVector(zero(RealT))
            t = zero(RealT)
            u = u_ll = u_rr = cons = SVector(
                one(RealT), zero(RealT), zero(RealT),
                zero(RealT),
                zero(RealT),
                zero(RealT),
                zero(RealT),
            ...
            @test eltype(@inferred flux_derigs_etal(u_ll, u_rr, orientation, equations)) ==
                RealT
            @test eltype(
                @inferred flux_hindenlang_gassner(
                    u_ll, u_rr,
                    orientation,
                    equations
                )
    ...
    @timed_testset "Ideal Glm Mhd 2D" begin
        for RealT in (Float32, Float64)
            ...
            orientations = [1, 2]
            directions = [1, 2, 3, 4]
            normal_direction = normal_direction_ll = normal_direction_average = SVector(
                one(RealT),
                zero(RealT)
            )
            ...
            @test eltype(@inferred flux(u, normal_direction, equations)) == RealT
            @test eltype(
                @inferred flux_nonconservative_powell(
                    u_ll, u_rr,
                    normal_direction_ll,
            ...
            for orientation in orientations
                ...
                @test eltype(
                    @inferred flux_nonconservative_powell_local_symmetric(
                        u_ll,
                        u_rr,
                        orientation,
                        equations
            ...
            @test typeof(
                @inferred Trixi.calc_fast_wavespeed(
                    cons, normal_direction,
                    equations
                )
            ) == RealT
            @test eltype(
                @inferred Trixi.calc_fast_wavespeed_roe(
                    u_ll, u_rr,
                    normal_direction,
                    equations
                )
    ...
    @timed_testset "Ideal Glm Mhd Multicomponent 1D" begin
        for RealT in (Float32, Float64)
            ...
            equations = @inferred IdealGlmMhdMulticomponentEquations1D(
                gammas = gammas,
                gas_constants = gas_constants
            )
    ...
    @timed_testset "Inviscid Burgers 1D" begin
        for RealT in (Float32, Float64)
            ...
            @test eltype(@inferred flux_godunov(u_ll, u_rr, orientation, equations)) ==
                RealT
            @test eltype(
                @inferred Trixi.flux_engquist_osher(
                    u_ll, u_rr, orientation,
                    equations
                )
            ) ==
    ...
    @timed_testset "Laplace Diffusion 1D" begin
        for RealT in (Float32, Float64)
            ...
            boundary_condition_neumann = BoundaryConditionNeumann(
                (x, t, equations) -> oftype(
                    t,
                    0
                )
            )
            ...
            # BC tests
            @test eltype(
                @inferred boundary_condition_dirichlet(
                    flux_inner, u_inner, normal,
                    x, t,
                    operator_gradient,
                    equations_parabolic
                )
            ...
            @test eltype(
                ...
                RealT
            @test eltype(
                @inferred boundary_condition_neumann(
                    flux_inner, u_inner, normal,
                    x, t,
                    operator_gradient,
                    equations_parabolic
    ...
    @timed_testset "Laplace Diffusion 2D" begin
        for RealT in (Float32, Float64)
            ...
            @test eltype(
                @inferred Trixi.penalty(
                    u_outer, u_inner, inv_h,
    ...
    @timed_testset "Laplace Diffusion 3D" begin
        for RealT in (Float32, Float64)
            ...
            x = SVector(zero(RealT), zero(RealT), zero(RealT))
            t = zero(RealT)
            u = u_inner = u_outer = inv_h = gradients = SVector(
                one(RealT), one(RealT),
                one(RealT)
            )
            orientations = [1, 2, 3]
    ...
    @timed_testset "Lattice Boltzmann 2D" begin
        for RealT in (Float32, Float64)
            ...
            for orientation in orientations
                for direction in directions
                    @test eltype(
                        @inferred boundary_condition_noslip_wall(
                            u_inner,
                            orientation,
                            direction, x, t,
                            surface_flux_function,
    ...
    @timed_testset "Linear Scalar Advection 1D" begin
        for RealT in (Float32, Float64)
            ...
            for direction in directions
                @test eltype(
                    @inferred Trixi.boundary_condition_linear_x(
                        u_inner,
                        orientation,
                        direction, x,
                        t,
    ...
    @timed_testset "Linear Scalar Advection 2D" begin
        for RealT in (Float32, Float64)
            ...
            for orientation in orientations
                for direction in directions
                    @test eltype(
                        @inferred Trixi.boundary_condition_linear_x_y(
                            u_inner,
                            orientation,
                            direction,
                            x,
                    ...
                    @test eltype(
                        ...
                        RealT
                    @test eltype(
                        @inferred Trixi.boundary_condition_linear_y(
                            u_inner,
                            orientation,
                            direction,
                            x,
    ...
    @timed_testset "Linear Scalar Advection 3D" begin
        for RealT in (Float32, Float64)
            equations = @inferred LinearScalarAdvectionEquation3D(
                RealT(1), RealT(1),
                RealT(1)
            )
            ...
            for orientation in orientations
                for direction in directions
                    @test eltype(
                        @inferred Trixi.boundary_condition_linear_z(
                            u_inner,
                            orientation,
                            direction,
                            x,
    ...
    @timed_testset "Linearized Euler 1D" begin
        for RealT in (Float32, Float64)
            equations = @inferred LinearizedEulerEquations1D(
                v_mean_global = RealT(0),
    ...
    @timed_testset "Linearized Euler 2D" begin
        for RealT in (Float32, Float64)
            equations = @inferred LinearizedEulerEquations2D(
                v_mean_global = (
                    RealT(0),
                    RealT(0),
                ),
    ...
    @timed_testset "Linearized Euler 3D" begin
        for RealT in (Float32, Float64)
            equations = @inferred LinearizedEulerEquations3D(
                v_mean_global = (
                    RealT(0),
                    RealT(0),
                    RealT(0),
    ...
    @timed_testset "Polytropic Euler 2D" begin
        for RealT in (Float32, Float64)
            equations1 = @inferred PolytropicEulerEquations2D(
                RealT(1),
                RealT(1)
            ) # equations.gamma == 1
    ...
    @timed_testset "Shallow Water 1D" begin
        for RealT in (Float32, Float64)
            ...
            for direction in directions
                ...
                @test eltype(
                    @inferred Trixi.calc_wavespeed_roe(
                        u_ll, u_rr, direction,
                        equations
                    )
                ) ==
            ...
            @test eltype(@inferred flux(u, orientation, equations)) == RealT
            @test eltype(
                @inferred flux_nonconservative_wintermeyer_etal(
                    u_ll, u_rr,
                    orientation,
                    equations
                )
                ...
                RealT
            @test eltype(
                @inferred flux_nonconservative_fjordholm_etal(
                    u_ll, u_rr,
                    orientation,
                    equations
                )
            ...
            @test eltype(
                @inferred flux_nonconservative_audusse_etal(
                    u_ll, u_rr,
                    orientation,
                    equations
                )
            ...
            @test eltype(
                @inferred flux_fjordholm_etal(
                    u_ll, u_rr, orientation,
                    equations
                )
            ) == RealT
            @test eltype(
                @inferred flux_wintermeyer_etal(
                    u_ll, u_rr, orientation,
                    equations
                )
            ) == RealT
            ...
            @test eltype(
                eltype(
                    @inferred hydrostatic_reconstruction_audusse_etal(
                        u_ll,
                        u_rr,
                        equations
                    )
