...
@testset "Examples 3D" begin
  ...
  @testset "taal-confirmed parameters_hypdiff_lax_friedrichs.toml with initial_refinement_level=2" begin
  ...
  @testset "taal-confirmed parameters_hypdiff_lax_friedrichs.toml with initial_refinement_level=2, surface_flux=flux_upwind)" begin
    test_trixi_run(joinpath(EXAMPLES_DIR, "parameters_hypdiff_lax_friedrichs.toml"),
  ...
  @testset "taal-confirmed parameters_mhd_ec.toml" begin
    test_trixi_run(joinpath(EXAMPLES_DIR, "parameters_mhd_ec.toml"),
            ...
            linf = [0.26459488761558436, 0.3360978919197787, 0.33609789191977707, 0.36954525736870697, 1.2313599542279157, 0.09833294849101182, 0.09833294849101237, 0.10494487848599776, 0.007993540589733391])
  end
  ...
  @testset "taal-confirmed parameters_mhd_alfven_wave.toml" begin
    test_trixi_run(joinpath(EXAMPLES_DIR, "parameters_mhd_alfven_wave.toml"),
            l2   = [0.003873451052838288, 0.009037433137692093, 0.004173271078426907, 0.011605022986462266, 0.006248048889157039, 0.00922613624746438, 0.0034604877253724005, 0.011684976982110353, 0.002201065710054377],
            linf = [0.01263000774167411, 0.03265661154288797, 0.012913518354883094, 0.04474890983575379, 0.027980468666095604, 0.03453489099647533, 0.010268706624875001, 0.04475917251596939, 0.00999049213346832])
  end
  @testset "taal-confirmed parameters_mhd_alfven_wave_mortar.toml" begin
    test_trixi_run(joinpath(EXAMPLES_DIR, "parameters_mhd_alfven_wave_mortar.toml"),
            l2   = [0.0021484229935627365, 0.006826954380911822, 0.003065485208398127, 0.008736001002299107, 0.0051597638037571844, 0.0071585492826525534, 0.00282893419965326, 0.008815066789289458, 0.0022268123079223112],
            linf = [0.01295722309524816, 0.05529013882056741, 0.020854761607773175, 0.05897932879286029, 0.031628684472211344, 0.05513449181023805, 0.017933788713705545, 0.06005951724325258, 0.0130249290510531],
            t_end = 0.25)
  end
