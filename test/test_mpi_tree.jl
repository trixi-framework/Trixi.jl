...
@testset "Examples 2D" begin
    ...
    @trixi_testset "elixir_hypdiff_harmonic_nonperiodic.jl" begin
        @test_trixi_include(joinpath(EXAMPLES_DIR,
                            ...
                            l2=[8.61813235543625e-8,
                                5.619399844542781e-7,
                                5.6193998447443e-7],
                            linf=[1.124861862180196e-6,
    ...
    @trixi_testset "elixir_hypdiff_nonperiodic.jl" begin
        @test_trixi_include(joinpath(EXAMPLES_DIR, "elixir_hypdiff_nonperiodic.jl"),
                            l2=[8.523077653955306e-6,
                                2.8779323653065056e-5,
                                5.4549427691297846e-5],
                            linf=[5.5227409524905013e-5,
    ...
    if !CI_ON_WINDOWS # see comment on `CI_ON_WINDOWS` in `test/test_mpi.jl`
        @trixi_testset "elixir_hypdiff_godunov.jl" begin
            @test_trixi_include(joinpath(EXAMPLES_DIR, "elixir_hypdiff_godunov.jl"),
                                l2=[5.868147556427088e-6,
                                    3.80517927324465e-5,
                                    3.805179273249344e-5],
                                linf=[3.701965498725812e-5,
    ...
    if !CI_ON_WINDOWS # see comment on `CI_ON_WINDOWS` in `test/test_mpi.jl`
        @trixi_testset "elixir_euler_ec.jl" begin
            ...
            @testset "error-based step size control" begin
                ...
                if Trixi.mpi_isroot()
                    @test errors.l2≈[0.061653630426688116,
                                     0.05006930431098764,
                                     0.05007694316484242,
                                     0.22550689872331683] rtol=1.0e-4
                    @test errors.linf≈[0.28516937484583693,
                                       0.2983633696512788,
                                       0.297812036335975,
                                       1.027368795517512] rtol=1.0e-4
                end
            end
        end
    end
