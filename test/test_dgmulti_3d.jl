...
@trixi_testset "elixir_euler_taylor_green_vortex.jl" begin
    @test_trixi_include(joinpath(EXAMPLES_DIR, "elixir_euler_taylor_green_vortex.jl"),
                        ...
                        l2 = [0.00036128264902931644, 0.06219350570157671, 0.062193505701565316, 0.08121963725209637, 0.0708269605813566], 
                        linf = [0.0007893500666786846, 0.14819541663164099, 0.14819541663231595, 0.148472950090691, 0.2131352319423172])
    # Ensure that we do not have excessive memory allocations
    # (e.g., from type instabilities)
    let
        t = sol.t[end]
...
@trixi_testset "elixir_euler_taylor_green_vortex.jl (GaussSBP)" begin
    @test_trixi_include(joinpath(EXAMPLES_DIR, "elixir_euler_taylor_green_vortex.jl"),
                        ...
                        l2 = [0.0003612826490291416, 0.06219350570157282, 0.06219350570156088, 0.08121963725209767, 0.07082696058040763], 
                        linf = [0.0007893500666356079, 0.14819541663140268, 0.14819541663222624, 0.14847295009030398, 0.21313523192395678])
    # Ensure that we do not have excessive memory allocations
    # (e.g., from type instabilities)
    let
        t = sol.t[end]
...
@trixi_testset "elixir_euler_fdsbp_periodic.jl" begin
    @test_trixi_include(joinpath(EXAMPLES_DIR, "elixir_euler_fdsbp_periodic.jl"),
                        l2 = [0.16917000223831077, 0.1584186395735661, 0.15841863957356606, 0.15841863957356622, 0.21512078067145463], 
                        linf = [0.28301503739228906, 0.2938823666715491, 0.2938823666715442, 0.29388236667154466, 0.3670612316913644])
    # Ensure that we do not have excessive memory allocations
    # (e.g., from type instabilities)
    let
        t = sol.t[end]
