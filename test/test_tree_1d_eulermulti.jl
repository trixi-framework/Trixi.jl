...
@testset "Compressible Euler Multicomponent" begin
    @trixi_testset "Testing entropy2cons and cons2entropy" begin
        using ForwardDiff
        gammas = (
            1.3272378792562836, 1.5269959187969864, 1.8362285750521512,
            1.0409061360276926, 1.4652015053812224, 1.3626493264184423,
        )
        gas_constants = (
            1.817636851910076, 6.760820475922636, 5.588953939749113,
            6.31574782981543, 3.362932038038397, 3.212779569399733,
        )
        equations = CompressibleEulerMulticomponentEquations1D(
            gammas = SVector{length(gammas)}(gammas...),
            gas_constants = SVector{length(gas_constants)}(gas_constants...)
        )
        u = [
            -1.4632513788889214, 0.9908786980927811, 0.2909066990257628,
    ...
    @trixi_testset "elixir_eulermulti_ec.jl" begin
        @test_trixi_include(
            joinpath(EXAMPLES_DIR, "elixir_eulermulti_ec.jl"),
            l2 = [
                0.15330089521538684, 0.4417674632047301,
                0.016888510510282385, 0.03377702102056477,
                0.06755404204112954,
            ],
            linf = [
                0.29130548795961864, 0.8847009003152357,
    ...
    @trixi_testset "elixir_eulermulti_es.jl" begin
        @test_trixi_include(
            joinpath(EXAMPLES_DIR, "elixir_eulermulti_es.jl"),
            l2 = [
                0.1522380497572071,
                0.43830846465313206,
                0.03907262116499431,
    ...
    @trixi_testset "elixir_eulermulti_convergence_es.jl" begin
        @test_trixi_include(
            joinpath(EXAMPLES_DIR, "elixir_eulermulti_convergence_es.jl"),
            l2 = [
                1.8983933794407234e-5, 6.207744299844731e-5,
                1.5466205761868047e-6, 3.0932411523736094e-6,
                6.186482304747219e-6, 1.2372964609494437e-5,
            ],
            linf = [
                0.00012014372605895218, 0.0003313207215800418,
    ...
    @trixi_testset "elixir_eulermulti_convergence_es.jl with flux_chandrashekar" begin
        @test_trixi_include(
            joinpath(EXAMPLES_DIR, "elixir_eulermulti_convergence_es.jl"),
            l2 = [
                1.888450477353845e-5, 5.4910600482795386e-5,
                9.426737161533622e-7, 1.8853474323067245e-6,
                3.770694864613449e-6, 7.541389729226898e-6,
            ],
            linf = [
                0.00011622351152063004, 0.0003079221967086099,
    ...
    @trixi_testset "elixir_eulermulti_two_interacting_blast_waves.jl" begin
        @test_trixi_include(
            ...
            l2 = [
                1.288867611915533, 82.71335258388848, 0.00350680272313187,
                0.013698784353152794,
                0.019179518517518084,
            ],
            linf = [
                29.6413044707026, 1322.5844802186496, 0.09191919374782143,
                0.31092970966717925,
                0.4417989757182038,
            ],
            tspan = (0.0, 0.0001)
