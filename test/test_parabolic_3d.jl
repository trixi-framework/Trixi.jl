...
@trixi_testset "DGMulti: elixir_navierstokes_convergence.jl" begin
    @test_trixi_include(
        joinpath(examples_dir(), "dgmulti_3d", "elixir_navierstokes_convergence.jl"),
...
@trixi_testset "TreeMesh3D: elixir_navierstokes_convergence.jl" begin
    @test_trixi_include(
        joinpath(examples_dir(), "tree_3d_dgsem", "elixir_navierstokes_convergence.jl"),
    ...
    @test_trixi_include(
        ...
        heat_bc_top_bottom = Isothermal(
            (x, t, equations) -> Trixi.temperature(
                initial_condition_navier_stokes_convergence_test(x, t, equations),
                equations,
            ),
        ),
        l2 = [
...
@trixi_testset "TreeMesh3D: elixir_navierstokes_taylor_green_vortex.jl (Refined mesh)" begin
    ...
    analysis_callback = AnalysisCallback(
        ...
        save_analysis = true,
        extra_analysis_integrals = (energy_kinetic, energy_internal, enstrophy),
    )
    callbacks = CallbackSet(summary_callback, alive_callback, analysis_callback)
...
@trixi_testset "TreeMesh3D: elixir_advection_diffusion_amr.jl" begin
    @test_trixi_include(
        joinpath(examples_dir(), "tree_3d_dgsem", "elixir_advection_diffusion_amr.jl"),
