...
@testset "Visualization tests" begin
    ...
    @testset "PlotData2D, PlotDataSeries, PlotMesh with $mesh" for mesh in keys(test_examples_2d)
        ...
        if mesh == "TreeMesh"
            ...
            @test PlotData2D(
                sol; nvisnodes = 0, grid_lines = false,
                solution_variables = cons2cons
            ) isa Trixi.PlotData2DCartesian
    ...
    @timed_testset "PlotData1D, PlotDataSeries, PlotMesh" begin
        ...
        @testset "plotting multiple variables" begin
            ...
            fake2d = Trixi.PlotData2DCartesian(
                x, y, data2d, variable_names,
    ...
    @timed_testset "adapt_to_mesh_level" begin
        ...
        @test isapprox(sol.u[end], u_ode_level4, atol = 1.0e-13)

        @test adapt_to_mesh_level!(sol, 5) isa Tuple
        @test isapprox(sol.u[end], u_ode_level5, atol = 1.0e-13)
    end
    ...
    @timed_testset "plot 3D" begin
        ...
        @test_nowarn_mod trixi_include(
            @__MODULE__,
            joinpath(
                examples_dir(), "structured_3d_dgsem",
                "elixir_advection_basic.jl"
            )
        )
    ...
    @timed_testset "Makie visualization tests for UnstructuredMesh2D" begin
        @test_nowarn_mod trixi_include(
            @__MODULE__,
            joinpath(
                examples_dir(), "unstructured_2d_dgsem",
