    ...
    @testset "Convergence test" begin
        if !coverage
            ...
            @timed_testset "p4est_2d_dgsem" begin
                ...
                no_refine = @cfunction(
                    (p4est, which_tree, quadrant) -> Cint(0), Cint,
                    (
            ...
            @test_nowarn_mod convergence_test(
                @__MODULE__,
                joinpath(
                    EXAMPLES_DIR, "tree_2d_dgsem",
                    "elixir_advection_basic.jl"
            ...
            @test_nowarn_mod convergence_test(
                @__MODULE__,
                joinpath(
                    EXAMPLES_DIR, "structured_2d_dgsem",
                    "elixir_advection_basic.jl"
                ), 2
            )
            @test_nowarn_mod convergence_test(
    ...
    @testset "AD using ForwardDiff" begin
        @timed_testset "Euler equations 1D" begin
            function entropy_at_final_time(k) # k is the wave number of the initial condition
                ...
                mesh = TreeMesh(
                    (-1.0,), (1.0,), initial_refinement_level = 3,
                    n_cells_max = 10^4
                )
                solver = DGSEM(
                    3, FluxHLL(min_max_speed_naive),
        ...
        @timed_testset "Linear advection 2D" begin
            function energy_at_final_time(k) # k is the wave number of the initial condition
                ...
                mesh = TreeMesh(
                    (-1.0, -1.0), (1.0, 1.0), initial_refinement_level = 3,
                    n_cells_max = 10^4
                )
