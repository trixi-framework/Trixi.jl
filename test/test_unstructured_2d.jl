...
@trixi_testset "elixir_euler_periodic.jl (O2 inner reconstruction)" begin
    @test_trixi_include(joinpath(EXAMPLES_DIR, "elixir_euler_periodic.jl"),
                        ...
                        l2=[
                            0.005879366744758224, 0.0040532157937838665,
                            0.0040532157937835, 0.010488150760384196
                        ],
                        linf=[
                            0.015729471569279507, 0.011414878105429516,
                            0.011414878105437953, 0.02866698043126714
                        ])
    # Ensure that we do not have excessive memory allocations
    # (e.g., from type instabilities)
    @test_allocations(Trixi.rhs!, semi, sol, 1000)
