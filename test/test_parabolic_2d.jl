...
@testset "SemidiscretizationHyperbolicParabolic (2D)" begin
    ...
    @trixi_testset "DGMulti 2D rhs_parabolic!" begin
        dg = DGMulti(
            polydeg = 2, element_type = Quad(), approximation_type = Polynomial(),
        ...
        Trixi.calc_gradient!(
            gradients, ode.u0, t, mesh, equations_parabolic,
        ...
        Trixi.calc_viscous_fluxes!(
            u_flux, ode.u0, gradients, mesh, equations_parabolic,
        ...
        du = similar(ode.u0)
        Trixi.calc_divergence!(
            du, ode.u0, t, u_flux, mesh, equations_parabolic,
    ...
    @trixi_testset "elixir_navierstokes_NACA0012airfoil_mach08.jl" begin
        ...
        drag_p = Trixi.analyze(
            drag_coefficient, du, u, tspan[2], mesh, equations, solver,
            semi.cache, semi
        )
        lift_p = Trixi.analyze(
            lift_coefficient, du, u, tspan[2], mesh, equations, solver,
            semi.cache, semi
        )

        drag_f = Trixi.analyze(
            drag_coefficient_shear_force, du, u, tspan[2], mesh,
            equations, equations_parabolic, solver,
            semi.cache, semi, semi.cache_parabolic
        )
        lift_f = Trixi.analyze(
            lift_coefficient_shear_force, du, u, tspan[2], mesh,
            equations, equations_parabolic, solver,
            semi.cache, semi, semi.cache_parabolic
        )

        @test isapprox(drag_p, 0.17963843913309516, atol = 1.0e-13)
        @test isapprox(lift_p, 0.26462588007949367, atol = 1.0e-13)

        @test isapprox(drag_f, 1.5427441885921553, atol = 1.0e-13)
        @test isapprox(lift_f, 0.005621910087395724, atol = 1.0e-13)
    end
end
