...
"""
function initial_condition_jeans_instability(
        x, t,
...
parameters = ParametersEulerGravity(
    background_density = 1.5e7, # aka rho0
...
function Trixi.analyze(
        ::Val{:energy_potential}, du, u_euler, t,
    ...
    e_potential = Trixi.integrate_via_indices(
        u_euler, mesh, equations_euler, dg, cache,
        equations_gravity,
        u_gravity
    ) do u, i, j, element,
            equations_euler, dg,
        ...
        u_gravity_local = Trixi.get_node_vars(
            u_gravity, equations_gravity, dg, i, j,
            element
        )
...
callbacks = CallbackSet(
    summary_callback, stepsize_callback,
