...
# heaviside step function over [-0.5, 0.5]
function initial_condition_heaviside_step(x, t, equations::LinearScalarAdvectionEquation1D)
    u = abs(x[1]) < 0.5 ? 1.0 : 0.0
    return SVector(u)
end
function initial_condition_near_zero(x, t, equations::LinearScalarAdvectionEquation1D)
    u = 1 + sinpi(2 * x[1])
    return SVector(u)
end
initial_condition = initial_condition_heaviside_step
# initial_condition = initial_condition_near_zero
...
local_limiter! = PositivityPreservingLimiterZhangShu(thresholds = (-0.01,),
