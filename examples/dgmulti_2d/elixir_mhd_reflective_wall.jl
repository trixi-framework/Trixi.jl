...
solver = DGMulti(
    polydeg = 3, element_type = Quad(), approximation_type = GaussSBP(),
...
mesh = DGMultiMesh(
    solver, cells_per_dimension; periodicity = (false, false),
    is_on_boundary
)
...
boundary_conditions = (;
    x_neg = boundary_condition_velocity_slip_wall,
...
sol = solve(
    ...
    dt = 1.0e-5, # solve needs some value here but it will be overwritten by the stepsize_callback
