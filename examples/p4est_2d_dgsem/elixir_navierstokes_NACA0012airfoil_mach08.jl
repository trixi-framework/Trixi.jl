...
mesh_file = Trixi.download(
    "https://gist.githubusercontent.com/Arpit-Babbar/339662b4b46164a016e35c81c66383bb/raw/8bf94f5b426ba907ace87405cfcc1dcc2ef7cbda/NACA0012.inp",
...
@inline function boundary_condition_subsonic_constant(
        u_inner,
...
velocity_bc_square = NoSlip(
    (x, t, equations) -> momenta_initial_condition_mach08_flow(
        x, t,
        equations
    )
)
...
boundary_condition_square = BoundaryConditionNavierStokesWall(
    velocity_bc_square,
    heat_bc_square
)
...
drag_coefficient = AnalysisSurfaceIntegral(
...
drag_coefficient_shear_force = AnalysisSurfaceIntegral(
    semi, force_boundary_names,
    DragCoefficientShearStress(
        aoa(),
        rho_inf(),
        u_inf(equations),
        l_inf()
...
lift_coefficient_shear_force = AnalysisSurfaceIntegral(
    semi, force_boundary_names,
    LiftCoefficientShearStress(
        aoa(),
        rho_inf(),
        u_inf(equations),
        l_inf()
...
sol = solve(
    ode, RDPK3SpFSAL49(thread = OrdinaryDiffEq.True()); abstol = 1.0e-8,
    reltol = 1.0e-8,
