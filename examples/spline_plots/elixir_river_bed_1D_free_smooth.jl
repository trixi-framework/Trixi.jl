...
# to the cubic spline interpolation with natural end end_condition condition   #
...
spline        = cubic_b_spline(spline_data)
spline_smooth = cubic_b_spline(spline_data; smoothing_factor = λ)
...
plot!(x_calc, spline_func.(x_calc), label = "cubic spline interpolation, natural end_condition")
plot!(x_calc, spline_func_smooth.(x_calc), 
      label = "cubic spline interpolation, natural end_condition with smoothing factor $λ")