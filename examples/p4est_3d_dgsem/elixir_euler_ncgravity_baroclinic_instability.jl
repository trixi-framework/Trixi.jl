...
gamma = 1.4
equations = CompressibleEulerEquationsNCGravity3D(gamma)
...
function steady_state_baroclinic_instability(x,equations::CompressibleEulerEquationsNCGravity3D)
    lon, lat, r = cartesian_to_sphere(x)
    radius_earth = 6.371229e6
    gravitational_acceleration = 9.80616     # g
...
surface_flux = (FluxLMARS(340.0), flux_nonconservative_waruszewski_lnmean)
volume_flux = (flux_kennedy_gruber, flux_nonconservative_waruszewski_lnmean)
