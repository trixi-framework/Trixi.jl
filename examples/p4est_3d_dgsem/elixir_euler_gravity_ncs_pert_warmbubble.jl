...
function (setup::WarmBubbleSetup)(x, t,
                                  ::CompressibleEulerEquationsGravityNCSPert3D)
    RealT = eltype(x)
...
# Steady state
function (setup::WarmBubbleSetup)(x, ::CompressibleEulerEquationsGravityNCSPert3D)
    @unpack g, c_p, c_v = setup
...
equations = CompressibleEulerEquationsGravityNCSPert3D(warm_bubble_setup.gamma)
...
@inline function v1_pert(u, ::CompressibleEulerEquationsGravityNCSPert3D)
    return abs(u[2])
end
@inline function v2_pert(u, ::CompressibleEulerEquationsGravityNCSPert3D)
    return abs(u[3])
end
...
save_solution = SaveSolutionCallback(dt = 10.0, #interval = 1, #dt = 10.0,
                                     ...
                                     output_directory="out_bubble_3d_gncs")
