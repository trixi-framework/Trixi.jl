...
function steady_state_baroclinic_instability(x,equations::CompressibleEulerEquationsPerturbationGravity3D)
    lon, lat, r = cartesian_to_sphere(x)
    radius_earth = 6.371229e6
    gravitational_acceleration = 9.80616     # g
    # Make sure that the r is not smaller than radius_earth
...
gamma = 1.4
equations = CompressibleEulerEquationsPerturbationGravity3D(gamma)
