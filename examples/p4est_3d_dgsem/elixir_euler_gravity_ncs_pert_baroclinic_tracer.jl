...
function steady_state_baroclinic_instability(x,equations::PassiveTracerEquations)
    lon, lat, r = cartesian_to_sphere(x)
    radius_earth = 6.371229e6
    gravitational_acceleration = 9.80616     # g
    # Make sure that the r is not smaller than radius_earth
    ...
    return vcat(prim2cons(SVector(rho, v1, v2, v3, p), equations.flow_equations), phi)
end
...
@inline function source_terms_coriolis(u, aux, x, t,
                                       equations::PassiveTracerEquations)
    u_flow = Trixi.flow_variables(u, equations)
    u_total = Trixi.cons2cons_total(u_flow, aux, equations.flow_equations)
    angular_velocity = 7.29212e-5  # Ω
...
@inline function vel_mag(u, aux, equations::PassiveTracerEquations)
    u_flow = Trixi.flow_variables(u, equations)
    rho, rho_v1, rho_v2, rho_v3 = Trixi.cons2cons_total(u_flow, aux, equations.flow_equations)
...
gamma = 1.4
flow_equations = CompressibleEulerEquationsGravityNCSPert3D(gamma)
