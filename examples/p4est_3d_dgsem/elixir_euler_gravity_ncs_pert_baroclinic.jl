...
function steady_state_baroclinic_instability(x,equations::CompressibleEulerEquationsGravityNCSPert3D)
    lon, lat, r = cartesian_to_sphere(x)
    radius_earth = 6.371229e6
    gravitational_acceleration = 9.80616     # g
    # Make sure that the r is not smaller than radius_earth
    ...
    return vcat(prim2cons(SVector(rho, v1, v2, v3, p), equations), phi)
end
...
@inline function vel_mag(u, aux, equations::CompressibleEulerEquationsGravityNCSPert3D)
...
gamma = 1.4
equations = CompressibleEulerEquationsGravityNCSPert3D(gamma)
