...
function initial_condition_convergence_test_sphere(x,
    t,
...
@inline function source_terms_convergence_test_sphere(u,
    x,
    t,
    equations::CompressibleEulerEquations3D)
    x_scaled = x / 6.371229e6
...
function indicator_test(u::AbstractArray{<:Any, 5},
    ...
    for element in eachelement(dg, cache)
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            x = Trixi.get_node_coords(cache.elements.node_coordinates,
                equations,
                dg,
                i,
                j,
...
function Trixi.get_element_variables!(element_variables,
    indicator::typeof(indicator_test),
    ::AMRCallback)
    return nothing
end
