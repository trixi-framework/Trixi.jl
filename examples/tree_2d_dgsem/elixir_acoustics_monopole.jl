...
equations = AcousticPerturbationEquations2D(
    v_mean_global = (0.0, 0.0), c_mean_global = 0.0,
    rho_mean_global = 0.0
)
...
"""
function boundary_condition_monopole(
    ...
    if -0.05 <= x[1] <= 0.05 # Monopole
        ...
        prim_boundary = SVector(
            v1_prime, v2_prime, p_prime, u_inner[4], u_inner[5],
            u_inner[6], u_inner[7]
        )
        ...
        u_boundary = SVector(
            u_inner[1], -u_inner[2], u_inner[3], u_inner[4], u_inner[5],
            u_inner[6],
            u_inner[7]
        )
    end
...
"""
function boundary_condition_zero(
        u_inner, orientation, direction, x, t,
        surface_flux_function,
        equations::AcousticPerturbationEquations2D
    )
