...
module VortexPairSetup
    ...
    function velocity(x, t, vortex_pair::VortexPair)
        ...
        v1 = -circulation / (2 * pi) * (
            r_plus / (rc^2 + r_plus^2) * si_plus +
                r_minus / (rc^2 + r_minus^2) * si_minus
        )
        v2 = circulation / (2 * pi) * (
            r_plus / (rc^2 + r_plus^2) * co_plus +
    ...
    function (initial_condition::InitialCondition)(
            x, t,
    ...
    function SpongeLayer(;
            sponge_layer_min, sponge_layer_max, reference_values,
    ...
    function (sponge_layer::SpongeLayer)(u, x, t, equations)
        ...
        if lies_in_sponge_layer(x, sponge_layer_min, sponge_layer_max)
            return source_term_sponge_layer(
                u, x, t, equations, sponge_layer_min,
                sponge_layer_max,
                reference_values
            )
    ...
    function source_term_sponge_layer(
            u, x, t, equations::AcousticPerturbationEquations2D,
        ...
        alphas = ntuple(
            i -> calc_damping_factor(x, i, sponge_layer_min, sponge_layer_max),
    ...
    function source_term_sponge_layer(
            u, x, t, equations::CompressibleEulerEquations2D,
            sponge_layer_min::NTuple{4}, sponge_layer_max::NTuple{4},
            reference_values
        )
        # Calculate the damping factors for each direction (=0 if x is not in the corresponding sponge
        ...
        s = SVector(
            -alpha_square * (u_prim[1] - reference_values[1]), 0, 0,
...
sponge_layer_euler = VortexPairSetup.SpongeLayer(
    sponge_layer_min = (
        -135 * r0, 115 * r0,
        -135 * r0, 115 * r0,
    ),
...
boundary_condition_euler = VortexPairSetup.BoundaryCondition(
    rho,
    (rho * c0^2 / gamma) /
        (gamma - 1)
)
...
semi_euler = SemidiscretizationHyperbolic(
    mesh, equations_euler, initial_condition_euler,
    solver,
...
equations_acoustics = AcousticPerturbationEquations2D(
    v_mean_global = (13.0, 26.0),
    c_mean_global = 39.0,
    rho_mean_global = 52.0
) # global mean values will be overwritten

sponge_layer_acoustics = VortexPairSetup.SpongeLayer(
    sponge_layer_min = (
        -135 * r0,
        100 * r0,
        -135 * r0,
...
semi_acoustics = SemidiscretizationHyperbolic(
    mesh, equations_acoustics,
...
callbacks_averaging = CallbackSet(
    summary_callback, alive_callback, averaging_callback,
    stepsize_callback
)
...
sol_averaging = solve(
    ode_averaging, CarpenterKennedy2N54(williamson_condition = false),
...
euler_acoustics_coupling = EulerAcousticsCouplingCallback(
    ode_euler, "out/averaging.h5",
    CarpenterKennedy2N54(williamson_condition = false),
    cfl_acoustics, cfl_euler,
    callback = SaveRestartCallback(
        interval = 2300,
...
callbacks = CallbackSet(
    summary_callback, alive_callback, analysis_callback, save_solution,
