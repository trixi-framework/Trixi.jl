...
@muladd begin
    ...
    function global_plotting_triangulation_makie(
            pds::PlotDataSeries{<:PlotData2DTriangulated};
        ...
        for element in Base.OneTo(num_elements)
            ...
            trimesh[element] = GeometryBasics.normal_mesh(
                Makie.to_vertices(coordinates),
                makie_triangles
            )
        end
    ...
    function convert_PlotData2D_to_mesh_Points(
            pds::PlotDataSeries{<:PlotData2DTriangulated};
            set_z_coordinate_zero = false
        )
        @unpack variable_id = pds
        ...
        # plotting.
        xyz_wireframe = GeometryBasics.Point.(
            map(
                x -> vec(
                    vcat(
                        x,
    ...
    function iplot(
        ...
        plotting_mesh = Makie.@lift(
            global_plotting_triangulation_makie(
                getindex(
                    pd,
        ...
        wire_points = Makie.@lift(
            convert_PlotData2D_to_mesh_Points(
                getindex(
                    pd,
                    variable_names[$(menu.selection)]
                )
        ...
        Makie.translate!(wire_mesh_top, 0, 0, 1.0e-3)
        Makie.translate!(wire_mesh_bottom, 0, 0, -1.0e-3)
        ...
        function scaled_extrema(x)
            ...
            if ex[2] ≈ ex[1] # if solution is close to constant, perturb colorbar
                return ex .+ 1.0e-5 .* maximum(abs.(ex)) .* (-1, 1)
            else
                return ex
            end
        end
        ...
        Makie.Colorbar(
            fig[1, 3], limits = Makie.@lift(scaled_extrema($solution_z)),
            colormap = colormap
        )
    ...
    function iplot(
        ...
        fig_axis_plt = iplot!(
            FigureAndAxes(fig, ax), pd; colormap = colormap,
            plot_mesh = plot_mesh
        )
    ...
    function iplot!(
            fig_axis::Union{FigureAndAxes, Makie.FigureAxisPlot},
            pd::PlotData2DTriangulated{<:ScalarData};
    ...
    function Makie.plot!(myplot::TrixiHeatmap)
        ...
        plotting_mesh = global_plotting_triangulation_makie(
            pds;
        ...
        Makie.mesh!(
            myplot, plotting_mesh, color = solution_z, shading = false,
        ...
        if plot_mesh
            xyz_wireframe = convert_PlotData2D_to_mesh_Points(
                pds;
                set_z_coordinate_zero = true
            )
    ...
    function Makie.plot(
        ...
        return Makie.plot(
            PlotData2DTriangulated(sol; solution_variables); plot_mesh,
            colormap
        )
    end

    function Makie.plot(
            pd::PlotData2DTriangulated, fig = Makie.Figure();
    ...
    function Makie.plot!(
        ...
        axes = [
            Makie.Axis(fig[i, j], xlabel = "x", ylabel = "y")
                for j in 1:rows, i in 1:cols
        ]
        row_list, col_list = (
            [i for j in 1:rows, i in 1:cols],
