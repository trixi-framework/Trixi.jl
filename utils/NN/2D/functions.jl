...
function trainfunction(func,x,y)
    ...
    elseif func == 3
        ...
        u = a[1]*sin(2*pi*x)+b[2]cos(2*pi*y)
    elseif func == 4
...
function good_cell(node_coord, length, func, m, x0, y0)
    ...
    if func == 1
        if -(1/2)*length < (y-y0)-m*(x-x0) && (y-y0)-m*(x-x0) <(1/2)*length 
            return false
        else 
            return true
        end
    elseif func == 2
        if (-(1/2)*length < y-y0-m*(x-x0) && y-y0-m*(x-x0) < (1/2)*length) || (-(1/2)*length < y-y0+(1/m)*(x-x0) && y-y0+(1/m)*(x-x0) < (1/2)*length) 
            return false
        else
            return true
        end
