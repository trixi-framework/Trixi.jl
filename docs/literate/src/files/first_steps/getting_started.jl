...
# Here, ``\rho`` is the density, ``v_1`` and ``v_2`` are the velocities, ``e_{total}`` is the specific
# total energy, and
# ```math
...
# initial values for ``\rho``, ``\rho v_1``, ``\rho v_2`` and ``\rho e_{total}``. One of the common initial
