...
# named advection velocity, suitable for our problem is (0.2, -0.7).

advection_velocity = (0.2, -0.7)
equations = LinearScalarAdvectionEquation2D(advection_velocity)

# To solve our problem numerically using Trixi.jl, we have to define an instruction for spatial
# discretization. To do it, we set up a mesh. The most widely used in Trixi.jl is
...
# [step controll callbacks](https://docs.sciml.ai/DiffEqCallbacks/stable/step_control/) that are
...
# that is complemented to 6 digits with zeros from the begining. With [Trixi2Vtk](@ref) you
