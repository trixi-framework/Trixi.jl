...
# of the most important parts required for succesfull mesh generation that can later be used by the `p4est` library
# and `Trixi.jl`.
...
# In principle, it should also be possible to use the recombine function of `gmsh` to convert the traingles to quads, 
