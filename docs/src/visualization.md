...
keyword argument to `Trixi2Vtk.convert`, which causes Trixi2Vtk to generate
...
multiple input files as the positional arguments to `Trixi2Vtk.convert`, e.g.,
```julia
julia> Trixi2Vtk.convert("out/solution_000000.h5", "out/solution_000040.h5")
```
You may also use file globbing to select a range of files based on filename patterns, e.g.,
```julia
julia> Trixi2Vtk.convert("out/solution_*.h5")
```
to convert all solution files in the `out/` directory or
```julia
julia> Trixi2Vtk.convert("out/restart_00[0-9]000.h5")
```
...
`Trixi2Vtk.convert` can be found in the [Trixi2Vtk.jl API](@ref).
...
`Trixi2Img.convert` or use file globbing, e.g.,
```julia
julia> Trixi2Img.convert("out/solution_*.h5")
```
...
`Trixi2Img.convert` can be found in the [Trixi2Img.jl API](@ref).
