...
# `@__BINDER_ROOT_URL__` and `@__NBVIEWER_ROOT_URL__` should work on GITHUB-Actions automatically 
binder_url = joinpath("https://mybinder.org/v2/gh/bennibolm/Trixi.jl/tutorials?filepath=docs/src/tutorials/notebooks/")
nbviewer_url = joinpath("https://nbviewer.jupyter.org/github/bennibolm/Trixi.jl/tree/tutorials/docs/src/tutorials/notebooks/")
...
Literate.markdown(joinpath(repo_src,"index.jl"), pages_dir; name="index", documenter=false,
                  execute=true, codefence="```julia" => "```", preprocess=preprocess_docs)
...
for (i, (title, filename)) in enumerate(files)
    ...
    # TODO: With `documenter=true` no references are written to notebook file.
    #       With `documenter=false` they are existing but not working right now.
    Literate.notebook(joinpath(repo_src,filename), notebooks_dir; name=tutorial_file, documenter=false,
